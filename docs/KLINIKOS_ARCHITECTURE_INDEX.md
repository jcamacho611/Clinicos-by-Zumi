...
23. Legacy briefs, stale pricing estimates, old PR bodies, old brand hierarchy, and superseded dark/game-like design experiments as history only.
...
2. Fetch latest `main`, open PRs, relevant branches and CI.
3. Read `SOURCE_OF_TRUTH.md` for current state — not for law.
...
4. Read `KLINIKOS_KNOWLEDGE_TO_ARCHITECTURE_LEDGER.md` for material cross-domain architecture, Clinic OS, profession/authority, scheduling/capacity, patient/financial case, clinical composition/change, orders/results, integration, revenue, credentialing, Grid, EDU, Provider, Patient, Network, reference-environment or Zumi work.
5. Read `KLINIKOS_ECOSYSTEM_CANON.md`.
6. For any frontend, product design, role journey, route, mobile, accessibility, Figma/prototype, public experience, clinical UI, Grid UI, EDU UI, billing UI, portal, or enterprise UI work, read `KLINIKOS_UNIVERSAL_FRONTEND_AND_USER_OUTCOMES_CANON.md` and `FRONTEND_EXPERIENCE_CANON.md`.
...
8. For historical design evidence read `KLINIKOS_DESIGN_PACKAGE_AUTHORITY_2026-08-16.md` and `KLINIKOS_DESIGN_AND_WIRING_CANON.md`, applying the newest approved visual/product-clarity law when they conflict.
9. For pricing/commercial work read `KLINIKOS_PRICING_AND_MONETIZATION_CANON.md`.
10. Read `FEATURE_STATUS.md` and relevant code/tests before claiming something is built.
11. Read `EXTERNAL_DEPENDENCY_MATRIX.md` before claiming an external rail is connected.
12. Read the relevant specialist spec and journey evidence.
13. For recovery work, read `BRANCH_LEDGER.md` and `RECOVERY_AND_COMPLETION_ROADMAP.md`.
14. Resolve conflicts explicitly rather than silently choosing old material.
...
- Universal frontend/user-route/design decisions → `KLINIKOS_UNIVERSAL_FRONTEND_AND_USER_OUTCOMES_CANON` + `FRONTEND_EXPERIENCE_CANON` + `SOURCE_OF_TRUTH` when the repository-wide design invariant changes.
