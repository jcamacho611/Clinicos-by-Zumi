# KLINIKOS — BBFH 2027 FINAL EXECUTION DELTA

Status: DECISION READY — NOT SUBMITTED / NOT SELECTED / NOT AWARDED

Verified: 2026-08-26

## Purpose

This file overrides stale BBFH execution assumptions in older working packages. It is the final completion gate for the direct 2027 Building Better Futures for Health Challenge application.

## Current verified external facts

- Official BBFH 2027 applications are open.
- Applications close **2026-08-31 at 16:59 Sydney time**.
- Semi-finalists are announced **2026-10-15** on the current public challenge page.
- Semi-finalist showcase: **2026-11-12**.
- Grand Finals: **2027-03-03**.
- Winner receives **AUD $30,000 of IDE Group services**.
- People's Choice receives **AUD $10,000 of IDE Group services**.
- These prizes are services, not cash.
- Published judging criteria emphasize impact, novelty, and growth potential.
- IDE Group's public eligibility language says international corporations must intend to conduct some or part of the research, development, manufacture, or commercialisation of the product/service in Australia.

Primary public authority: https://ide.group/bbfh-challenge/
Application form / live terms authority: https://ide.group/2027-bbfh-application-form/

## Live form requirements — verified 2026-08-26

### Basic Information

Required:
- Full Name
- Contact Number
- Email
- Base Location
- Date Founded
- Country where company is domiciled
- Key Area

Not marked required on the live form:
- Alternative Phone Number
- How did you hear about us?
- Company Name
- Company Website
- **Number of Employees**

Therefore **employee count is not a submission blocker**. Leave it blank rather than inventing current headcount.

### Vision

Required:
- Vision / product-business objective — 175 words
- Difference to people's lives — 150–200 words

### Desirability

Required:
- Clinical problem/challenge — 150–200 words
- Patient/caregiver benefit — 150–200 words
- Usability / human-factors benefit — 150–200 words

### Feasibility

Required:
- Development stage — 20 words
- Clinical evidence — 150–200 words

Not marked required:
- Technical support showing product can work — 150–200 words

Recommendation: complete it anyway because the existing answer is strong and evidence-bounded.

### Viability

Required:
- Primary / secondary markets — 20 words
- Proposed regulatory pathway — 20 words
- Business model — 150–200 words
- Competitor analysis — 150–200 words

Not marked required:
- How Klinikos will make a profit — 150–200 words

Recommendation: complete it anyway because the answer already exists and materially strengthens viability.

### Team

Required:
- Founder background / experience — 150–200 words
- Supporting team — 150–200 words

Not marked required:
- Advisors / board summary — 150–200 words

Do not invent formal advisors or board roles.

### BBFH Utilisation

Required:
- Prize utilisation / impact of IDE support — 150–200 words
- Sydney Semi-Final attendance on 2026-11-12 — Yes/No
- Sydney Final attendance on 2027-03-03 — Yes/No
- Australian market/collaboration interest — Yes/No

A further-information field follows the Australian-market question. The evidence-backed Australia pathway and ready-to-paste follow-up answer now live in:

`docs/business/funding/BBFH_AUSTRALIA_COMMERCIALISATION_PATHWAY_2026-08-26.md`

Use that answer only if the authorized representative genuinely adopts that pathway. Do not convert a planning document into an attestation by wording.

### Submission / supporting document

Required:
- Answer whether supporting documents will be submitted — Yes/No

If Yes:
- one combined file only;
- accepted page-listed formats include PDF/DOC/DOCX/PPT/PPTX;
- maximum upload size: 20 MB.

Use the already-prepared non-confidential BBFH supporting packet rather than sending proprietary implementation material.

## Canonical company facts for form population

- Legal name: **Klinikos, Inc.**
- Entity: New York Business Corporation
- NY DOS document number: **8001871**
- Exact filed date: **2026-08-20**
- County: Kings
- Business address: **1210 Nostrand Ave, Brooklyn, NY 11225**
- Website: **https://klinikos.io**
- Product description: **Digital health / governed AI healthcare operations software**
- Primary market: U.S. outpatient practices and multi-location clinics
- Current evidence posture: working software platform; early commercialization / production hardening; no invented clinical outcome evidence, regulatory clearance, customers, revenue, or Australian operations

## Public website / demo rule

Immediately before final submission:

1. attempt to load the public website and the strongest safe demo/supporting artifact;
2. if production is unavailable or stale, do not characterize it as a healthy live production deployment;
3. use the non-confidential supporting packet and truthful working-software evidence instead of sending judges to a broken surface;
4. never disclose PHI, customer confidential information, source code, unpublished patentable mechanisms, proprietary ranking/policy logic, or unnecessary security internals.

## Final application answer source

Use the answer bank in:

`docs/business/funding/BBFH_MEDICAL_ALLEY_APPLICATION_WORKING_PACKAGE_2026-08-25.md`

Do not rewrite essays from scratch unless the live form changed.

## Australia-intent decision support now resolved

The earlier gate said only that Australia intent had to be genuine. That left the founder deciding against an undefined concept. The repo now contains a concrete minimum viable pathway:

**BBFH/IDE Australian product-development work → Australian workflow/market validation → controlled configuration/pilot only if evidence supports it → Australian entity/location decision only if justified.**

Official Austrade and Investment NSW evidence supports Australia/NSW as a credible digital-health and medtech commercialisation environment. This does not create current Australian operations or eligibility for Australian grants.

If the authorized representative genuinely intends to pursue at least the product-development + market-validation stages if selected and commercially reasonable, the live form's Australia follow-up can use the prepared answer in the pathway file. If not, do not manufacture the intent.

## Remaining non-delegable decisions

Only an authorized company representative should complete these items:

1. **Australia intent** — adopt or reject the concrete pathway in `BBFH_AUSTRALIA_COMMERCIALISATION_PATHWAY_2026-08-26.md` and answer the form truthfully.
2. **Sydney attendance** — answer the November 12, 2026 semi-final and March 3, 2027 final attendance questions from actual travel capability/commitment.
3. **Terms/IP/publicity** — review the live competition terms, including: broad non-exclusive entry-content license; originality/IP warranty and indemnity; personal-information/marketing permissions; winner publicity; entrant-paid participation/travel; and the requirement that prize services are subject to IDE's standard product-development agreement.
4. **Final legal acceptance / Submit** — by applying the entrant agrees to the published terms, so final application submission remains an authorized-company legal action.

## Exact completion sequence

1. Open the official application form and accept the cookie setting required for all embedded form fields to load.
2. Populate all routine company fields from the canonical facts above.
3. Leave Number of Employees blank unless the authorized representative chooses to provide a verified current number.
4. Paste the already-prepared Vision, Desirability, Feasibility, Viability, Team, and Prize Utilization answers from the working package.
5. Complete the optional technical-support and profit fields because strong evidence-bounded answers already exist.
6. For supporting documents select Yes and attach the single non-confidential combined packet if the file remains under the current 20 MB limit.
7. Use the Australia pathway answer only after genuine adoption by the authorized representative.
8. Complete the Sydney attendance decisions and legal terms review.
9. Submit only after authorized legal acceptance.
10. Record submission evidence only after the portal actually confirms receipt.

## Truth boundary

Application prepared != application submitted.
Program fit response != endorsement.
Working software != verified clinical outcome.
Prize amount != cash.
Australian interest != Australian operations.

Do not change those classifications for persuasive wording.
