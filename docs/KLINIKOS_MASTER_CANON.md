...
This law specifically protects Grid; EDU; students; schools, colleges, universities, and workforce boards; placements and preceptors; professional/public profile projections; résumé evidence; governed professional progressions; Med Spa; Clinic OS; Current Visit; patients/caregivers; No-Fault and Workers’ Compensation; RCM; Expert Grid; enterprise; RFP/procurement; Pricing Fabric; Zumi; all five planes; and every governed Path accepted into this Canon. A later conversation changing subject does not retire an accepted lifecycle.

## 0.6 SOURCE-TO-CANON / BLUEPRINT COMPLETENESS GATE

Accepted requirements may not live only in a chat, interview, prototype, screenshot, PR comment, raw transcript, design handoff, or historical prompt. Before an authority-convergence PR can merge, each accepted source requirement must be traced to:

`SOURCE / PROVENANCE → MASTER CANON LAW → ENGINEERING BLUEPRINT CONSEQUENCE → MACHINE CONTRACT / ROUTE / SCREEN CONTRACT / STATE / POLICY WHERE APPLICABLE → CURRENT IMPLEMENTATION OR EXPLICIT GAP → TEST / EVIDENCE / OWNER / KPI / COMMERCIAL CONSEQUENCE`.

The raw source remains a provenance receipt. It is not a sibling specification and does not become another source of truth. A completeness sweep fails when an accepted requirement exists only in provenance, when implementation status is inferred rather than verified, or when a clinical, operational, design, pricing, security, or business consequence was summarized away.

**PATIENT / PROVIDER / GRID / EDU / ENTERPRISE PROPAGATION** is part of completeness. Any accepted change to identity, relationship, authority, clinical evidence, configuration, financial state, experience, pricing, retention, or external-rail truth must be evaluated across every affected projection. A requirement is not reconciled merely because it appears in one domain section while downstream patient, provider, Grid, EDU, organization, network, enterprise, Zumi, audit, and commercial consequences remain unspecified.

The Engineering Blueprint must classify each accepted build consequence through **`REUSE → EXTEND → GENERALIZE → CONNECT → PARTNER → BUILD NEW`**, name the existing kernel or external authority where one exists, state why the chosen consequence applies, and preserve a verified gap rather than inventing implementation. `BUILD NEW` is last, not the default.

## 0.7 Merge-forward law
...
## 6.3 Patient identity, documents, relationships, and reconciliation

**CURRENT RECOGNITION PHOTO ≠ GOVERNMENT PHOTO IDENTIFICATION.** A current patient-recognition image may be captured or uploaded as a clinical/operational projection used to help the care team recognize the right person. A government photo-identification document is a separately governed identity-evidence artifact with its own capture/upload, source, verification, access, retention, expiration, replacement, and disclosure rules. Neither may silently overwrite the other, and raw identity documents must not be projected wherever a recognition image is sufficient.

**QUICK REGISTRATION ≠ COMPLETE REGISTRATION.** Quick registration captures the minimum safe identity, contact, encounter, consent/readiness, and duplicate-check inputs required for the immediate purpose, then creates explicit unfinished work. Complete registration resolves the governed demographic, relationship, document, consent, coverage/FinancialCase, directive, communication-preference, and other policy-required evidence without forcing duplicate entry or falsely blocking lawful urgent care.

The relationship model explicitly supports **NEXT OF KIN / SPOUSE / PARENT OR GUARDIAN / CAREGIVER / EMERGENCY CONTACT / HEALTHCARE PROXY / AUTHORIZED REPRESENTATIVE** and other lawful relationship classes. Each class remains independently effective-dated, attributable, revocable where applicable, and scoped to its actual legal/operational meaning.

**RELATIONSHIP EXISTS ≠ CONTACT PERSON ≠ EMERGENCY CONTACT ≠ AUTHORIZED DISCLOSURE RECIPIENT ≠ HEALTHCARE PROXY ≠ GUARDIAN ≠ CHART ACCESS AUTHORITY.** Kinship and contact data are first-class effective-dated relationships, but relationship alone grants no chart access, consent authority, disclosure authority, or ability to act for a patient. Scope, purpose, legal basis, patient capacity, consent/directive state, dates, revocation, and audit remain explicit.

Patient identity also preserves versioned consent, advance-care/advance-directive state, immunization/vaccination evidence, identity assurance, and document provenance without converting a document upload into verified truth.

**PATIENT DUPLICATE MATCH / RECONCILIATION** is a governed human-review workflow. Candidate similarity is not an automatic merge. Klinikos must preserve both source identities, explain matching evidence, prevent cross-tenant leakage, require authorized resolution, maintain reversible linkage/merge provenance where practicable, and retain a durable audit of what changed.

## 6.4 Provider-context readiness and chart authority

Clinical readiness is contextual:

`PROVIDER × ORGANIZATION × LOCATION × SPECIALTY / PROFESSION × SERVICE / APPOINTMENT TYPE × PAYER / FINANCIAL CASE × CREDENTIAL / PRIVILEGE / SUPERVISION × SCHEDULE POLICY × JURISDICTION × TIME`.

A professional may be licensed but not privileged at a location, trained for one service but not another, insured for one activity but not another, or unenrolled with the payer/case required for billable work. License, credential, malpractice, supervision/collaboration, facility privilege, service scope, payer enrollment, organization relationship, availability, and location readiness are separate effective-dated truths.

**ADMIN AUTHORITY ≠ CHART AUTHORITY.** Organization administration, scheduling, billing, configuration, or ownership never automatically grants clinical-record access. Chart access requires the relevant care/operations relationship, purpose, assignment, minimum-necessary scope, and policy. Break-glass access, where legally and operationally permitted, requires explicit reason, heightened audit, notification/review, time limit, and post-event reconciliation.
...
## 7.1 MF-001 — UNIVERSAL FREE ECOSYSTEM ENTRY

People and organizations should enter a useful governed ecosystem, not register for an empty software account. Canonical entry is:

`DISCOVERY → PUBLIC ZUMI / SEARCH / REFERRAL / INVITATION → JOIN FREE → ONE KLINIKOS IDENTITY → INTENT → BASIC PROFILE → RELATIONSHIPS / ORGANIZATION CONTEXT → GRID PRESENCE WHERE APPLICABLE → RELEVANT EDU / CARE / WORK / ORGANIZATION PATHS → FIRST USEFUL RESULT → LIVING HOME → PROGRESSIVE VERIFICATION → DEEPER PARTICIPATION`.

**NO CREDIT CARD IS REQUIRED MERELY TO EXIST IN THE KLINIKOS ECOSYSTEM.** Payment never creates identity, verification, eligibility, relationship authority, professional authority, or access to another tenant’s data.

The free experience provides **FREE PARTICIPATION AND FIRST VALUE** appropriate to purpose and policy: one identity, a basic profile, lawful need/resource expression and discovery, basic Grid presence, relevant free EDU experiences, basic Zumi guidance, invitations/organization connection, Path participation, legitimate evidence-building, and a clear next action. “Free” must be truthful about included capabilities and limits.

**PAID CAPABILITY FOLLOWS ADDITIONAL ECONOMIC VALUE**—advanced automation, higher usage, professional or organizational operations, lawful transactions, premium education, Clinic/Financial OS, enterprise controls, implementation, integrations, or support. Paid capability must not manufacture professional authority or create artificial scarcity around identity, portability, safety, or lawful access.

**MF-002 — ECOSYSTEM PASSPORT.** The **ECOSYSTEM PASSPORT** is a human-readable projection of governed truth, not a credential. For a person it may summarize identity, relationships, claims, verified evidence, EDU, Grid status, credentials, organizations, work, reputation, and next action. For an organization it may summarize identity, locations, people, operations, capacity, Grid needs/resources, financial state, quality, integrations, workforce/EDU, and next action. Each field preserves its own source, verification, visibility, authority, and effective-time semantics.

## 7.2 MF-003 — LIVING UNIVERSE INTERACTION LAW AND ROUTE / PATH LAW

The **LIVING UNIVERSE** is the production experience projection of the five-plane Canon, not another plane, database, authority, or runtime. Its interaction loop is:
...
Core projection primitives are **`UniverseShell / LivingHome / ObjectStage / PlaneLens / PathConstellation / NarrativeTimeline / Inspector / RelationshipGraph / SpatialView / ActionDock / ZumiCommandSurface / EcosystemPassport / LivingEdge / ExceptionQueue`**. They project canonical objects and policy; they do not copy domain truth or replace ordinary calm task surfaces when spatial/graph views add no value.
...
The **recruiter-facing résumé provenance use case** requires review that can understand education, clinical experience, strengths, professional stage, goals, growth direction, support needs, and fit before interview while preserving claim-versus-verified distinctions. Personal names and private source discussions remain provenance, not product labels or fixtures.
...
## 9.2 MF-005 — INSTITUTIONAL EDU / WORKFORCE BOARD / GOVERNMENT TRAINING DELIVERY

**SCHOOLS, COLLEGES, UNIVERSITIES, AND WORKFORCE INSTITUTIONS ARE FIRST-CLASS ORGANIZATIONS.** They use the same identity, relationship, EDU, Grid, evidence, entitlement, communication, reporting, and enterprise kernels rather than receiving a separate LMS, student marketplace, or product fork.
...
Institutional delivery may include program/curriculum versions, cohorts/sessions, faculty/instructor/preceptor relationships, learner relationships, scheduling, attendance, assignments/simulations, rubrics/assessment, remediation, human completion approval, certificates, placement approvals, employer/site relationships, accessibility, reporting, issue/corrective-action tracking, and renewal/expansion. **PORTABLE ACHIEVEMENT EVIDENCE** preserves source, version, evaluator, learner confirmation where applicable, verification and release controls; it is not a license or professional authority.

Placement remains multi-party: **`PLACEMENT MATCH ≠ SCHOOL APPROVAL ≠ SITE APPROVAL ≠ LEARNER ACCEPTANCE ≠ PRECEPTOR / SUPERVISOR ASSIGNMENT ≠ COMPLETION ≠ LICENSURE`**. Clinical-hour events are attributable, auditable, correctable without silent overwrite, and reviewable by the authorized parties.

The recovered workforce-board program is a **WORKFORCE PROGRAM CONFIGURATION, NOT A PRODUCT FORK**. Its source-locked configuration includes an **AI INDUSTRY ACCELERATOR** (representative 6–8 hour pathways across manufacturing, construction, logistics, healthcare, and business operations) and separate **AI-POWERED CAREER READINESS** (representative 2–3 hour program), using live instruction, applied AI, occupational simulation, intentionally flawed outputs, verification, privacy/security judgment, human escalation, assessment, instructor review, attendance/completion evidence, accessibility, reporting, and continuous improvement. It follows **SYNTHETIC-DATA-FIRST PRACTICE** and remains configurable/versioned; inclusion here does not claim an active contract, award, deployed curriculum, or customer outcome.
...
**COVERAGE ≠ FINANCIAL CASE.** Coverage represents an effective-dated insurance/self-pay/benefit relationship and verification state. A financial/legal case represents the governed episode under which services, authorizations, accident/employment facts, documents, parties, billing rules, liens/assignments where lawful, claims, balances, and reconciliation are organized. One person may simultaneously or sequentially have commercial coverage, multiple No-Fault matters, Workers’ Compensation, and self-pay without one context overwriting another.

Recurring care uses an **APPOINTMENT SERIES**, not manually duplicated appointments. The series stores recurrence rule, care plan/need, participants, service, location/telemedicine mode, resource requirements, payer/case context, start/end/exception rules, and generation policy. Each occurrence remains independently reschedulable, cancellable, fulfillable, documentable, billable, auditable, and linked to its series.
...
**EHR MIGRATION / OVERLAY** is a governed program, not a bulk-copy shortcut. It requires source inventory, data classification, mapping/version rules, identity and duplicate reconciliation, authorization, trial import, validation, exception queues, provenance, customer/clinical sign-off, cutover or overlay boundaries, rollback, archive/read-only policy, retention/deletion obligations, and post-migration reconciliation. Klinikos must preserve source identifiers and evidence, must not infer clinical truth from malformed legacy data, and must not call a migration complete because files were uploaded.

## 13.1 First-clinic operating provenance

The **first-clinic operating provenance** establishes the practical wedge: a real clinic needs a simpler, lower-cost workflow than fragmented incumbent tooling, including schedule → intake → Current Visit → telemedicine → documentation → billing-ready → follow-up. Personal names remain in private provenance only. This use case informs product priority; it does not waive production, privacy, billing, interoperability, or safety requirements.
...
The **ONE-PAGE CURRENT VISIT** is one coherent, progressive clinical workspace—not one literal print page and not a shallow summary. It must answer, at minimum: who the patient is; why they are here; the active coverage/FinancialCase; what existed before; what changed; what staff collected; what is missing; which reports/results/procedures matter; what is done; what remains pending; what the authorized provider must decide today; and the next action. Secondary detail opens in context without forcing the provider through unrelated modules or duplicate entry.
...
Clinical change states may include `NEW / IMPROVED / WORSE / UNCHANGED / RESOLVED / PENDING / NEWLY_DOCUMENTED / UNRESOLVED` where evidence supports them. `NEWLY_DOCUMENTED` means newly recorded evidence, not automatically a new condition; AI may explain supplied evidence but may not invent change.

The **VERSIONED BODYMAP** preserves region, laterality, finding/symptom, severity, annotation, functional impact, source, encounter, timestamp, and author for each version. Corrections or later observations supersede/link without overwriting history, and `INITIAL → PREVIOUS → TODAY` comparisons must disclose their evidence and unknowns.

**STAFF INTAKE IS SHALLOW, ENCOUNTER-SPECIFIC, AND REUSED.** Front desk and MA/LPN/RN workflows capture only what their role, assignment, and visit require—identity/readiness exceptions, reason for visit, vitals, medication/allergy confirmation, targeted history/screening, BodyMap observations, documents, and concerns. The provider receives an attributed handoff and may deepen/correct it; the system does not ask the patient or staff to re-enter governed data already captured for the same purpose.

The **PROCEDURE / TREATMENT TIMELINE** preserves attributable, effective-dated treatments and procedures such as injections/trigger points, epidural or other interventions, surgery, EMG/EEG, imaging, physical therapy, consultations, shockwave, Botox or other governed services where applicable. Each event carries source, performer/organization/location, order/indication, date, area/laterality, product/device/lot/dose when required, consent, procedure note, outcome/adverse event, related evidence, follow-up, coding candidates, and billing-readiness state. A procedure entry does not by itself prove indication, completion, billability, payment, or outcome.
...
The governed ambient path is **CONSENT → VISIT AUDIO / DICTATION → SOURCE-PRESERVING TRANSCRIPT → EVIDENCE-LINKED DRAFT → MISSING-INFORMATION QUESTIONS → HUMAN EDIT / ACCEPT / REJECT → AUTHORIZED PROVIDER SIGNATURE**. Audio consent, recording state, speakers, timestamps, source spans, model/version, prompts/policies, generated assertions, edits, rejection, signature, retention, and deletion/legal-hold status remain attributable. Ambient assistance never invents a fact or converts inference into signed clinical truth.

Coding assistance follows **DOCUMENTED CLINICAL EVIDENCE → CODING ASSISTANCE → EXPLANATION / RATIONALE → RULE / TERMINOLOGY VERSION → AUTHORIZED HUMAN REVIEW → BILLING-READY EVIDENCE**. It may prepare candidates and identify gaps; it may not diagnose, select final codes without authorized review, prove medical necessity, guarantee coverage, or submit a claim by implication.

The **PROVIDER → BILLER STRUCTURED HANDOFF** carries signed/locked documentation state, supported diagnoses/procedures, place of service, rendering/billing/facility identity, payer/FinancialCase, authorization/evidence, expected charges, exceptions, attachments, provider attestations, and unresolved work. The handoff does not make an incomplete visit billable.
...
The **NO-FAULT LONGITUDINAL MULTI-SPECIALTY CASE** composes the same patient, encounter, clinical-change, order/result, document, referral, report, procedure, coding, claim, payment, and network substrates across applicable EMG/neurology, orthopedics, pain management, physical therapy, behavioral health/psychiatry, imaging, laboratory, surgery/procedures, and other participating specialties. Every external report preserves source organization, author/provider, specialty, service date, report date/version, impression/findings, receipt, review, communication, case linkage, and follow-up obligation.

**WORKERS’ COMPENSATION REUSES THE FINANCIAL CASE ARCHITECTURE** with jurisdiction-specific employer, carrier, claim, injury, work-status, authorization, deadline, form, document, party, billing, payment, denial/appeal, and reporting rules. It does not create a second patient, encounter, order/result, document, billing, or integration kernel, and one case never overwrites another coverage or FinancialCase context.

**CLOSE VISIT IS EXCEPTION RESOLUTION**, not merely a Sign button. It makes missing documentation, unsigned drafts, unresolved orders/results, required communication, coding/billing-readiness gaps, supervision/co-signature, consent, follow-up, and other blockers visible; it assigns or resolves each permitted exception before the authoritative encounter state advances.

Clinical behavior is composed through:

`KLINIKOS CORE + SPECIALTY PACK + ORGANIZATION CONFIG + LOCATION OVERRIDE + PROVIDER-PERMITTED CUSTOMIZATION`.

**SPECIALTY PACKS ARE PRODUCTS, NOT FORKS.** A specialty pack may version/configure questionnaires, clinical components, templates, screenings, appointment types, order favorites, workflows, reports, follow-up rules, coding-support rules, quality measures, forms, education, terminology, and display behavior. It may not redefine identity, tenant security, chart authority, credential truth, payment truth, audit, the core patient record, or the core encounter architecture.

**SPECIALTY STARTER PATHWAYS ARE CONFIGURATION, NOT CORE FORKS.** Initial No-Fault and other specialty template/pathway libraries may begin with a finite set of common governed workflows (including the approximately twelve initial No-Fault pathways recovered from clinical provenance), but they remain versioned starter content built from shared components. They may be extended, reviewed, and configured without hard-coding the product around one specialty, customer, or document length.

The **CONFIGURATION REGISTRY** resolves base → specialty → organization → location → provider-permitted layers with version, effective dates, provenance, approval, validation, conflict rules, preview, migration, rollback, and an explainable resolution trace. No specialty or customer receives a forked core codebase merely because configuration differs.
...
Clinical/preventive state includes versioned advance-care directives and immunization evidence with source, status, effective dates, review, refusal/contraindication where applicable, and correction history. A document attachment alone does not prove that a directive is current or an immunization occurred.
...
The **UNIVERSAL SERVICE ORDER** represents a requested governed action—laboratory, imaging, medication, referral/consultation, PT/rehabilitation, nursing action, procedure/surgery, DME, home instruction, or other permitted class—without creating a separate order engine for each specialty or vendor. It carries requester/authorizer, patient/encounter/FinancialCase, order class, coded request and version, indication/evidence, priority, timing, performing destination, prerequisites, status, amendments/cancellation, external identifiers/acknowledgement, result links, review/communication obligations, provenance, and audit. Order availability and required details remain contextual and authoritative compendia/configuration—not AI guesses—own specimen, tube, fasting, handling, medication, device, and other regulated requirements.
...
**RESULT RECEIVED ≠ FINAL ≠ REVIEWED ≠ COMMUNICATED ≠ FOLLOW-UP COMPLETED.** Result source, preliminary/final/corrected status, version, abnormal/critical indicators from the authoritative source, receipt, patient/encounter/case linkage, responsible reviewer, review decision, communication method/attempt/recipient, follow-up task/order, completion, and exception remain separately evidenced.

**CORRECTED RESULT REQUIRES RE-REVIEW.** A correction or amendment supersedes the prior result without erasing it, invalidates any closure that depended on the changed content where policy requires, creates a new attributable review obligation, and may reopen communication/follow-up work.

The **INTEGRATION RECONCILIATION WORKBENCH** exposes outbound intent, transmission attempts, idempotency key, acknowledgement, inbound payload/version, mapping, retry/backoff, duplicate handling, dead-letter state, owner, resolution, replay controls, and audit. An adapter or API key does not prove delivery, acceptance, clinical review, payment, or production readiness.

A **TERMINOLOGY RELEASE** is an effective-dated, source-attributed, licensed/authorized version of ICD-10-CM, CPT, HCPCS, modifiers, payer edits, specialty terms, mappings, and related rules. Historical records retain the version used; updates require validation, impact review, migration/compatibility policy, and rollback. Named vendors or code sets never define the core encounter/order/result model.
...
The **quality-expert provenance use case** captures how quality, audit, documentation, care-gap, reimbursement, and workflow expertise inform this lane. Personal names remain in private provenance only. Provenance does not imply a signed partnership or customer relationship.

The **QUALITY MEASURE / HEDIS FRAMEWORK** stores measure identity/version, steward/source/licensing constraints, population, denominator, exclusions/exceptions, numerator, time window, required evidence, effective dates, organization/program applicability, evaluation trace, human override/correction policy, tasks, outcome, and audit. Permanent law: **deterministic measure/rule logic owns status**; AI may explain, summarize, identify missing evidence, or prepare remediation, but it never manufactures measure compliance or replaces the effective-dated rule definition.
...
`CUSTOMER SELECTS → SERVER-OWNED CHECKOUT / AGREEMENT → VERIFIED PAYMENT EVENT → TENANT / SUBSCRIPTION / ENTITLEMENTS → NATIVE CAPABILITIES → EXTERNAL PROVISIONING IF REQUIRED → ONBOARDING → ACTIVE`.

Provisioning must be idempotent, retry-safe, recoverable, tenant-scoped, and audited. A failed external activation cannot roll back verified payment truth or masquerade as a usable connection; it creates explicit reconciliation/refund/remediation work under policy.
...
**THERE IS ONE ZUMI.** EDU, Grid, Clinic OS, Current Visit, Med Spa, Financial OS, enterprise, and company workflows use governed **DOMAIN CAPABILITY PACKS** and Screen Contract modes over the same policy/admission/provider/tool/audit substrate; they do not create separate bots with divergent memory, authority, safety, or cost truth.
...
**EVIDENCE DURABILITY ≠ INFINITE RETENTION.** Consequential event/audit history may be append-oriented or tamper-evident where required, while source records still obey classification-specific retention schedules, legal holds, contractual/regulatory preservation, lawful deletion, de-identification, archival, and tombstone/deletion evidence. “Immutable” never means retaining every copy forever or defeating user/customer data rights.
...
Public discovery should expose the actual breadth of Klinikos in audience language where relevant—including Clinic/EHR/PM, telemedicine, scheduling, patient experience, Current Visit/clinical change, orders/results, referral/network, RCM/claims, Grid work/services/space/capacity, professional verification, EDU/placements/preceptors, No-Fault/Workers’ Compensation, enterprise/interoperability, and security/trust—without implying every target is currently implemented, connected, or generally available.
...
The free/paid boundary follows the universal entry law: free maximizes legitimate participation and first value; paid monetizes additional economic value. Free identity/basic profile, appropriate discovery/participation, invitations, basic guidance, and portable user data must not be turned into artificial lock-in. Usage, automation, organization controls, implementation, external cost, premium programs, and lawful transactions may be monetized through current server-owned policy.

**MF-007 — PRICING STATES.** The **PRICING FABRIC** is the versioned commercial expression of the same configurable product architecture:

`BASE PLATFORM + LOCATIONS + OPERATING SCALE + SPECIALTY PACK(S) + ADVANCED CLINICAL + REVENUE / RCM + GRID / NETWORK + EDU + ZUMI / AI USAGE + INTEGRATIONS + IMPLEMENTATION + ENTERPRISE GOVERNANCE`.

These components configure one Klinikos; they do not create customer or specialty code forks. Each price generation and quote is classified `ACTIVE_PUBLIC / ACTIVE_PRIVATE / LEGACY_QUOTED / GRANDFATHERED / TARGET / SCENARIO / RETIRED` with offer/policy/version, effective dates, currency, billing interval, buyer/segment, person/provider/seat/organization/location/scale basis, inclusions, modules/specialty packs/integrations, limits, credits, allowances, usage/overage, add-ons, discounts/coupons/promotions, enterprise/contract overrides, external-cost treatment, implementation/deposit/milestone/change-order terms, entitlements, quote/contract references, refund/cancellation rules, transaction-specific fee policy, margin/cost recovery, migration/renewal/grandfathering behavior, approval, and public-presentation eligibility. `ACTIVE_PRIVATE` is not public pricing; `LEGACY_QUOTED` and `GRANDFATHERED` preserve contractual history without making stale pricing generally available.
...
**HISTORICAL PRICE GENERATION CONTEXT** remains provenance, not current public authority: prior work discussed a private Clinic Operating Analysis/workflow review around `$500`, a founding clinic evaluation/implementation blueprint around `$1,500`, implementation around `$8,000+`, and earlier recurring labels/figures such as Core `$1,495/month`, Command `$2,495/month`, Growth `$3,495/month`, and Complete `$4,995+/month`, plus historical add-on context. Preserve applicable quotes/contracts, but classify these values `LEGACY_QUOTED`, `GRANDFATHERED`, `HISTORICAL_ONLY`, or another governed non-public state unless the current executable registry explicitly activates them.

**TARGET / SCENARIO OFFER CONTEXT** may include free executive triage, an Operating Recovery Assessment, a Proof Sprint, Deployment, and Launch/Core/Growth/Enterprise recurring configurations. These are planning inputs only until approved, versioned, effective-dated, and activated by the server-owned Offer Registry; Canon inclusion never publishes or sells them.
...
Klinikos compounds five owned, portable, governed graph families across the fixed five planes:

- **CLINICAL CHANGE GRAPH** — how a patient’s condition, function, findings, treatment, results, and plan change over time;
- **HEALTHCARE EXECUTION GRAPH** — how needs, people, organizations, resources, authority, work, exceptions, and outcomes connect;
- **REVENUE INTEGRITY GRAPH** — how performed work, evidence, charge expectation, claim, adjudication, payment, and reconciliation connect;
- **CONFIGURATION GRAPH** — how core, specialty, organization, location, provider-permitted, contract, and release configuration resolves;
- **EVIDENCE GRAPH** — which source, actor, time, policy/rule/model version, review, and outcome supports each consequential assertion/action.

These are generated projections of canonical data and relationships, not a sixth plane, a second source of truth, or a promise of proprietary ownership over customer data. Portability, lawful deletion, de-identification, retention policy, clean offboarding, and customer data rights remain features.
...
Every requirement must trace to a source section, owner, response/evidence, product/configuration consequence, gap, risk, approval, and submission artifact. Buyer-specific commitments do not silently change product law or manufacture capability, certification, staffing, integration, price, customer, outcome, or award status.

**EACH BID MUST COMPOUND** into reusable buyer language, requirement patterns, security/accessibility answers, implementation/training plans, pricing/configuration evidence, legal/commercial clauses, objections, risk controls, reporting patterns, source citations, and verified outcomes where they exist. A loss still produces governed learning. Reuse is subject to confidentiality, IP, procurement, and customer-data boundaries.
...
**BLACK LABEL V2 IS A PRODUCT-WIDE EXPERIENCE CONVERSION, NOT A THEME.** It changes information hierarchy, workflow projection, navigation, interaction, responsive adaptation, accessibility, empty/error/blocked truth, and role/context continuity across real production routes. A color-token swap or isolated prototype is not completion.

**LIVING HOME IS THE UNIVERSAL AUTHENTICATED OPERATING FRONT DOOR.** It is conversational and role/context-aware, exposes current work and obligations, preserves persistent governed Zumi assistance, and routes people into only the relevant Klinikos experiences without becoming a brochure or generic dashboard.

The **PUBLIC HOMEPAGE** must explain the healthcare-fragmentation problem, Klinikos, Zumi, the five-plane system in human language, representative use cases, network value, trust boundaries, and an honest next action. The **LANDING FUNNEL** progresses `VISITOR → ROLE / ORGANIZATION → PROBLEM → OPERATING GAPS → SIGNALS → RECOMMENDED CONFIGURATION → PROOF → OFFER / CTA` without fake ROI, fake customers, or invented readiness.
...
Every approved prototype/reference receives a **DESIGN-TO-PRODUCTION CROSSWALK** recording purpose, user, public/authenticated state, route, production component, domain/plane, data/DTO, authorization, Zumi mode, interactions, responsive behavior, status, next action, tests, and evidence. Reference HTML/images are provenance inputs, never iframes, second runtimes, or proof that a route is wired.

Core narrative patterns include `BEFORE → NOW → NEXT`, `EXPECTED → ACTUAL → GAP`, `INITIAL → PREVIOUS → TODAY`, `NEED → AVAILABLE → ELIGIBLE → ACTION`, `ORDERED → RESULTED → REVIEWED → CLOSED`, `PERFORMED → CHARGED → CLAIMED → PAID`, and `LEARN → PRACTICE → EVIDENCE → COMPETENCY → OPPORTUNITY` where the domain supports them.

**NO DASHBOARD CARD WALL.** Cards may group local content, but the product must present one dominant purpose/object/action, progressive disclosure, narrative state, clear exceptions, and the next governed action. Do not turn every workflow into equal-weight tiles or bury the operational path below brochure content.
...
The Living Universe primitives named in §7.2 are the shared production vocabulary underneath these experiences. They must reuse the authenticated shell, Active Experience Envelope, Screen Contracts, real route registry, canonical objects/relationships, server-authoritative DTOs, Zumi controls, and accessibility foundations; they may not ship as a design-only second application.
...
## 46.1 MF-004 — BEFORE / NOW / NEXT UNIVERSAL GRAMMAR

The shared narrative grammar adapts by domain: clinical `INITIAL → PREVIOUS → TODAY → NEXT`; financial `EXPECTED → ACTUAL → GAP → NEXT`; Grid `NEED → ELIGIBILITY → MATCH → FULFILLMENT`; EDU `LEARN → PRACTICE → EVIDENCE → COMPETENCY → PLACEMENT`; provider `CLAIM → VERIFY → CREDENTIAL → PRIVILEGE → WORK`; customer `FREE VALUE → FIRST RESULT → PAID VALUE → OUTCOME → EXPANSION`; procurement `DISCOVERED → QUALIFIED → SUBMITTED → AWARDED / LOST → IMPLEMENTED → RENEWED`. This is a projection pattern, not permission to collapse the underlying state machines.

## 46.2 MF-008 — REPRESENTATIVE HUMAN JOURNEYS

Canon compression may not erase the reconstructable human paths that prove the five planes operate as one system. Preserve, at minimum:

- **Patient / caregiver:** public-safe value → private identity → registration → relationship/proxy/consent → Coverage and FinancialCase → scheduling → intake → Current Visit → orders/results → follow-up → claim/payment/outcome → recurring care/referral.
- **Provider / professional:** Person → claim/evidence → verification → organization/location/specialty/service/payer/privilege/schedule context → availability → care/work → fulfillment evidence → reputation → advancement.
- **Student / professional:** join free → school/program → profile/resume claims → EDU → practice/simulation → human-reviewed competency evidence → placement readiness → multi-party placement approval → hours/work → graduation → external licensure/credential verification → professional projection → Grid work → continuing development → mentor/preceptor/owner.
- **School / workforce institution:** institution → program/pathway → cohort/session → learner/instructor → attendance/learning/simulation/assessment → human review → completion evidence → placement/employer/site → reporting/invoice evidence where applicable → feedback/improvement → renewal/expansion.
- **Grid participant:** I NEED / I HAVE → requirements → eligibility → discovery/ranking → match/offer → agreement/reservation/assignment → fulfillment → evidence → conditional obligation/payment/settlement → outcome/reputation/repeat.
- **Clinic / Med Spa:** discovery/free value → organization/location/team setup → patient/workforce/capacity operations → governed clinical/commerce/resource workflows → quality/financial evidence → measured outcome → recurring value/expansion; regulated inventory and professional authority remain separately governed.
- **Current Visit / orders / results:** scheduled → intake/staff handoff → initial/previous/today → assessment/plan → order/transmission/partner state → result version → provider review → patient communication → follow-up → documentation/coding/billing readiness → exception-resolved close.
- **No-Fault / Workers' Compensation / RCM:** FinancialCase → coverage/authorization → longitudinal care/evidence → charge/claim/transmission → acceptance/adjudication/denial/appeal/remittance → verified payment → reconciliation/outcome.
- **RFP / procurement/customer:** opportunity/free value → qualification/go-no-go → requirements/capability/gap/pricing → approval/submission → award/loss → contract/payment evidence → implementation → measured outcome/reporting → renewal/expansion → reusable evidence.

These are governed projections over canonical objects and state machines, not separate applications, databases, marketplaces, authority systems, or guaranteed outcomes.
...
At the 2026-08-30 #367 convergence checkpoint, repository search did not find concrete `FinancialCase`, `AppointmentSeries`, patient recognition-photo/government-ID separation, duplicate-patient reconciliation, `SpecialtyPack`, `ConfigurationRegistry`, or `TerminologyRelease` implementation models by those concepts. Those requirements remain `DESIGNED / NOT_BUILT` unless later exact code/schema/migration/test evidence proves an equivalent governed model. Existing staff intake, immutable BodyMap versions, clinical-change comparison, Close Visit resolution, result correction/re-review behavior, and integration retry/dead-letter foundations must be reused rather than rebuilt.

## 49.1 KLINIKOS REFERENCE ENVIRONMENT

The reference environment is a real configurable, role-aware, synthetic-data proving ground—not disconnected screenshots. It loads the same Klinikos core with specialty/configuration packages and demonstrates permitted and forbidden actions for patient, front desk, MA/LPN/RN, provider, biller, owner, Grid, EDU, Zumi, and external-rail boundaries.

The first required proof is a **SOURCE-LOCKED SYNTHETIC NO-FAULT GOLDEN CASE** covering a returning case-aware patient, recognition/identity distinction, staff intake, longitudinal `INITIAL → PREVIOUS → TODAY`, immutable BodyMap, multi-specialty reports, procedures, universal orders/results including correction and re-review, Current Visit, ambient draft with provenance and human signature, coding/provider-to-biller handoff, claim-readiness/FinancialCase linkage, communication/follow-up, and unresolved external-rail states. **NO LIVE PATIENT DATA** may be used for this proof.
...
The product test for any proposed capability is: which plane(s) and canonical object does it improve; which lifecycle changes; which existing primitive/engine/external authority can be reused or connected; whether Grid, EDU, Network, or Zumi already owns the relevant exchange/learning/relationship/assistance concern; whether deterministic authority and evidence remain intact; what legitimate user/economic value results; and what implementation, risk, test, owner, KPI, and commercial consequence follows. If those answers are absent, do not build by default.

The company proof remains `WORKING PRODUCT → PAYING CUSTOMER → MEASURED RESULT → REPEATABLE SALE → RECURRING REVENUE → RETENTION → NETWORK EFFECT → EXPANSION`. Architecture, proposals, pipeline, and scenarios are not substitutes for those states.
