...
const HIGH_STAKES = /\b(medical|clinical|diagnosis|treatment|drug|medication|legal|lawsuit|contract|tax|financial advice|credential|license|compliance|HIPAA|insurance coverage)\b/i;
const MULTI_SOURCE = /\b(compare|versus|vs\.?|evidence|prove|verify|sources?|research|why|tradeoffs?|best option|recommend)\b/i;
const BUILD = /\b(code|debug|architecture|system design|API|database|security|implementation|build|integrate)\b/i;
...
export function estimateResearchComplexity(question: string): ResearchComplexity {
  ...
  return {
    score,
    depth: score >= 5 ? "deep" : score >= 2 ? "research" : "direct",
    reasons,
  };
}
...
export const zumiStrategyCapsuleSchema = z.object({
  version: z.literal(1),
  taskPattern: z.string().trim().min(3).max(500),
  successfulApproach: z.array(z.string().trim().min(3).max(800)).min(1).max(20),
  preferredSourceTypes: z.array(z.string().trim().min(2).max(200)).max(20).default([]),
  usefulTools: z.array(z.string().trim().min(2).max(100)).max(20).default([]),
  failureModes: z.array(z.string().trim().min(3).max(500)).max(20).default([]),
  verificationPattern: z.array(z.string().trim().min(3).max(500)).max(20).default([]),
  observedQuality: z.enum(["weak", "acceptable", "strong"]),
  capturedAt: z.string().datetime(),
  reuseUntil: z.string().datetime().nullable().default(null),
});
export type ZumiStrategyCapsule = z.infer<typeof zumiStrategyCapsuleSchema>;
...
export function buildResearchPlannerInstruction(question: string) {
  return [
    ...
    "Your job is to identify what must be known, which evidence would resolve it, which calculations may be useful, and how the final answer should be verified.",
    "Use broad search only when needed. Prefer authoritative/primary sources for consequential claims and current facts.",
...
export function buildResearchCriticInstruction(input: { question: string; draft: string }) {
  return [
    ...
    "Do not reveal hidden chain-of-thought. Return a concise verification report with: VERIFIED, NEEDS_MORE_RESEARCH, or FAIL; then list only actionable gaps.",
    `Original question: ${input.question}`,
    `Draft answer:\n${input.draft}`,
  ].join("\n\n");
}

export function buildStrategyReflectionInstruction(input: {
  question: string;
  ...
  return [
    "You are Zumi's skill-learning process.",
    "Do not retain the subject matter as a textbook. Extract only the reusable problem-solving strategy that would help solve similar future questions faster and more accurately.",
    "Return JSON only using the strategy capsule contract.",
    `Question pattern example: ${input.question}`,
    `Tools used: ${input.toolsUsed.join(", ") || "none"}`,
    `Source domains: ${input.sourceDomains.join(", ") || "none"}`,
    `Verification outcome: ${input.verificationOutcome}`,
    JSON.stringify({
      version: 1,
      taskPattern: "generalized task pattern",
      successfulApproach: ["step"],
      preferredSourceTypes: ["primary regulator documentation"],
      usefulTools: ["web"],
      failureModes: ["failure mode to avoid"],
      verificationPattern: ["check"],
      observedQuality: "strong",
