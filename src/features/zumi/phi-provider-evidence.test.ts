...
describe("PHI provider evidence gate", () => {
  it("fails closed when persisted evidence is absent even if runtime flags claim approval", () => {
  ...
  it("requires the persisted proof to match the exact provider deployment identity", () => {
  ...
  it("keeps runtime BAA/config flags as independent gates instead of allowing persisted evidence to override them", () => {
    expect(evaluatePhiProviderEligibility({
      runtime,
      evidence: evidence(),
  ...
  it("permits only when runtime configuration and current persisted evidence all agree", () => {
    expect(evaluatePhiProviderEligibility({
      runtime,
      evidence: evidence(),
      adapterBaaOnFile: true,
