...
/**
 * Evaluate whether one exact model-provider deployment is eligible for PHI egress.
 *
 * This function deliberately does not inspect prompts, patient data, entitlements, or
 * clinical authority. It answers only one narrow question: does the exact runtime
 * provider configuration have current, independently persisted evidence authorizing
 * the `phi_inference` vendor capability?
 *
 * An adapter/env flag is corroborating runtime configuration, never sufficient proof.
 * Conversely, persisted evidence cannot override a runtime that no longer declares the
 * expected BAA/deployment approval. Both sides must agree and every failure is closed.
 */
export function evaluatePhiProviderEligibility(input: {
  ...
  if (!evidence.evidenceReference.trim() || !Number.isInteger(evidence.authorityVersion) || evidence.authorityVersion < 1) {
