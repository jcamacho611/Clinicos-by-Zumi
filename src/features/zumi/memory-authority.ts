...
export function resolveZumiGovernedContextConflicts(items: readonly ZumiGovernedContextItem[]) {
  ...
  for (const candidates of grouped.values()) {
    ...
    const preferred = organizationItems.length ? organizationItems : globalItems.length ? globalItems : personalItems;
...
export function rankZumiGovernedContext(items: readonly ZumiGovernedContextItem[], question: string, take = 8) {
  const queryTerms = new Set(terms(question));
  return [...items]
    .map((item) => {
