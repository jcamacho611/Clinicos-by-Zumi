...
export function PaymentConsole({ workspace }: { workspace: PaymentWorkspace }) {
  ...
  return <div className="space-y-6">
    <div className="grid gap-4 sm:grid-cols-3">
      <div className="rounded-2xl border border-slate-200 bg-white p-4"><p className="text-[9px] font-extrabold uppercase tracking-[.16em] text-slate-400">Open patient balances</p><p className="mt-2 text-2xl font-black text-slate-950">{formatCurrency(outstanding / 100)}</p><p className="mt-1 text-[10px] text-slate-500">Latest tenant-scoped balance snapshots</p></div>
      <div className="rounded-2xl border border-slate-200 bg-white p-4"><p className="text-[9px] font-extrabold uppercase tracking-[.16em] text-slate-400">Payment links</p><p className="mt-2 text-2xl font-black text-slate-950">{workspace.paymentLinks.filter((link) => link.status === "active").length}</p><p className="mt-1 text-[10px] text-slate-500">Manual fallback ready for checkout adapters</p></div>
      <div className="rounded-2xl border border-slate-200 bg-white p-4"><p className="text-[9px] font-extrabold uppercase tracking-[.16em] text-slate-400">Payment events</p><p className="mt-2 text-2xl font-black text-slate-950">{workspace.events.length}</p><p className="mt-1 text-[10px] text-slate-500">Every post, link, refund, and package action is logged</p></div>
    </div>
    <div className="grid gap-6 xl:grid-cols-[.85fr_1.15fr]">
      <div className="space-y-4">
        <section className="rounded-2xl border border-slate-200 bg-white p-5">
          <div className="flex items-center gap-3"><span className="grid size-9 place-items-center rounded-xl bg-teal-50 text-teal-700"><CreditCard className="size-4" /></span><div><h3 className="text-sm font-extrabold text-slate-950">Record a payment</h3><p className="mt-1 text-[10px] text-slate-500">Manual posting is available before Stripe or Square is connected.</p></div></div>
