...
import type { LivingHomeCommandView, LivingHomePathView } from "@/lib/home/living-home-view-model";
 ...
 * server-projected Path produced. Derived from where the work actually points — never
 * guessed from the words the person typed.
 */
...
export function LivingHome({
  ...
  initialPaths: LivingHomePathView[];
  ...
  const activeDestination = destinationForHref(activePath?.nextActionHref ?? null, rail);
  const phaseIndex = PHASES.indexOf(phase);
  const orbState = orbStateFor(phase, failed, intelligence.available);

  // Every row is read from the minimum-necessary Path projection the authenticated
  // server produced. The browser presents truth; it no longer re-runs Path logic.
  const workspaceRows = useMemo<WorkspaceRow[]>(() => {
    if (!activePath) return [];
    const rows: WorkspaceRow[] = [
      { key: "outcome", label: "Outcome", value: activePath.title, state: "Organized", tone: "neutral" },
      { key: "goal", label: "What you asked", value: activePath.goal, state: "Recorded", tone: "neutral" },
      {
        key: "progress",
        label: "Progress",
        value: `${activePath.progressPercent}% complete`,
        state: activePath.stateLabel,
        tone: activePath.state === "blocked"
          ? "blocked"
          : activePath.state === "done"
            ? "settled"
            : "open",
      },
    ];
    for (const blocker of activePath.blockers) {
      rows.push({
  ...
  async function submitIntent(event?: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("error" in payload && payload.error ? payload.error : "Klinikos could not organize that yet.");
      }

      setPhase("preparing");
      if (!("kind" in payload)) throw new Error("Klinikos returned an incomplete response.");

      if (payload.kind === "path") {
        setPaths((current) => [payload.path, ...current.filter((path) => path.instanceId !== payload.path.instanceId)]);
        setActiveInstanceId(payload.path.instanceId);
        say("Klinikos", payload.message);
      } else if (payload.kind === "surface") {
        setSurfaceAnswer({ answer: payload.message, label: payload.surface.label, href: payload.surface.href });
        say("Klinikos", payload.message);
      } else if (payload.kind === "clarification") {
        setClarification(payload.clarification);
        say("Klinikos", payload.message);
      } else {
        setFailed(payload.kind === "unavailable");
        say("Klinikos", payload.message);
      }
      setPhase("ready");
    } catch (error) {
      setFailed(true);
      setPhase("ready");
      say("Klinikos", error instanceof Error ? error.message : "Klinikos could not organize that yet.");
    }
  }
          ...
          {working ? (
            <section aria-labelledby="workspace-title" className="mt-14 overflow-hidden rounded-[20px] border border-[var(--line-dark)]">
              <div className="flex flex-wrap items-center justify-between gap-4 border-b border-[var(--line-dark)] px-6 py-4">
                <div className="flex flex-wrap items-baseline gap-4">
                  ...
                  <h2 className="text-lg font-semibold tracking-[var(--tracking-tight)]" id="workspace-title">
                    {phase === "ready" ? activePath?.title ?? "One more detail" : "Working…"}
                  </h2>
                </div>
                <button
                ...
                <div className="px-6 py-6">
                  ...
                  {activePath?.nextActionHref ? (
                    <Link
                      ...
                      href={activePath.nextActionHref}
                    >
                      Open the next step
                    </Link>
                  ) : null}
              ...
              <div className="flex flex-wrap items-center justify-between gap-4 border-t border-[var(--line-dark)] px-6 py-4">
                ...
                <span className="text-[var(--text-micro)] font-extrabold uppercase tracking-[var(--tracking-wide)] text-[var(--text-secondary)]">
                  {activePath?.state === "needs_review" ? "Requires human review" : "Governed by your role"}
                </span>
              </div>
            </section>
          ) : (
