  ...
  return <div className="space-y-6">
    <div className="grid gap-6 xl:grid-cols-[1.1fr_.9fr]">
      <SectionCard title="Care Team Room" description="A minimum-necessary collaboration room. Clinical decisions remain with licensed humans; every message and membership change is audited.">
        {room ? <div className="space-y-4 p-5"><div className="flex flex-wrap items-start gap-3"><span className="grid size-11 place-items-center rounded-2xl bg-teal-50 text-teal-700"><Users className="size-5" /></span><div className="min-w-0 flex-1"><p className="text-sm font-black text-slate-950">{room.name}</p><p className="mt-1 text-[10px] text-slate-500">{room.patient.firstName} {room.patient.lastName} · {room.patient.mrn} · {room.sourceType.replaceAll("_", " ")}</p></div><StatusBadge status={room.status} /><Badge tone="teal"><LockKeyhole className="mr-1 size-3" /> Minimum necessary</Badge></div><div className="grid gap-3 sm:grid-cols-3"><div className="rounded-xl bg-slate-50 p-3"><p className="text-[9px] font-bold text-slate-400">MEMBERS</p><p className="mt-1 text-xl font-black text-slate-950">{room.members.length}</p></div><div className="rounded-xl bg-slate-50 p-3"><p className="text-[9px] font-bold text-slate-400">MESSAGES</p><p className="mt-1 text-xl font-black text-slate-950">{room.messages.length}</p></div><div className="rounded-xl bg-slate-50 p-3"><p className="text-[9px] font-bold text-slate-400">TASKS</p><p className="mt-1 text-xl font-black text-slate-950">{room.tasks.length}</p></div></div><div className="rounded-2xl border border-slate-200 p-4"><p className="text-[9px] font-extrabold uppercase tracking-[.16em] text-slate-400">Shared plan</p><p className="mt-2 text-xs leading-6 text-slate-700">{JSON.stringify(room.sharedPlan ?? { goals: [], nextActions: [] })}</p></div><div className="space-y-2">{room.members.map((member) => <div className="flex items-center gap-3 rounded-xl border border-slate-200 p-3" key={`${member.id}-${member.role}`}><span className="grid size-8 place-items-center rounded-lg bg-sky-50 text-sky-700"><Users className="size-4" /></span><div className="min-w-0 flex-1"><p className="text-xs font-extrabold text-slate-900">{member.provider}</p><p className="text-[10px] text-slate-500">{member.organization} · {member.role}</p></div><StatusBadge status={member.status} /></div>)}</div></div> : <form className="space-y-3 p-5" onSubmit={createRoom}><select aria-label="Patient" className="h-10 w-full rounded-xl border border-slate-200 bg-white px-3 text-xs font-bold" onChange={(event) => setPatientId(event.target.value)} value={patientId}>{data.patients.map((patient) => <option key={patient.id} value={patient.id}>{patient.firstName} {patient.lastName} · {patient.mrn}</option>)}</select><input aria-label="Care Team Room name" className="h-10 w-full rounded-xl border border-slate-200 px-3 text-xs" onChange={(event) => setRoomName(event.target.value)} placeholder="Room name" value={roomName} /><Button disabled={busy || !canCreate || !patientId || roomName.trim().length < 3} type="submit" variant="primary">Create Care Team Room <Plus className="size-4" /></Button>{message && <p className="text-[10px] text-slate-500">{message}</p>}</form>}
      </SectionCard>
