...
export function RemoteObservationCreateAction({ patients, enabled }: { patients: RemoteMonitoringWorkspace["patients"]; enabled: boolean }) {
  ...
  async function submit() {
    ...
    try { await post("/api/remote-monitoring", { patientId, type, value, unit, sourceType, deviceId: deviceId || null, observedAt: new Date().toISOString(), thresholdConfig: { label: thresholdLabel, instruction: thresholdInstruction }, consentConfirmed }); window.location.reload(); }
  ...
  return <div className="space-y-3 p-5"><select aria-label="Remote monitoring patient" className={controlClass} onChange={(event) => setPatientId(event.target.value)} value={patientId}>{patients.map((patient) => <option key={patient.id} value={patient.id}>{patient.firstName} {patient.lastName} · {patient.mrn}</option>)}</select><div className="grid gap-3 sm:grid-cols-2"><select aria-label="Observation type" className={controlClass} onChange={(event) => setType(event.target.value)} value={type}><option value="blood_pressure">Blood pressure</option><option value="glucose">Glucose</option><option value="weight">Weight</option><option value="pulse_oximetry">Pulse oximetry</option><option value="symptom_check_in">Symptom check-in</option></select><Input aria-label="Observation value" onChange={(event) => setValue(event.target.value)} placeholder="Enter measured value" value={value} /></div><div className="grid gap-3 sm:grid-cols-2"><Input aria-label="Observation unit" onChange={(event) => setUnit(event.target.value)} placeholder="Unit" value={unit} /><select aria-label="Observation source" className={controlClass} onChange={(event) => setSourceType(event.target.value)} value={sourceType}><option value="patient_entered">Manual patient entry</option><option value="device_adapter">Device adapter queue</option></select></div>{sourceType === "device_adapter" && <Input aria-label="Device identifier" onChange={(event) => setDeviceId(event.target.value)} placeholder="Device identifier" value={deviceId} />}<Input aria-label="Provider threshold label" onChange={(event) => setThresholdLabel(event.target.value)} placeholder="Provider threshold label" value={thresholdLabel} /><textarea aria-label="Provider threshold instruction" className={areaClass} onChange={(event) => setThresholdInstruction(event.target.value)} value={thresholdInstruction} /><label className="flex items-start gap-2 rounded-xl border border-slate-200 bg-slate-50 p-3 text-[12px] leading-5 text-slate-600"><input aria-label="Confirm patient consent for remote observation" checked={consentConfirmed} className="mt-1" onChange={(event) => setConsentConfirmed(event.target.checked)} type="checkbox" /><span>I confirm applicable patient consent or authorization has been obtained for this observation. This attestation does not create consent by itself.</span></label><div className="flex items-center justify-between gap-3"><p className="text-[12px] leading-5 text-slate-400">Provider review remains mandatory. Klinikos does not clinically interpret the submitted value.</p><Button disabled={busy || !patientId || !value.trim() || !consentConfirmed} onClick={submit} size="sm" variant="primary">{busy ? <LoaderCircle className="size-3.5 animate-spin" /> : <Plus className="size-3.5" />} Add observation</Button></div>{error && <p className="text-[12px] font-bold text-rose-600" role="alert">{error}</p>}</div>;
}
