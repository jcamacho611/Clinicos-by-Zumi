...
export function CaseCreateAction({ patients, enabled }: { patients: CaseWorkspace["patients"]; enabled: boolean }) {
  ...
  return <div className="w-full rounded-3xl border border-teal-200 bg-teal-50/70 p-5 xl:w-[720px]">
    <div className="flex items-center justify-between"><div><p className="text-xs font-black text-slate-950">Open governed case room</p><p className="mt-1 text-[10px] text-slate-500">Creates a persistent packet checklist, owner task, and audit event.</p></div><Button aria-label="Close case form" onClick={() => setOpen(false)} size="sm" variant="ghost"><X className="size-4" /></Button></div>
...
export function CaseUpdateAction({ room, enabled }: { room: CaseRoom; enabled: boolean }) {
  ...
  if (!enabled) return <p className="text-[10px] text-slate-500">Read-only case access.</p>;
...
export function CaseTaskTransitionAction({ room, task, enabled }: { room: CaseRoom; task: CaseRoom["tasks"][number]; enabled: boolean }) {
  ...
  if (!enabled) return null;
  return <div className="mt-3 flex flex-wrap gap-2">{task.status === "open" ? <><Button disabled={busy} onClick={() => transition("complete")} size="sm" variant="primary"><CheckCircle2 className="size-3.5" /> Complete</Button><Button disabled={busy} onClick={() => transition("cancel")} size="sm" variant="ghost"><X className="size-3.5" /> Cancel</Button></> : <Button disabled={busy} onClick={() => transition("reopen")} size="sm" variant="secondary"><RotateCcw className="size-3.5" /> Reopen</Button>}{error && <p className="basis-full text-[10px] font-bold text-rose-600" role="alert">{error}</p>}</div>;
}
...
export function CasePacketActions({ room, packet, enabled }: { room: CaseRoom; packet: CaseRoom["packets"][number]; enabled: boolean }) {
  ...
  return <div className="mt-4 border-t border-slate-100 pt-4">
    {enabled && ["draft", "needs_attention"].includes(packet.status) && <><Input aria-label="Packet review note" onChange={(event) => setNote(event.target.value)} placeholder="Checklist review evidence" value={note} /><div className="mt-3 space-y-2">{packet.checklist.map((item) => <div className="flex flex-wrap items-center gap-2 rounded-xl bg-slate-50 p-3" key={item.key}><span className={`grid size-6 place-items-center rounded-lg ${item.complete ? "bg-teal-100 text-teal-700" : "bg-white text-slate-400"}`}>{item.complete ? <CheckCircle2 className="size-3.5" /> : <span className="size-2 rounded-full bg-current" />}</span><p className="min-w-0 flex-1 text-[10px] font-bold text-slate-700">{item.label}{item.required ? " *" : ""}</p><Button disabled={busy || note.trim().length < 8} onClick={() => transition("mark_item", item.key, !item.complete)} size="sm" variant="ghost">{item.complete ? "Mark missing" : "Mark reviewed"}</Button></div>)}</div></>}
