...
function Feedback({ error, message }: { error: string; message: string }) {
  return <>{message && <p className="text-[10px] font-bold text-emerald-700" role="status">{message}</p>}{error && <p className="text-[10px] font-bold text-rose-600" role="alert">{error}</p>}</>;
}
...
export function CreateImagingOrderControl({ patients, providers, integrations, priorAuthorizations, disabled = false }: {
  ...
  return <div className="space-y-4 p-5">
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
      <label className="text-[9px] font-black uppercase tracking-[.12em] text-slate-500">Patient<select className={fieldClass} onChange={(event) => { setPatientId(event.target.value); setPriorAuthorizationId(""); setAuthorizationStatus("not_required"); }} value={patientId}>{patients.map((patient) => <option key={patient.id} value={patient.id}>{patient.name} · {patient.mrn}</option>)}</select></label>
      <label className="text-[9px] font-black uppercase tracking-[.12em] text-slate-500">Ordering provider<select className={fieldClass} onChange={(event) => setProviderId(event.target.value)} value={providerId}>{providers.map((provider) => <option key={provider.id} value={provider.id}>{provider.label}</option>)}</select></label>
      <label className="text-[9px] font-black uppercase tracking-[.12em] text-slate-500">Study<Input className="mt-1" onChange={(event) => setStudy(event.target.value)} value={study} /></label>
      <label className="text-[9px] font-black uppercase tracking-[.12em] text-slate-500">Modality<select className={fieldClass} onChange={(event) => setModality(event.target.value)} value={modality}>{["xray", "ct", "mri", "ultrasound", "mammography", "pet", "dexa", "fluoroscopy", "nuclear_medicine", "other"].map((item) => <option key={item} value={item}>{item.replaceAll("_", " ").toUpperCase()}</option>)}</select></label>
      <label className="text-[9px] font-black uppercase tracking-[.12em] text-slate-500">Body part<Input className="mt-1" onChange={(event) => setBodyPart(event.target.value)} value={bodyPart} /></label>
      <label className="text-[9px] font-black uppercase tracking-[.12em] text-slate-500">Laterality<select className={fieldClass} onChange={(event) => setLaterality(event.target.value)} value={laterality}>{["none", "left", "right", "bilateral"].map((item) => <option key={item} value={item}>{item}</option>)}</select></label>
      <label className="text-[9px] font-black uppercase tracking-[.12em] text-slate-500">Chart diagnosis codes<Input className="mt-1" onChange={(event) => setDiagnosisCodes(event.target.value)} placeholder="R05.9, R06.02" value={diagnosisCodes} /></label>
      <label className="text-[9px] font-black uppercase tracking-[.12em] text-slate-500">Priority<select className={fieldClass} onChange={(event) => setPriority(event.target.value)} value={priority}><option value="routine">Routine</option><option value="urgent">Urgent</option><option value="stat">STAT</option></select></label>
      <label className="text-[9px] font-black uppercase tracking-[.12em] text-slate-500">Imaging facility<Input className="mt-1" onChange={(event) => setFacility(event.target.value)} value={facility} /></label>
    </div>
    <label className="block text-[9px] font-black uppercase tracking-[.12em] text-slate-500">Clinical indication<textarea className={`${fieldClass} min-h-20 py-3`} onChange={(event) => setClinicalIndication(event.target.value)} value={clinicalIndication} /></label>
    <div className="grid gap-3 rounded-2xl bg-indigo-50/70 p-4 sm:grid-cols-3">
      <label className="text-[9px] font-black uppercase tracking-[.12em] text-indigo-800">Authorization<select className={fieldClass} onChange={(event) => setAuthorizationStatus(event.target.value)} value={authorizationStatus}><option value="not_required">Not required</option><option value="pending">Pending</option><option value="approved">Approved</option><option value="denied">Denied</option><option value="expired">Expired</option></select></label>
      <label className="text-[9px] font-black uppercase tracking-[.12em] text-indigo-800">Authorization record<select className={fieldClass} disabled={authorizationStatus === "not_required"} onChange={(event) => setPriorAuthorizationId(event.target.value)} value={priorAuthorizationId}><option value="">Select patient record</option>{patientAuthorizations.map((item) => <option key={item.id} value={item.id}>{item.payer} · {item.service} · {item.status}</option>)}</select></label>
      <label className="text-[9px] font-black uppercase tracking-[.12em] text-indigo-800">Delivery pathway<select className={fieldClass} onChange={(event) => setDeliveryMethod(event.target.value)} value={deliveryMethod}>{["manual", "print", "fax", "direct", "electronic_adapter"].map((item) => <option key={item} value={item}>{item.replaceAll("_", " ")}</option>)}</select></label>
      {deliveryMethod === "electronic_adapter" && <label className="text-[9px] font-black uppercase tracking-[.12em] text-indigo-800 sm:col-span-3">Active adapter<select className={fieldClass} onChange={(event) => setIntegrationId(event.target.value)} value={integrationId}><option value="">Select tested adapter</option>{activeIntegrations.map((item) => <option key={item.id} value={item.id}>{item.vendor}</option>)}</select></label>}
    </div>
    <div className="flex flex-wrap items-center gap-3"><Button disabled={unavailable} onClick={createOrder} variant="primary">{pending ? <LoaderCircle className="size-4 animate-spin" /> : <Send className="size-4" />} Create draft order</Button><p className="text-[9px] text-slate-400">Diagnosis codes must already exist on this patient&apos;s chart.</p></div>
...
export function ReceiveImagingResultControl({ patients, orders, documents, integrations, disabled = false }: {
  ...
  return <div className="space-y-4 p-5">
    <div className="grid gap-3 sm:grid-cols-2">
      <label className="text-[9px] font-black uppercase tracking-[.12em] text-slate-500">Completed order<select className={fieldClass} onChange={(event) => selectOrder(event.target.value)} value={orderId}><option value="">Unlinked report</option>{eligibleOrders.map((order) => <option key={order.id} value={order.id}>{order.patientName} · {order.study}</option>)}</select></label>
      <label className="text-[9px] font-black uppercase tracking-[.12em] text-slate-500">Patient<select className={fieldClass} disabled={Boolean(orderId)} onChange={(event) => { setPatientId(event.target.value); setReportDocumentId(""); }} value={patientId}>{patients.map((patient) => <option key={patient.id} value={patient.id}>{patient.name} · {patient.mrn}</option>)}</select></label>
      <label className="text-[9px] font-black uppercase tracking-[.12em] text-slate-500">Report title<Input className="mt-1" onChange={(event) => setReportTitle(event.target.value)} value={reportTitle} /></label>
      <label className="text-[9px] font-black uppercase tracking-[.12em] text-slate-500">Facility<Input className="mt-1" onChange={(event) => setFacility(event.target.value)} value={facility} /></label>
      <label className="text-[9px] font-black uppercase tracking-[.12em] text-slate-500">Study performed<Input className="mt-1" onChange={(event) => setStudyPerformedAt(event.target.value)} type="datetime-local" value={studyPerformedAt} /></label>
      <label className="text-[9px] font-black uppercase tracking-[.12em] text-slate-500">Source pathway<select className={fieldClass} onChange={(event) => setSourceType(event.target.value)} value={sourceType}><option value="structured_manual">Structured manual entry</option><option value="manual_upload">Patient-bound PDF</option><option value="electronic_interface">Electronic interface</option></select></label>
      {sourceType === "manual_upload" && <label className="text-[9px] font-black uppercase tracking-[.12em] text-slate-500 sm:col-span-2">Patient report PDF<select className={fieldClass} onChange={(event) => setReportDocumentId(event.target.value)} value={reportDocumentId}><option value="">Select a patient-bound PDF</option>{patientDocuments.map((document) => <option key={document.id} value={document.id}>{document.name}</option>)}</select></label>}
      {sourceType === "electronic_interface" && <label className="text-[9px] font-black uppercase tracking-[.12em] text-slate-500 sm:col-span-2">Active adapter<select className={fieldClass} onChange={(event) => setIntegrationId(event.target.value)} value={integrationId}><option value="">Select tested adapter</option>{activeIntegrations.map((item) => <option key={item.id} value={item.id}>{item.vendor}</option>)}</select></label>}
      <label className="text-[9px] font-black uppercase tracking-[.12em] text-slate-500 sm:col-span-2">Source reference<Input className="mt-1" onChange={(event) => setSourceReference(event.target.value)} placeholder="Accession or report reference" value={sourceReference} /></label>
    </div>
    <div className="grid gap-3 sm:grid-cols-2"><label className="text-[9px] font-black uppercase tracking-[.12em] text-slate-500">Source findings<textarea className={`${fieldClass} min-h-28 py-3`} onChange={(event) => setFindings(event.target.value)} value={findings} /></label><label className="text-[9px] font-black uppercase tracking-[.12em] text-slate-500">Source impression<textarea className={`${fieldClass} min-h-28 py-3`} onChange={(event) => setImpression(event.target.value)} value={impression} /></label></div>
    <label className="block text-[9px] font-black uppercase tracking-[.12em] text-slate-500">Image or PACS reference<Input className="mt-1" onChange={(event) => setImageReference(event.target.value)} placeholder="Reference only; no unverified PACS claim" value={imageReference} /></label>
    <label className="flex items-center gap-3 rounded-2xl border border-rose-200 bg-rose-50 p-3 text-[10px] font-bold text-rose-900"><input checked={urgentSourceFlag} onChange={(event) => setUrgentSourceFlag(event.target.checked)} type="checkbox" /> Source explicitly marks this report urgent; create immediate human escalation.</label>
...
export function ImagingResultTransitionControl({ result, canSign, disabled = false }: { result: { id: string; status: string; patientNotificationStatus: string }; canSign: boolean; disabled?: boolean }) {
  ...
  const [pending, setPending] = useState(false);
  const [error, setError] = useState("");
  if (!actions.length) return !canSign && result.status === "needs_review" ? <p className="mt-4 rounded-xl bg-amber-50 px-3 py-2 text-[10px] font-bold text-amber-800">Awaiting an active provider identity with imaging sign-off permission.</p> : null;
