...
export function IdentityScanControl({ disabled = false }: { disabled?: boolean }) {
  ...
  return <div className="flex flex-wrap items-center gap-2">
    ...
    {message && <p className="text-[10px] font-bold text-teal-700" role="status">{message}</p>}
...
export function MatchReviewControl({ matchId, disabled = false }: { matchId: string; disabled?: boolean }) {
  ...
  return <div className="mt-4 space-y-3 border-t border-slate-100 pt-4">
    <div className="grid gap-2 sm:grid-cols-3">
      {resolutionFields.map((field) => <label className="text-[9px] font-extrabold uppercase tracking-[.12em] text-slate-400" key={field}>
        {field.replace(/([A-Z])/g, " $1")}
        <select
          className="mt-1 h-8 w-full rounded-lg border border-slate-200 bg-white px-2 text-[10px] font-semibold normal-case tracking-normal text-slate-700 outline-none focus:border-sky-400"
...
export function ConsentCaptureControl({
  ...
  return <div className="space-y-4 p-5">
    <div className="grid gap-3 md:grid-cols-2">
      <label className="text-[10px] font-bold text-slate-600">Patient
      ...
      <label className="text-[10px] font-bold text-slate-600">Recipient clinic
      ...
      <label className="text-[10px] font-bold text-slate-600">Purpose of use
      ...
      <label className="text-[10px] font-bold text-slate-600">Expires in days
        <Input max={365} min={1} onChange={(event) => setExpiresInDays(Number(event.target.value))} type="number" value={expiresInDays} />
      </label>
      <label className="text-[10px] font-bold text-slate-600">Signer name
        <Input onChange={(event) => setSignerName(event.target.value)} placeholder="Patient or authorized representative" value={signerName} />
      </label>
      <label className="text-[10px] font-bold text-slate-600">Relationship
        <Input onChange={(event) => setRelationship(event.target.value)} value={relationship} />
      </label>
    </div>
    <fieldset><legend className="text-[10px] font-bold text-slate-600">Authorized record categories</legend><div className="mt-2 flex flex-wrap gap-2">
      {categories.map((category) => <label className={`cursor-pointer rounded-full px-3 py-1.5 text-[10px] font-bold ring-1 ring-inset ${selected.includes(category) ? "bg-teal-600 text-white ring-teal-600" : "bg-white text-slate-600 ring-slate-200"}`} key={category}>
    ...
    <p className="text-[9px] leading-5 text-slate-400">This records a signer acknowledgment receipt. It does not open any chart until a separate record request is approved.</p>
    {error && <p className="text-[10px] font-bold text-rose-600" role="alert">{error}</p>}
  </div>;
}
...
export function RecordLocatorControl({ masterPatientId }: { masterPatientId: string }) {
  ...
  return <div className="mt-3">
    ...
    {records.length > 0 && <div className="mt-2 space-y-1">{records.map((record) => <div className="flex items-center gap-2 rounded-lg bg-slate-50 px-3 py-2 text-[10px]" key={`${record.organization}-${record.patientId}`}><Check className="size-3 text-teal-600" /><span className="font-bold text-slate-700">{record.organization}</span><span className="text-slate-400">{record.mrn} · {record.identityStatus.replaceAll("_", " ")}</span></div>)}</div>}
    {error && <p className="mt-2 text-[10px] font-bold text-rose-600" role="alert">{error}</p>}
  </div>;
}
