...
export function FeatureRegistryWorkspace({ sections }: { sections: RegistrySectionView[] }) {
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      ...
      ].map(([value, label, detail]) => <Card className="p-5" key={label}><p className="text-3xl font-black tracking-[-.05em] text-slate-950">{value}</p><p className="mt-2 text-xs font-extrabold text-slate-800">{label}</p><p className="mt-1 text-[10px] text-slate-400">{detail}</p></Card>)}
    </div>
    ...
    <Card className="overflow-hidden">
      <div className="grid gap-4 border-b border-slate-100 p-4 lg:grid-cols-[1fr_auto]">
        ...
        <div className="flex flex-wrap gap-1.5">{["ALL", "CONNECTED", "DEMO", "FOUNDATION", "PLANNED", "VENDOR_DEPENDENT", "GOVERNED"].map((value) => <button className={`rounded-lg px-2.5 py-2 text-[9px] font-extrabold transition ${status === value ? "bg-slate-950 text-white" : "bg-slate-100 text-slate-500 hover:bg-slate-200"}`} key={value} onClick={() => setStatus(value)}>{value.replace("_", " ")}</button>)}</div>
      </div>
      <div className="divide-y divide-slate-100">
        {filtered.map((section) => <details className="group" key={section.id}>
          <summary className="flex cursor-pointer list-none items-start gap-4 p-5 transition hover:bg-slate-50/70">
            ...
            <span className="min-w-0 flex-1"><span className="flex flex-wrap items-center gap-2"><span className="text-sm font-extrabold text-slate-950">{section.title}</span><Badge tone={statusTone(section.deliveryStatus)}>{section.deliveryStatus.replace("_", " ")}</Badge><Badge tone="rose">P0</Badge></span><span className="mt-1 block text-[11px] leading-5 text-slate-500">{section.mandate}</span><span className="mt-2 block text-[9px] font-bold uppercase tracking-[.12em] text-slate-400">{section.capabilities.length} visible capabilities · {section.interfaceRoute} · {section.deliveryMode}</span></span>
          ...
          <div className="border-t border-slate-100 bg-[#f8faf9] p-5">
            <div className="mb-4 grid gap-3 md:grid-cols-3"><div className="rounded-xl bg-white p-3 ring-1 ring-slate-200"><ShieldCheck className="size-4 text-teal-600" /><p className="mt-2 text-[9px] font-extrabold text-slate-900">OWNERS & ACCESS</p><p className="mt-1 text-[10px] leading-5 text-slate-500">{section.ownerRoles.join(", ")}</p></div><div className="rounded-xl bg-white p-3 ring-1 ring-slate-200"><Database className="size-4 text-sky-600" /><p className="mt-2 text-[9px] font-extrabold text-slate-900">DATA ARCHITECTURE</p><p className="mt-1 text-[10px] leading-5 text-slate-500">{section.databaseObjects.join(", ")}</p></div><div className="rounded-xl bg-white p-3 ring-1 ring-slate-200"><LockKeyhole className="size-4 text-amber-600" /><p className="mt-2 text-[9px] font-extrabold text-slate-900">DEFINITION OF COMPLETE</p><p className="mt-1 text-[10px] leading-5 text-slate-500">All {completionGateKeys.length} gates must be evidenced before status can become connected.</p></div></div>
            <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3">{section.capabilities.map((capability) => <div className="flex items-start gap-2 rounded-xl border border-slate-200 bg-white p-3" key={capability.id}><CheckCircle2 className="mt-0.5 size-3.5 shrink-0 text-teal-600" /><div><p className="text-[10px] font-bold leading-4 text-slate-700">{capability.name}</p><p className="mt-1 text-[8px] font-extrabold uppercase tracking-[.1em] text-slate-400">{capability.priority} · {capability.deliveryStatus.replace("_", " ")}</p></div></div>)}</div>
          </div>
        </details>)}
