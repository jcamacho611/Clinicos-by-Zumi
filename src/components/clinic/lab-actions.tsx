...
function Feedback({ error, message }: { error: string; message: string }) {
  return <>{message && <p className="text-[10px] font-bold text-teal-700" role="status">{message}</p>}{error && <p className="text-[10px] font-bold text-rose-600" role="alert">{error}</p>}</>;
}
...
export function CreateLabOrderControl({ patients, providers, integrations, disabled = false }: { patients: LabPatient[]; providers: LabProvider[]; integrations: LabIntegration[]; disabled?: boolean }) {
  ...
  return <div className="space-y-4 p-5">
    <div className="grid gap-3 md:grid-cols-2">
      <label className="text-[10px] font-bold text-slate-600">Patient<select className={fieldClass} onChange={(event) => setPatientId(event.target.value)} value={patientId}>{patients.map((patient) => <option key={patient.id} value={patient.id}>{patient.name} · {patient.mrn}</option>)}</select></label>
      <label className="text-[10px] font-bold text-slate-600">Ordering provider<select className={fieldClass} onChange={(event) => setProviderId(event.target.value)} value={providerId}>{providers.map((provider) => <option key={provider.id} value={provider.id}>{provider.label}</option>)}</select></label>
      <label className="text-[10px] font-bold text-slate-600">Laboratory<Input className="mt-1" onChange={(event) => setVendor(event.target.value)} value={vendor} /></label>
      <label className="text-[10px] font-bold text-slate-600">Priority<select className={fieldClass} onChange={(event) => setPriority(event.target.value)} value={priority}><option value="routine">Routine</option><option value="urgent">Urgent</option><option value="stat">STAT</option></select></label>
      <label className="text-[10px] font-bold text-slate-600 md:col-span-2">Tests, comma separated<Input className="mt-1" onChange={(event) => setTests(event.target.value)} placeholder="Hemoglobin A1C, CMP" value={tests} /></label>
      <label className="text-[10px] font-bold text-slate-600">Attached diagnosis codes<Input className="mt-1" onChange={(event) => setDiagnosisCodes(event.target.value)} placeholder="E11.9, I10" value={diagnosisCodes} /></label>
      <label className="text-[10px] font-bold text-slate-600">Delivery pathway<select className={fieldClass} onChange={(event) => setDeliveryMethod(event.target.value)} value={deliveryMethod}><option value="manual">Manual order</option><option value="print">Print fallback</option><option value="fax">Fax fallback</option>{activeIntegrations.length > 0 && <option value="electronic_adapter">Active electronic adapter</option>}</select></label>
      {deliveryMethod === "electronic_adapter" && <label className="text-[10px] font-bold text-slate-600 md:col-span-2">Verified interface<select className={fieldClass} onChange={(event) => setIntegrationId(event.target.value)} value={integrationId}><option value="">Select active adapter</option>{activeIntegrations.map((integration) => <option key={integration.id} value={integration.id}>{integration.vendor}</option>)}</select></label>}
      <label className="text-[10px] font-bold text-slate-600">Specimen<Input className="mt-1" onChange={(event) => setSpecimenType(event.target.value)} value={specimenType} /></label>
      <label className="text-[10px] font-bold text-slate-600">Collection site<Input className="mt-1" onChange={(event) => setCollectionSite(event.target.value)} value={collectionSite} /></label>
    </div>
    <div className="flex flex-wrap items-center gap-3"><Button disabled={unavailable} onClick={create} size="sm" variant="primary">{pending ? <LoaderCircle className="size-3.5 animate-spin" /> : <Plus className="size-3.5" />} Create order draft</Button><p className="text-[9px] leading-5 text-slate-400">Codes must already exist on the patient chart. Vendor names do not imply a live interface.</p></div>
...
export function LabOrderTransitionControl({ order, disabled = false }: { order: { id: string; status: string; deliveryStatus: string }; disabled?: boolean }) {
  ...
  return <div className="mt-4 space-y-2 border-t border-slate-100 pt-4"><div className="grid gap-2 md:grid-cols-[1fr_1.4fr_auto]"><select className="h-9 rounded-xl border border-slate-200 bg-white px-3 text-[10px] font-bold text-slate-700" onChange={(event) => setAction(event.target.value as LabOrderAction)} value={selectedAction}>{actions.map((item) => <option key={item} value={item}>{orderActionLabels[item]}</option>)}</select>{selectedAction === "record_collection" ? <Input aria-label="Specimen collection time" onChange={(event) => setCollectedAt(event.target.value)} type="datetime-local" value={collectedAt} /> : <Input aria-label="Lab order action note" onChange={(event) => setNote(event.target.value)} placeholder={needsNote ? "Required human confirmation or reason" : "Optional workflow note"} value={note} />}<Button disabled={unavailable} onClick={transition} size="sm" variant={["fail_delivery", "cancel"].includes(selectedAction) ? "danger" : "primary"}>{pending ? <LoaderCircle className="size-3.5 animate-spin" /> : <Workflow className="size-3.5" />}{orderActionLabels[selectedAction]}</Button></div>{error && <p className="text-[10px] font-bold text-rose-600" role="alert">{error}</p>}</div>;
}
...
export function ReceiveLabResultControl({ patients, orders, documents, integrations, disabled = false }: { patients: LabPatient[]; orders: LabOrderOption[]; documents: LabDocument[]; integrations: LabIntegration[]; disabled?: boolean }) {
  ...
  return <div className="space-y-4 p-5">
    <div className="grid gap-3 md:grid-cols-2">
      <label className="text-[10px] font-bold text-slate-600">Linked order<select className={fieldClass} onChange={(event) => chooseOrder(event.target.value)} value={orderId}><option value="">Unsolicited / external result</option>{eligibleOrders.map((order) => <option key={order.id} value={order.id}>{order.patientName} · {order.vendor} · {order.tests.map((test) => test.name).join(", ")}</option>)}</select></label>
      <label className="text-[10px] font-bold text-slate-600">Patient<select className={fieldClass} disabled={Boolean(orderId)} onChange={(event) => setPatientId(event.target.value)} value={patientId}>{patients.map((patient) => <option key={patient.id} value={patient.id}>{patient.name} · {patient.mrn}</option>)}</select></label>
      <label className="text-[10px] font-bold text-slate-600">Panel<Input className="mt-1" onChange={(event) => setPanelName(event.target.value)} value={panelName} /></label>
      <label className="text-[10px] font-bold text-slate-600">Resulted at<Input className="mt-1" onChange={(event) => setResultedAt(event.target.value)} type="datetime-local" value={resultedAt} /></label>
      <label className="text-[10px] font-bold text-slate-600">Source pathway<select className={fieldClass} onChange={(event) => setSourceType(event.target.value)} value={sourceType}><option value="manual_entry">Structured manual entry</option><option value="manual_upload">Verified chart document upload</option>{activeIntegrations.length > 0 && <option value="electronic_interface">Active electronic interface</option>}</select></label>
      <label className="text-[10px] font-bold text-slate-600">Source laboratory<Input className="mt-1" onChange={(event) => setVendor(event.target.value)} value={vendor} /></label>
      {sourceType === "manual_upload" && <label className="text-[10px] font-bold text-slate-600 md:col-span-2">Patient source document<select className={fieldClass} onChange={(event) => setSourceDocumentId(event.target.value)} value={sourceDocumentId}><option value="">Select an uploaded document</option>{patientDocuments.map((document) => <option key={document.id} value={document.id}>{document.name}</option>)}</select></label>}
      {sourceType === "electronic_interface" && <label className="text-[10px] font-bold text-slate-600 md:col-span-2">Inbound adapter<select className={fieldClass} onChange={(event) => setIntegrationId(event.target.value)} value={integrationId}><option value="">Select active adapter</option>{activeIntegrations.map((integration) => <option key={integration.id} value={integration.id}>{integration.vendor}</option>)}</select></label>}
    </div>
    <div className="space-y-3">{items.map((item, index) => <div className={`rounded-2xl border p-4 ${item.critical ? "border-rose-300 bg-rose-50/60" : "border-slate-200 bg-slate-50/50"}`} key={item.clientId}><div className="flex items-center justify-between"><p className="text-[9px] font-black uppercase tracking-[.15em] text-slate-400">Result item {index + 1}</p>{items.length > 1 && <Button aria-label="Remove result item" onClick={() => setItems(items.filter((candidate) => candidate.clientId !== item.clientId))} size="icon" variant="ghost"><X className="size-3.5" /></Button>}</div><div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-4"><Input aria-label="Test name" onChange={(event) => updateItem(index, "name", event.target.value)} placeholder="Test name" value={item.name} /><Input aria-label="LOINC code" onChange={(event) => updateItem(index, "loincCode", event.target.value)} placeholder="LOINC" value={item.loincCode} /><Input aria-label="Result value" onChange={(event) => updateItem(index, "value", event.target.value)} placeholder="Result value" value={item.value} /><Input aria-label="Numeric result value" onChange={(event) => updateItem(index, "numericValue", event.target.value)} placeholder="Numeric value for trend" type="number" value={item.numericValue} /><Input aria-label="Result unit" onChange={(event) => updateItem(index, "unit", event.target.value)} placeholder="Unit" value={item.unit} /><Input aria-label="Reference range" onChange={(event) => updateItem(index, "referenceRange", event.target.value)} placeholder="Reference range" value={item.referenceRange} /><select aria-label="Abnormal flag" className="h-9 rounded-xl border border-slate-200 bg-white px-3 text-[10px] font-bold text-slate-700" disabled={item.critical} onChange={(event) => updateItem(index, "abnormalFlag", event.target.value)} value={item.critical ? "critical" : item.abnormalFlag}><option value="normal">Normal</option><option value="high">High</option><option value="low">Low</option><option value="abnormal">Abnormal</option><option value="critical">Critical</option></select><label className="flex h-9 items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 text-[10px] font-bold text-slate-700"><input checked={item.critical} onChange={(event) => updateItem(index, "critical", event.target.checked)} type="checkbox" /> Critical source flag</label></div></div>)}</div>
...
export function LabResultTransitionControl({ result, canSign, disabled = false }: { result: { id: string; status: string; patientNotificationStatus: string }; canSign: boolean; disabled?: boolean }) {
  ...
  const [pending, setPending] = useState(false);
  const [error, setError] = useState("");
  if (!actions.length) return !canSign && result.status === "needs_review" ? <p className="mt-4 rounded-xl bg-amber-50 px-3 py-2 text-[10px] font-bold text-amber-800">Awaiting a user with provider review permission.</p> : null;
