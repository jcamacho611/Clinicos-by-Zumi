...
export function CreateReferralControl({
  ...
  return <div className="space-y-4 p-5">
    <div className="grid gap-3 md:grid-cols-2">
      ...
      <label className="text-[10px] font-bold text-slate-600">Delivery pathway
      ...
      {connected ? <>
        <label className="text-[10px] font-bold text-slate-600">Connected clinic
        ...
        <label className="text-[10px] font-bold text-slate-600">Verified facility
      ...
      </> : <label className="text-[10px] font-bold text-slate-600 md:col-span-2">External destination
      ...
      <label className="text-[10px] font-bold text-slate-600">Specialty
        <Input className="mt-1" onChange={(event) => setSpecialty(event.target.value)} value={specialty} />
      </label>
      <label className="text-[10px] font-bold text-slate-600">Priority
      ...
      <label className="text-[10px] font-bold text-slate-600">Authorization
    ...
    <label className="block text-[10px] font-bold text-slate-600">Reason for referral
    ...
    <label className="block text-[10px] font-bold text-slate-600">Clinical question
    ...
    <div className="flex flex-wrap items-center gap-3">
      ...
      <p className="text-[9px] leading-5 text-slate-400">Connected delivery requires an active relationship, sharing agreement, and patient consent for demographics and referrals.</p>
    </div>
...
export function ReferralTransitionControl({ referral, disabled = false }: { referral: { id: string; status: string; deliveryStatus: string; destinationType: string; direction: string; patientOutreachStatus: string }; disabled?: boolean }) {
  ...
  return <div className="mt-4 space-y-2 border-t border-slate-100 pt-4">
    <div className="grid gap-2 sm:grid-cols-[1fr_1.4fr_auto]">
      <select className="h-9 rounded-xl border border-slate-200 bg-white px-3 text-[10px] font-bold text-slate-700 outline-none focus:border-sky-400" onChange={(event) => setAction(event.target.value as ReferralAction)} value={selectedAction}>{actions.map((item) => <option key={item} value={item}>{labels[item]}</option>)}</select>
