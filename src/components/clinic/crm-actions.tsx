...
export function LeadTransitionAction({ lead, enabled }: { lead: CRMWorkspace["leads"][number]; enabled: boolean }) {
  ...
  return <div className="mt-4 flex flex-wrap gap-2">{["new", "contacted"].includes(lead.status) && <Button disabled={busy} onClick={() => transition("contact")} size="sm" variant="secondary"><MessageSquareText className="size-3.5" /> Contact</Button>}{["contacted", "new"].includes(lead.status) && <Button disabled={busy} onClick={() => transition("book")} size="sm" variant="primary"><Check className="size-3.5" /> Booked</Button>}{["new", "contacted"].includes(lead.status) && <Button disabled={busy} onClick={() => transition("lose")} size="sm" variant="ghost"><X className="size-3.5" /> Lost</Button>}{lead.status === "booked" && <Button disabled={busy} onClick={() => transition("complete")} size="sm" variant="secondary"><Check className="size-3.5" /> Completed</Button>}{lead.status === "lost" && <Button disabled={busy} onClick={() => transition("reactivate")} size="sm" variant="secondary">Reactivate</Button>}{error && <p className="basis-full text-[12px] font-bold text-rose-600" role="alert">{error}</p>}</div>;
}
