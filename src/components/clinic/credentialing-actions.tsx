...
export function NppesLookupAction({ npi, providerName }: { npi: string | null; providerName: string }) {
  ...
  if (!npi) return <p className="mt-3 text-[12px] text-slate-400">Add the provider NPI before checking the free federal public registries.</p>;
  return <div className="mt-3 space-y-2">
    <div className="rounded-2xl border border-slate-200 bg-white p-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div><p className="text-[12px] font-extrabold uppercase tracking-[.14em] text-slate-500">Free public NPI evidence</p><p className="mt-1 text-[12px] text-slate-400">CMS NPPES · {providerName}</p></div>
      ...
      {evidence === null && <p className="mt-3 text-[12px] font-bold text-amber-700">No NPPES record was returned for NPI {npi}.</p>}
      {evidence && <div className="mt-3 space-y-2 text-[12px] leading-5 text-slate-600"><p><strong>{evidence.name}</strong> · NPI {evidence.npi} · {evidence.status ?? "status not supplied"}</p>{evidence.taxonomies.length > 0 && <p>{evidence.taxonomies.slice(0, 3).map((taxonomy) => `${taxonomy.primary ? "Primary: " : ""}${taxonomy.code ?? "taxonomy"}${taxonomy.description ? ` ${taxonomy.description}` : ""}`).join(" · ")}</p>}<p className="text-amber-800">{evidence.authorityNotice}</p></div>}
...
export function CredentialTransitionAction({ credentialId, verificationStatus, canManage }: { credentialId: string; verificationStatus: string; canManage: boolean }) {
  ...
  if (verificationStatus === "verified") return <span className="inline-flex items-center gap-1 text-[12px] font-extrabold text-teal-700"><BadgeCheck className="size-3.5" /> Primary source recorded</span>;
