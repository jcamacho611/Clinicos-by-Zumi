...
import { encounters, labResults, patientTimeline, qualityGaps } from "@/lib/clinic-data";
  ...
  return <div className="space-y-5">
    ...
    <section className="sticky top-[94px] z-20 rounded-[24px] border border-slate-200 bg-white/95 p-5 shadow-[0_18px_45px_rgba(15,23,42,.08)] backdrop-blur-xl">
      <div className="flex flex-col gap-5 xl:flex-row xl:items-center">
        ...
        <div className="flex gap-2"><Button size="icon" variant="secondary" aria-label="Print chart"><Printer className="size-4" /></Button><Button size="icon" variant="secondary" aria-label="More actions"><MoreHorizontal className="size-4" /></Button><Button asChild variant="primary"><Link href="/encounters/enc-1001"><ClipboardPlus className="size-4" /> Start encounter</Link></Button></div>
      </div>
    ...
    <Tabs.Root defaultValue="Summary">
      ...
      <Tabs.Content className="mt-5 outline-none" value="Encounters"><EncountersTab /></Tabs.Content>
...
function EncountersTab() { return <SectionCard title="Encounter history" action={<Button asChild size="sm" variant="primary"><Link href="/encounters/enc-1001"><Plus className="size-3.5" /> New encounter</Link></Button>}><div className="divide-y divide-slate-100">{encounters.map((encounter) => <Link className="flex items-center gap-4 p-5 hover:bg-slate-50" href={`/encounters/${encounter.id}`} key={encounter.id}><span className="grid size-10 place-items-center rounded-xl bg-teal-50 text-teal-700"><Stethoscope className="size-5" /></span><div className="flex-1"><p className="text-xs font-extrabold text-slate-900">{encounter.type}</p><p className="mt-1 text-[10px] text-slate-400">{encounter.date} · {encounter.provider}</p></div><StatusBadge status={encounter.status} /><ChevronRight className="size-4 text-slate-300" /></Link>)}</div></SectionCard>; }
