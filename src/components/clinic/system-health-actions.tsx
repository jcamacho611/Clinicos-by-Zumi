...
export function IntegrationRetryAction({ eventId, enabled }: { eventId: string; enabled: boolean }) {
  ...
  return <div className="flex items-center gap-2"><Button disabled={busy} onClick={retry} size="sm" variant="secondary">{busy ? <LoaderCircle className="size-3.5 animate-spin" /> : <RotateCw className="size-3.5" />} Queue retry</Button>{error && <p className="text-[12px] font-bold text-rose-600" role="alert">{error}</p>}</div>;
}
