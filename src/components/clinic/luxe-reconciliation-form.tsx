  ...
  return (
    <details className="mt-3 rounded-xl border border-slate-200 bg-slate-50/70 p-3">
      ...
      <form className="mt-3 grid gap-3 sm:grid-cols-2" onSubmit={submit}>
        ...
        <label className="text-[10px] font-bold text-slate-600 sm:col-span-2">Review note<textarea className="mt-1 min-h-20 w-full rounded-lg border border-slate-200 bg-white p-2 text-xs" maxLength={800} minLength={8} name="note" placeholder="Document the source you checked and what you confirmed." required /></label>
        <div className="flex flex-wrap items-center gap-3 sm:col-span-2"><button className="rounded-lg bg-slate-950 px-3 py-2 text-xs font-extrabold text-white disabled:opacity-50" disabled={busy} type="submit">{busy ? "Recording…" : "Record reconciliation"}</button><p aria-live="polite" className="text-[10px] text-slate-500">{notice}</p></div>
      </form>
    </details>
  );
}
