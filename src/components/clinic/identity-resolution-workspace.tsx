  ...
  return <div className="space-y-6">
    ...
    <section className="identity-command relative overflow-hidden rounded-[30px] bg-[#061720] p-6 text-white shadow-[0_30px_90px_rgba(2,18,28,.26)] sm:p-8">
      ...
      <div className="relative grid gap-8 xl:grid-cols-[.72fr_1.28fr] xl:items-center">
        <div><div className="flex items-center gap-3"><span className="grid size-12 place-items-center rounded-2xl bg-cyan-300/10 text-cyan-200"><Fingerprint className="size-5" /></span><div><p className="text-[11px] font-extrabold uppercase tracking-[.22em] text-cyan-200">Master patient index</p><p className="mt-1 text-2xl font-black tracking-[-.05em]">Identity is a link, not a destructive merge.</p></div></div><p className="mt-5 max-w-xl text-xs leading-6 text-slate-300">The locator exposes only source organization, MRN, identity status, and verified identifiers. Clinical details still require the full connected-care authorization chain.</p></div>
        ...
        ].map(([number, title, detail]) => <div className="rounded-2xl border border-white/10 bg-white/[.055] p-4" key={number}><p className="text-[11px] font-black text-cyan-300">{number}</p><p className="mt-4 text-xs font-extrabold">{title}</p><p className="mt-2 text-[11px] leading-5 text-slate-400">{detail}</p></div>)}</div>
      </div>
    </section>
      ...
      <SectionCard title="Identity resolution queue" description="Every match requires a reason, an accountable reviewer, and explicit reconciliation choices.">
        <div className="space-y-4 p-4">{workspace.matches.length ? workspace.matches.map((match) => {
          ...
          return <article className="rounded-2xl border border-slate-200 bg-white p-4" key={match.id}>
            <div className="flex flex-wrap items-center gap-3"><span className="grid size-9 place-items-center rounded-xl bg-cyan-50 text-cyan-700"><GitCompareArrows className="size-4" /></span><div className="min-w-0 flex-1"><p className="text-xs font-extrabold text-slate-950">{Math.round(match.confidenceScore * 100)}% identity confidence</p><p className="mt-1 text-[12px] text-slate-500">Human decision required before any link is created.</p></div><StatusBadge status={match.status.replaceAll("_", " ")} /></div>
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              {[match.source, match.candidate].map((record, index) => <div className={`rounded-2xl p-4 ${index === 0 ? "bg-slate-950 text-white" : "bg-cyan-50 text-slate-950"}`} key={record.patientId}><p className={`text-[11px] font-extrabold uppercase tracking-[.16em] ${index === 0 ? "text-cyan-300" : "text-cyan-700"}`}>{index === 0 ? "Source chart" : "Candidate chart"}</p><p className="mt-3 text-sm font-extrabold">{record.name}</p><p className={`mt-1 text-[12px] ${index === 0 ? "text-slate-400" : "text-slate-500"}`}>{record.organization} · {record.mrn}</p><p className={`mt-3 text-[12px] leading-5 ${index === 0 ? "text-slate-300" : "text-slate-600"}`}>DOB {formatBirthDate(record.dateOfBirth)}<br />{record.phone ?? "No phone"}<br />{record.email ?? "No email"}</p></div>)}
            </div>
            ...
            {match.status === "possible" ? <MatchReviewControl disabled={!mayReview} matchId={match.id} /> : <div className="mt-4 rounded-xl bg-slate-50 p-3 text-[12px] leading-5 text-slate-600"><strong className="capitalize">{match.reviewAction?.replaceAll("_", " ") ?? match.status}</strong>{match.decisionReason ? ` · ${match.decisionReason}` : ""}</div>}
          </article>;
        }) : <div className="p-6 text-center"><ScanSearch className="mx-auto size-6 text-slate-300" /><p className="mt-3 text-xs font-bold text-slate-600">No identity candidates yet.</p><p className="mt-1 text-[12px] text-slate-400">Run a deterministic scan against connected clinics.</p></div>}</div>
      </SectionCard>

      <SectionCard title="Record locator" description="Find linked source charts without exposing clinical content.">
        <div className="space-y-3 p-4">{workspace.patients.map((patient) => <div className="rounded-2xl border border-slate-200 p-4" key={patient.id}><div className="flex items-start gap-3"><span className={`grid size-9 place-items-center rounded-xl ${patient.masterPatientId ? "bg-teal-50 text-teal-700" : "bg-slate-100 text-slate-500"}`}><Fingerprint className="size-4" /></span><div className="min-w-0 flex-1"><p className="text-xs font-extrabold text-slate-950">{patient.firstName} {patient.lastName}</p><p className="mt-1 text-[12px] text-slate-500">{patient.mrn} · {patient.identityStatus.replaceAll("_", " ")}</p><p className="mt-2 text-[11px] text-slate-400">{patient.masterPatientId ? `${patient.locatedRecordCount} source chart${patient.locatedRecordCount === 1 ? "" : "s"} located` : "No master identity assigned"}</p></div></div>{patient.masterPatientId && <RecordLocatorControl masterPatientId={patient.masterPatientId} />}</div>)}</div>
      </SectionCard>
    </section>
