...
export function UploadDocumentControl({ workspace, disabled = false }: { workspace: DocumentWorkspaceData; disabled?: boolean }) {
  ...
  return <div className="space-y-4 p-5">
    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
      <label className="text-[9px] font-black uppercase tracking-[.13em] text-stone-500">Patient<select className={fieldClass} onChange={(event) => { setPatientId(event.target.value); setLinkType("none"); setLinkId(""); }} value={patientId}>{workspace.patients.map((patient) => <option key={patient.id} value={patient.id}>{patient.name} · {patient.mrn}</option>)}</select></label>
      <label className="text-[9px] font-black uppercase tracking-[.13em] text-stone-500">Category<select className={fieldClass} onChange={(event) => setCategoryId(event.target.value)} value={categoryId}>{workspace.categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></label>
      <label className="text-[9px] font-black uppercase tracking-[.13em] text-stone-500">Capture pathway<select className={fieldClass} onChange={(event) => setSourceType(event.target.value)} value={sourceType}>{["upload", "camera", "scan", "import", "generated"].map((item) => <option key={item} value={item}>{item}</option>)}</select></label>
      <label className="text-[9px] font-black uppercase tracking-[.13em] text-stone-500">Source file<input accept=".pdf,.jpg,.jpeg,.png,.webp,.txt" capture={sourceType === "camera" ? "environment" : undefined} className={`${fieldClass} py-2`} onChange={(event) => selectFile(event.target.files?.[0] ?? null)} ref={inputRef} type="file" /></label>
      <label className="text-[9px] font-black uppercase tracking-[.13em] text-stone-500">Document name<Input className="mt-1" onChange={(event) => setName(event.target.value)} value={name} /></label>
      <label className="text-[9px] font-black uppercase tracking-[.13em] text-stone-500">Link to<select className={fieldClass} onChange={(event) => { setLinkType(event.target.value); setLinkId(""); }} value={linkType}><option value="none">Patient chart only</option><option value="encounter">Encounter</option><option value="referral">Referral</option><option value="nofault">No-fault case</option><option value="workers_comp">Workers comp case</option></select></label>
      <label className="text-[9px] font-black uppercase tracking-[.13em] text-stone-500">Linked record<select className={fieldClass} disabled={linkType === "none"} onChange={(event) => setLinkId(event.target.value)} value={linkId}><option value="">{linkType === "none" ? "No secondary link" : "Select a matching record"}</option>{linkedOptions.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select></label>
      <label className="text-[9px] font-black uppercase tracking-[.13em] text-stone-500">Expiration date<Input className="mt-1" onChange={(event) => setExpiresAt(event.target.value)} type="date" value={expiresAt} /></label>
    </div>
    <div className="grid gap-3 md:grid-cols-2"><label className="text-[9px] font-black uppercase tracking-[.13em] text-stone-500">Description<textarea className={`${fieldClass} min-h-20 py-3`} onChange={(event) => setDescription(event.target.value)} value={description} /></label><label className="text-[9px] font-black uppercase tracking-[.13em] text-stone-500">Tags, comma separated<textarea className={`${fieldClass} min-h-20 py-3`} onChange={(event) => setTags(event.target.value)} value={tags} /></label></div>
    <div className="flex flex-wrap items-center gap-3"><label className="flex items-center gap-2 rounded-xl bg-amber-50 px-3 py-2 text-[10px] font-bold text-amber-950"><input checked={requestPatientRelease} onChange={(event) => setRequestPatientRelease(event.target.checked)} type="checkbox" /> Request portal release after review</label><Button disabled={unavailable} onClick={upload} variant="primary">{pending ? <LoaderCircle className="size-4 animate-spin" /> : <UploadCloud className="size-4" />} Encrypt and receive</Button><p className="text-[9px] text-stone-400">PDF, JPG, PNG, WEBP, or TXT · 10 MB max · never public by default</p></div>
...
export function DocumentContentActions({ document, canDownload, canPrint }: { document: DocumentWorkspaceData["documents"][number]; canDownload: boolean; canPrint: boolean }) {
  if (!document.hasStoredContent) return <span className="rounded-lg bg-stone-100 px-2 py-1 text-[9px] font-bold text-stone-500">External source fallback</span>;
...
export function DocumentPacketExportControl({ selectedIds, disabled = false }: { selectedIds: string[]; disabled?: boolean }) {
  ...
  return <div className="rounded-2xl border border-stone-700 bg-stone-900 p-4 text-white"><div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-xl bg-amber-300 text-stone-950"><FileArchive className="size-4" /></span><div><p className="text-xs font-black">Secure packet export</p><p className="mt-1 text-[10px] text-stone-400">{selectedIds.length} selected · checksums verified · manifest included</p></div></div><Input className="mt-3 border-stone-700 bg-stone-800 text-white" onChange={(event) => setPurpose(event.target.value)} placeholder="Required purpose of export" value={purpose} /><Button className="mt-3 w-full" disabled={disabled || pending || !selectedIds.length || purpose.trim().length < 8} onClick={exportPacket} size="sm" variant="primary">{pending ? <LoaderCircle className="size-3.5 animate-spin" /> : <Download className="size-3.5" />} Export encrypted-source ZIP</Button>{error && <p className="mt-2 text-[10px] font-bold text-rose-300" role="alert">{error}</p>}</div>;
}
