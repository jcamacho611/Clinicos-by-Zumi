...
import {
  ArrowRight, Building2, CircleAlert, Handshake, Network, RadioTower, Route, ShieldCheck, Sparkles, } from "lucide-react";
...
function Signal({ children, tone = "slate" }: { children: React.ReactNode; tone?: "cyan" | "emerald" | "amber" | "rose" | "violet" | "slate" }) {
  ...
  return <span className={cn("inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[11px] font-bold", tones[tone])}>{children}</span>;
}
    ...
    <header className="border-b border-slate-200 pb-5">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-5 text-[12px] font-extrabold uppercase tracking-[.18em] text-slate-400">{meta[0]}</p>
...
function StatsStrip({ workspace }: { workspace: NetworkCommandWorkspaceData }) {
  ...
  return <section className="mt-5 overflow-x-auto border-y border-slate-200 bg-white">
    <div className="grid min-w-[780px] grid-cols-5 divide-x divide-slate-200">
      {items.map(([label, value, detail]) => <div className="px-4 py-4" key={label}><p className="text-2xl font-black tracking-[-.04em] text-slate-950">{value}</p><p className="mt-1 text-xs font-extrabold text-slate-700">{label}</p><p className="mt-1 text-[12px] leading-4 text-slate-400">{detail}</p></div>)}
    </div>
  </section>;
}
    ...
    <div className="overflow-x-auto">
      <table className="w-full min-w-[980px] text-left">
        ...
        <tbody>{workspace.partners.map((partner) => <tr className="border-b border-slate-100 text-xs last:border-0 hover:bg-slate-50" key={partner.id}>
          <td className="px-4 py-4"><div className="flex items-center gap-3"><span className="grid size-9 place-items-center rounded-lg bg-slate-100 text-slate-600"><Building2 className="size-4" /></span><div><p className="font-extrabold text-slate-950">{partner.name}</p><p className="mt-1 text-[12px] text-slate-400">{title(partner.type)} · {partner.locations.length} locations</p></div></div></td>
...
function NetworkMap({ workspace }: { workspace: NetworkCommandWorkspaceData }) {
  return <section className="mt-5 overflow-hidden rounded-lg border border-slate-200 bg-white">
    ...
    <div className="grid gap-4 p-5 lg:grid-cols-[1fr_180px_1fr] lg:items-center">
      <div className="space-y-2">{workspace.partners.filter((_, index) => index % 2 === 0).map((partner) => <div className="rounded-lg border border-slate-200 p-3" key={partner.id}><div className="flex items-center justify-between gap-2"><p className="text-xs font-extrabold text-slate-950">{partner.name}</p><Status value={partner.status} /></div><p className="mt-1 text-[12px] text-slate-400">{title(partner.relationshipType)} · {partner.pendingHandoffs} open</p></div>)}</div>
      ...
      <div className="space-y-2">{workspace.partners.filter((_, index) => index % 2 === 1).map((partner) => <div className="rounded-lg border border-slate-200 p-3" key={partner.id}><div className="flex items-center justify-between gap-2"><p className="text-xs font-extrabold text-slate-950">{partner.name}</p><Status value={partner.status} /></div><p className="mt-1 text-[12px] text-slate-400">{title(partner.relationshipType)} · {partner.capacityAvailable} capacity</p></div>)}</div>
    </div>
  </section>;
}
...
function HandoffCard({ handoff, workspace }: { handoff: NetworkCommandWorkspaceData["handoffs"][number]; workspace: NetworkCommandWorkspaceData }) {
  return <article className="rounded-lg border border-slate-200 bg-white p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex flex-wrap gap-1.5"><Signal tone={handoff.direction === "inbound" ? "violet" : "cyan"}>{handoff.direction}</Signal><Status value={handoff.status} /><Status value={handoff.priority} /></div><h3 className="mt-4 text-base font-black text-slate-950">{title(handoff.category)}</h3><p className="mt-1 text-xs text-slate-500">{handoff.sourceOrganizationName} → {handoff.destinationOrganizationName}</p></div><p className="text-right text-[12px] leading-4 text-slate-400">Updated<br />{formatDate(handoff.updatedAt)}</p></div><div className="mt-4 grid gap-3 sm:grid-cols-3"><div className="border-t border-slate-200 pt-3"><p className="text-[11px] font-bold uppercase tracking-[.1em] text-slate-400">Patient</p><p className="mt-1 text-xs font-bold text-slate-700">{handoff.patientName}</p><p className="mt-1 text-[12px] text-slate-400">{handoff.patientMrn}</p></div><div className="border-t border-slate-200 pt-3"><p className="text-[11px] font-bold uppercase tracking-[.1em] text-slate-400">Authority</p><p className="mt-1 text-xs font-bold text-slate-700">{title(handoff.purposeOfUse)}</p><p className="mt-1 text-[12px] text-slate-400">{handoff.dataCategories.length} minimum categories</p></div><div className="border-t border-slate-200 pt-3"><p className="text-[11px] font-bold uppercase tracking-[.1em] text-slate-400">Delivery</p><p className="mt-1 text-xs font-bold text-slate-700">{title(handoff.deliveryMethod)}</p><p className="mt-1 text-[12px] text-slate-400">{handoff.humanReviewRequired ? "Human review required" : "Review policy"}</p></div></div><p className="mt-4 text-xs leading-5 text-slate-600">{handoff.requestedAction}</p>{handoff.failureReason && <p className="mt-3 rounded-lg border border-rose-200 bg-rose-50 p-3 text-xs leading-5 text-rose-800">{handoff.failureReason}</p>}<div className="mt-4 flex flex-wrap gap-1.5">{handoff.dataCategories.map((category) => <Signal key={category}>{title(category)}</Signal>)}<Signal tone="amber">Due {formatDate(handoff.dueAt)}</Signal></div>{handoff.events.length > 0 && <details className="mt-4 rounded-lg border border-slate-200 bg-slate-50 p-3"><summary className="cursor-pointer text-[12px] font-bold text-slate-600">Audit trail · {handoff.events.length} events</summary><div className="mt-3 space-y-2">{handoff.events.map((event) => <div className="flex items-start justify-between gap-3 text-[12px] leading-4 text-slate-500" key={event.id}><span><strong className="text-slate-700">{title(event.action)}</strong>{event.note ? ` · ${event.note}` : ""}</span><span className="shrink-0">{formatDate(event.createdAt)}</span></div>)}</div></details>}<div className="mt-4"><NetworkHandoffActions canUpdate={workspace.permissions.canUpdate} handoff={handoff} /></div></article>;
}

function HandoffBoard({ workspace }: { workspace: NetworkCommandWorkspaceData }) {
  return <section className="mt-5"><div className="grid gap-5 xl:grid-cols-[.72fr_1.28fr]"><NetworkHandoffComposer workspace={workspace} /><div className="rounded-lg border border-slate-200 bg-white p-5"><div className="flex items-center justify-between gap-3"><div><p className="text-[12px] font-extrabold uppercase tracking-[.14em] text-slate-400">Delivery protocol</p><h3 className="mt-2 text-lg font-black text-slate-950">Truthful status sequence</h3></div><Route className="size-5 text-slate-400" /></div><div className="mt-5 divide-y divide-slate-100">{[["Connected", "Ready → Sent connected → Received → Acknowledged → Scheduled → Completed"], ["Manual", "Ready → Sent manual → Receipt confirmation → Completed"], ["Fax", "Ready → Fax pending → Received or Failed → Retry"], ["Direct", "Ready → Direct pending → Received or Failed → Retry"]].map(([label, detail]) => <div className="py-3" key={label}><p className="text-xs font-black text-slate-800">{label}</p><p className="mt-1 text-xs leading-5 text-slate-500">{detail}</p></div>)}</div><div className="mt-4 rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs leading-5 text-amber-900"><strong>No silent delivery.</strong> Fax and Direct remain pending until a human records receipt. Failed delivery stays visible and recoverable.</div></div></div><div className="mt-5 grid gap-4 xl:grid-cols-2">{workspace.handoffs.map((handoff) => <HandoffCard handoff={handoff} key={handoff.id} workspace={workspace} />)}{!workspace.handoffs.length && <p className="rounded-lg border border-dashed border-slate-300 bg-white p-10 text-center text-xs text-slate-500 xl:col-span-2">No governed partner handoffs have been prepared.</p>}</div></section>;
}
