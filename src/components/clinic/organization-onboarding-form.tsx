  ...
  return (
    <form className="rounded-[28px] border border-white/10 bg-white p-5 text-slate-950 shadow-[0_32px_100px_rgba(0,0,0,.35)] sm:p-7" onSubmit={submit}>
      <div className="flex items-center justify-between border-b border-slate-100 pb-5">
        <div>
          <p className="flex items-center gap-2 text-[12px] font-extrabold uppercase tracking-[.2em] text-cyan-700"><Sparkles className="size-3.5" /> Smart setup</p>
        ...
        <span className="rounded-full bg-slate-950 px-3 py-1.5 text-[12px] font-extrabold text-white">{completion}% ready</span>
      </div>
      ...
      {step === 1 ? (
          ...
          <section className="rounded-2xl border border-cyan-100 bg-cyan-50/70 p-4">
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-1 text-[12px] leading-5 text-slate-500">Example: “Brooklyn med spa with 8 staff. We need better lead follow-up, scheduling, and revenue tracking.”</p>
                ...
                <div className="mt-3 flex items-center gap-3">
                  ...
                  {smartApplied && <span className="text-[12px] font-bold text-emerald-700">Suggested fields applied. Review and confirm below.</span>}
                </div>
              </div>
            </div>
          </section>
        ...
        <div className="mt-6 space-y-4">
          ...
          <label className="block text-xs font-bold text-slate-700">Create password<Input autoComplete="new-password" className="mt-2 h-11" minLength={12} onChange={(event) => update("password", event.target.value)} required type="password" value={form.password} /><span className="mt-2 block text-[12px] font-medium leading-5 text-slate-400">12+ characters with uppercase, lowercase, number, and symbol.</span></label>
      ...
      <div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-5 text-[12px] font-semibold text-slate-500">
      ...
      <div className="mt-4 flex flex-wrap gap-2">{["Smart prefill", "Tenant isolated", "Audited", "Manual fallbacks ready"].map((label) => <span className="flex items-center gap-1 rounded-full bg-slate-100 px-2.5 py-1 text-[11px] font-bold text-slate-600" key={label}><Check className="size-3 text-cyan-700" />{label}</span>)}</div>
    </form>
  );
}
