        ...
        {workspace.tasks.map((task) => <div className="p-5" key={task.id}>
          <div className="grid gap-4 md:grid-cols-[1.4fr_.6fr_.5fr_auto] md:items-start">
            <div>
              ...
              <p className="mt-1 text-[10px] text-slate-400">{task.patientName}</p>
              {task.details ? <p className="mt-2 text-[10px] leading-5 text-slate-500">{task.details}</p> : null}
            </div>
            <div><p className="text-[8px] font-bold text-slate-400">OWNER</p><p className="mt-1 break-all text-xs font-bold text-slate-700">{task.ownerId ?? "Unassigned"}</p></div>
            <div><p className="text-[8px] font-bold text-slate-400">DUE</p><p className="mt-1 text-xs font-bold text-slate-700">{task.dueAt ? new Date(task.dueAt).toLocaleString() : "No due time"}</p></div>
