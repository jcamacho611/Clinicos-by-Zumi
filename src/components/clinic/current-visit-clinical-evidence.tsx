...
function AttentionBadge({ label, tone = "neutral" }: { label: string; tone?: "neutral" | "warning" | "danger" }) {
  ...
  return <span className={`rounded-full border px-2 py-1 text-[11px] font-extrabold ${classes}`}>{label}</span>;
}
...
export function CurrentVisitClinicalEvidenceCard({ evidence }: { evidence: CurrentVisitClinicalEvidence }) {
  return <section className="rounded-[20px] border border-slate-200 bg-white p-5 shadow-sm">
    ...
    {evidence.status === "none_available" ? <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-4">
      ...
      <div>
        <div className="flex items-center justify-between gap-3"><p className="text-[11px] font-extrabold uppercase tracking-[.13em] text-slate-500">Recent labs</p><span className="text-[11px] font-bold text-slate-400">Up to 6</span></div>
        <div className="mt-3 space-y-3">
          {evidence.labs.length === 0 ? <p className="text-[12px] text-slate-400">No lab evidence available.</p> : evidence.labs.map((result) => <div className="rounded-xl border border-slate-200 bg-slate-50/70 p-3" key={result.id}>
            ...
            <p className="mt-2 text-[11px] font-bold text-slate-400">{dateLabel(result.resultedAt)} · v{result.version} · {result.source}{result.sourceReference ? ` · ${result.sourceReference}` : ""}</p>
            {result.correctionOfId && <p className="mt-1 text-[11px] font-bold text-slate-500">Correction of prior result {result.correctionOfId}</p>}
            ...
            {result.itemsTruncated && <p className="mt-3 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-[11px] font-bold leading-5 text-amber-800">Current Visit is showing a partial panel. The source contains {result.totalItemCount} total source items; review the authoritative lab workspace for the complete result.</p>}
          </div>)}
        </div>
      </div>

      <div>
        <div className="flex items-center justify-between gap-3"><p className="text-[11px] font-extrabold uppercase tracking-[.13em] text-slate-500">Recent imaging</p><span className="text-[11px] font-bold text-slate-400">Up to 6</span></div>
        <div className="mt-3 space-y-3">
          {evidence.imaging.length === 0 ? <p className="text-[12px] text-slate-400">No imaging evidence available.</p> : evidence.imaging.map((result) => <div className="rounded-xl border border-slate-200 bg-slate-50/70 p-3" key={result.id}>
            ...
            <p className="mt-2 text-[11px] font-bold text-slate-400">{dateLabel(result.studyPerformedAt)} · v{result.version} · {result.modality} · {result.bodyPart} · {result.facility}</p>
            <p className="mt-1 text-[11px] font-bold text-slate-400">{result.source}{result.sourceReference ? ` · ${result.sourceReference}` : ""}</p>
            {result.correctionOfId && <p className="mt-1 text-[11px] font-bold text-slate-500">Correction of prior report {result.correctionOfId}</p>}
            {result.impression && <div className="mt-3"><p className="text-[11px] font-extrabold uppercase tracking-[.12em] text-slate-400">Source impression</p><p className="mt-1 text-[11px] leading-5 text-slate-700">{result.impression}</p></div>}
            {result.findings && <details className="mt-3"><summary className="cursor-pointer text-[11px] font-extrabold text-teal-700">Source findings</summary><p className="mt-2 text-[11px] leading-5 text-slate-600">{result.findings}</p></details>}
          </div>)}
        </div>
      </div>
    </div>}
