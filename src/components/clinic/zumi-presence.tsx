        ...
        >
          ...
          <div className="flex gap-1 overflow-x-auto border-b border-[#e6817b]/10 bg-[#0d0608] px-3 py-2 sm:px-5" role="tablist" aria-label="Zumi interaction mode">
            {MODE_META.map(({ key, label, icon: Icon }) => (
              <button
                ...
                className={cn("inline-flex shrink-0 items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-[10px] font-bold transition", mode === key ? "bg-[#e6817b]/14 text-[#fff8f6] ring-1 ring-[#e6817b]/18" : "text-[#9f8985] hover:bg-[#e6817b]/[.06] hover:text-[#e9d8d5]")}
                key={key}
                onClick={() => setMode(key)}
                role="tab"
          ...
          {settingsOpen && (
            <div className="grid gap-3 border-b border-[#e6817b]/10 bg-[#12090b] p-3 text-xs sm:grid-cols-2 sm:px-5">
              <label className="space-y-1"><span className="text-[9px] font-extrabold uppercase tracking-[.14em] text-[#8f7773]">Response</span><select className="w-full rounded-lg border border-[#e6817b]/14 bg-[#090405] px-2 py-1.5 text-[#f8efed]" onChange={(event) => setResponseLength(event.target.value as ResponseLength)} value={responseLength}><option value="concise">Concise</option><option value="balanced">Balanced</option><option value="detailed">Detailed</option></select></label>
              <label className="space-y-1"><span className="text-[9px] font-extrabold uppercase tracking-[.14em] text-[#8f7773]">Language</span><select className="w-full rounded-lg border border-[#e6817b]/14 bg-[#090405] px-2 py-1.5 text-[#f8efed]" onChange={(event) => setLanguageStyle(event.target.value as LanguageStyle)} value={languageStyle}><option value="plain">Plain</option><option value="professional">Professional</option><option value="technical">Technical</option></select></label>
              <label className="space-y-1"><span className="text-[9px] font-extrabold uppercase tracking-[.14em] text-[#8f7773]">Autonomy</span><select className="w-full rounded-lg border border-[#e6817b]/14 bg-[#090405] px-2 py-1.5 text-[#f8efed]" onChange={(event) => setAutonomy(event.target.value as Autonomy)} value={autonomy}><option value="answer_only">Answer only</option><option value="suggest_actions">Suggest actions</option><option value="prepare_actions">Prepare actions</option></select></label>
              <button className="flex items-center justify-center gap-2 rounded-lg border border-[#e6817b]/14 bg-[#090405] px-2 py-1.5 text-[10px] font-bold text-[#d8c1bd]" onClick={() => setSpeechOutput((value) => !value)} type="button">{speechOutput ? <Volume2 className="size-3.5" /> : <VolumeX className="size-3.5" />}{speechOutput ? "Speech on" : "Speech off"}</button>
              <div className="space-y-2 sm:col-span-2">
                <span className="text-[9px] font-extrabold uppercase tracking-[.14em] text-[#8f7773]">Memory</span>
                <div className="flex gap-2"><input className="min-w-0 flex-1 rounded-lg border border-[#e6817b]/14 bg-[#090405] px-2 py-1.5 text-[11px] text-[#f8efed] placeholder:text-[#725d59]" maxLength={1500} onChange={(event) => setMemoryNote(event.target.value)} placeholder="Remember a safe preference for future conversations…" value={memoryNote} /><Button className="h-8 px-3 text-[10px]" disabled={!memoryNote.trim()} onClick={saveMemory} type="button">Remember</Button></div>
                {memoryStatus && <p aria-live="polite" className="text-[10px] text-[#9f8985]">{memoryStatus}</p>}
              </div>
            </div>
          )}
          ...
          <div className="flex-1 overflow-y-auto bg-[radial-gradient(circle_at_50%_0%,rgba(92,22,29,.13),transparent_34%)]" aria-live="polite">
            <div className={cn("mx-auto space-y-5 px-4 py-6", dedicatedPage ? "max-w-[900px] sm:px-8 sm:py-10" : "max-w-full")}>
              ...
              {(trustedActions.length > 0 || trustedBlockers.length > 0) && (
                <div className="rounded-xl border border-[#d6b787]/20 bg-[#d6b787]/[.06] p-4 text-[10px] text-[#e9d8d5]">
              ...
              {(toolsUsed.length > 0 || sources.length > 0 || orchestration?.candidateTools?.length) && (
                <details className="rounded-xl border border-[#e6817b]/10 bg-[#12090b]/70 p-3 text-[10px] text-[#9f8985]">
          ...
          <form className="border-t border-[#e6817b]/12 bg-[#080304]/98 p-3 sm:px-5 sm:py-4" onSubmit={askZumi}>
            <div className={cn("mx-auto", dedicatedPage && "max-w-[900px]")}>
              ...
              <div className="mt-2 flex items-center gap-2">
                <VoiceInputButton className="[&_button]:h-8 [&_button]:rounded-lg [&_button]:border-[#e6817b]/14 [&_button]:bg-[#12090b] [&_button]:px-2.5 [&_button]:text-[10px] [&_button]:text-[#d8c1bd]" onTranscript={(transcript) => { setSpeechOutput(true); void sendQuestion(transcript, { voice: true }); }} />
                <span className="hidden text-[9px] text-[#725d59] sm:inline">Enter sends · Shift+Enter adds a line · voice sends when recognition completes</span>
