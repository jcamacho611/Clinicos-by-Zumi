...
function Stat({ icon, label, value, detail }: { icon: React.ReactNode; label: string; value: string; detail: string }) {
  return <Card className="border-stone-200 bg-[#fffdf7] p-4"><div className="flex items-center justify-between"><span className="grid size-9 place-items-center rounded-xl bg-stone-950 text-amber-300">{icon}</span><p className="text-2xl font-black tracking-[-.06em] text-stone-950">{value}</p></div><p className="mt-5 text-[9px] font-black uppercase tracking-[.16em] text-stone-500">{label}</p><p className="mt-1 text-[10px] text-stone-400">{detail}</p></Card>;
}
  ...
  return <div className="space-y-6">
    ...
    <section className="grid gap-6 2xl:grid-cols-[1fr_320px]">
      <SectionCard title="Governed document library" description={`${filtered.length} records match the current view. Superseded versions stay immutable and searchable.`}>
        ...
        <div className="divide-y divide-stone-100">{filtered.map((document) => <article className="p-5" key={document.id}><div className="grid gap-4 xl:grid-cols-[auto_1.1fr_.65fr_.55fr_auto] xl:items-start"><label className="pt-2"><input aria-label={`Select ${document.name} for packet export`} checked={selectedIds.includes(document.id)} disabled={!document.hasStoredContent || document.status !== "active"} onChange={() => toggleSelection(document.id)} type="checkbox" /></label><div className="flex gap-3"><span className="grid size-11 shrink-0 place-items-center rounded-2xl bg-amber-100 text-amber-950"><FileCheck2 className="size-5" /></span><div className="min-w-0"><p className="truncate text-xs font-black text-stone-950">{document.name}</p><p className="mt-1 text-[10px] text-stone-400">{document.patientName}{document.patientMrn ? ` · ${document.patientMrn}` : ""}</p><div className="mt-3 flex flex-wrap gap-1.5"><Badge tone="amber">{document.categoryName}</Badge><Badge>v{document.version}</Badge><Badge tone={document.status === "active" ? "teal" : "slate"}>{document.status}</Badge>{document.lockedAt && <Badge tone="slate">locked</Badge>}{document.tags.slice(0, 3).map((tag) => <Badge key={tag}><Tags className="mr-1 size-2.5" />{tag}</Badge>)}</div></div></div><div><p className="text-[9px] font-black uppercase tracking-[.13em] text-stone-400">Custody</p><p className="mt-2 text-[10px] font-bold text-stone-700">{document.sourceType} · {formatBytes(document.sizeBytes)}</p><p className="mt-1 text-[9px] text-stone-400">{formatDate(document.createdAt)}</p><p className="mt-2 truncate font-mono text-[8px] text-stone-400">{document.checksumSha256 ?? "external source reference"}</p></div><div className="space-y-2"><StatusBadge status={document.reviewStatus} /><StatusBadge status={document.patientVisible ? "Patient visible" : document.releaseStatus} />{document.expiresAt && <p className="text-[9px] text-stone-400">Expires {formatDate(document.expiresAt)}</p>}</div><DocumentContentActions canDownload={canManage} canPrint={canUpdate} document={document} /></div><DocumentTransitionControl canSign={canSign} canUpdate={canUpdate} document={document} />{canCreate && <CreateDocumentVersionControl document={document} />}</article>)}{!filtered.length && <div className="p-10 text-center"><ShieldAlert className="mx-auto size-7 text-stone-300" /><p className="mt-3 text-xs font-bold text-stone-700">No documents match this governed view.</p></div>}</div>
      </SectionCard>
      <div className="space-y-6"><DocumentPacketExportControl disabled={!canManage} selectedIds={selectedStoredIds} /><SectionCard title="Category policy"><div className="space-y-2 p-3">{workspace.categories.map((item) => <div className="rounded-xl border border-stone-100 p-3" key={item.id}><div className="flex items-center justify-between"><p className="text-[10px] font-black text-stone-900">{item.name}</p><Badge tone={item.reviewRequired ? "amber" : "slate"}>{item.reviewRequired ? "Review gate" : "No review"}</Badge></div><p className="mt-2 text-[9px] leading-4 text-stone-400">{item.defaultAccess.replaceAll("_", " ")} · {item.retentionDays ? `${item.retentionDays} day retention` : "policy retention"}</p></div>)}</div></SectionCard><SectionCard title="Custody ledger"><div className="space-y-2 p-3">{workspace.events.slice(0, 7).map((event) => <div className="flex gap-3 rounded-xl bg-stone-50 p-3" key={event.id}><History className="mt-0.5 size-3.5 shrink-0 text-amber-700" /><div><p className="text-[10px] font-bold text-stone-800">{event.eventType.replaceAll("_", " ")}</p><p className="mt-1 text-[9px] text-stone-400">{formatDate(event.createdAt)} · {event.actorId ?? "system"}</p></div></div>)}{!workspace.events.length && <p className="p-3 text-[10px] text-stone-400">Custody events will appear after the first governed action.</p>}</div></SectionCard><Card className="bg-amber-50 p-5"><Archive className="size-5 text-amber-800" /><p className="mt-4 text-sm font-black text-stone-950">Deletion is not a routine action.</p><p className="mt-2 text-[10px] leading-5 text-stone-600">ClinicOS archives, versions, and audits clinical records. Retention and authorized destruction require a separate policy workflow.</p></Card></div>
    </section>
  </div>;
}
