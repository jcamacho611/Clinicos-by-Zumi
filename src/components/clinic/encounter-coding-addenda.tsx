...
export function EncounterCodingAddenda({ canSign, editable, encounter }: { canSign: boolean; editable: boolean; encounter: Encounter }) {
  ...
  return <div className="space-y-5">
    ...
    {locked && <Card className="overflow-hidden"><div className="border-b border-slate-100 px-5 py-4"><p className="text-sm font-extrabold text-slate-950">Signed addenda</p><p className="mt-1 text-[12px] text-slate-500">Addenda append clarification without changing the locked original note.</p></div><div className="space-y-4 p-5">{addenda.map((addendum) => <article className="rounded-xl border border-slate-200 p-4" key={addendum.id}><div className="flex flex-wrap items-center gap-2"><Badge tone="amber">{addendum.reason}</Badge><span className="text-[11px] font-bold text-slate-400">{addendum.author} · {new Date(addendum.signedAt).toLocaleString()}</span></div><p className="mt-3 whitespace-pre-wrap text-xs leading-6 text-slate-700">{addendum.text}</p></article>)}{addenda.length === 0 && <p className="text-xs text-slate-500">No addenda have been signed.</p>}{canSign && <div className="grid gap-3 rounded-xl bg-amber-50 p-4"><Input onChange={(event) => setReason(event.target.value)} placeholder="Reason for addendum" value={reason} /><textarea className="min-h-28 rounded-xl border border-amber-200 bg-white p-3 text-xs outline-none focus:ring-4 focus:ring-amber-100" onChange={(event) => setText(event.target.value)} placeholder="Enter the clarification or correction. The original note will remain unchanged." value={text} /><label className="flex items-start gap-2 text-[12px] leading-5 text-amber-900"><input checked={attestation} className="mt-1 accent-amber-700" onChange={(event) => setAttestation(event.target.checked)} type="checkbox" />I attest that this addendum is accurate and understand it will be signed and locked.</label><Button className="justify-self-end" disabled={busy === "addendum" || !reason.trim() || !text.trim() || !attestation} onClick={() => void signAddendum()}>{busy === "addendum" ? <LoaderCircle className="size-4 animate-spin" /> : <FilePlus2 className="size-4" />} Sign addendum</Button></div>}</div></Card>}
