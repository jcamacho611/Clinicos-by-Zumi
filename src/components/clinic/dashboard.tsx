    ...
    return <div className="space-y-6">
      <section className="relative overflow-hidden rounded-[28px] bg-[#07151c] p-7 text-white shadow-[0_28px_70px_rgba(7,21,28,.24)] sm:p-10">
        ...
        <div className="relative max-w-3xl">
          <span className="inline-flex items-center gap-2 rounded-full bg-cyan-300/10 px-3 py-1.5 text-[12px] font-extrabold uppercase tracking-[.16em] text-cyan-200"><CheckCircle2 className="size-3.5" /> {onboardingComplete ? "Workspace created" : "Workspace ready"}</span>
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        {setupSteps.map(([Icon, title, body, href], index) => <Card className="group" key={title}><CardContent><div className="flex items-start gap-4"><span className="grid size-11 shrink-0 place-items-center rounded-xl bg-cyan-50 text-cyan-700"><Icon className="size-5" /></span><div className="min-w-0 flex-1"><p className="text-[12px] font-extrabold uppercase tracking-[.14em] text-slate-400">Step {index + 1}</p><h3 className="mt-1 text-base font-extrabold text-slate-950">{title}</h3><p className="mt-2 text-xs leading-5 text-slate-500">{body}</p><Link className="mt-4 inline-flex items-center gap-2 text-xs font-extrabold text-cyan-700 hover:text-cyan-600" href={href}>Open setup <ArrowRight className="size-3.5 transition group-hover:translate-x-1" /></Link></div></div></CardContent></Card>)}
      </section>
    ...
    <div className="space-y-6">
      <section className="relative overflow-hidden rounded-[26px] bg-[#102033] p-6 text-white shadow-[0_24px_60px_rgba(16,32,51,.2)] sm:p-8">
        ...
        <div className="relative flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
          <div>
            <p className="text-[12px] font-extrabold uppercase tracking-[.22em] text-teal-300">Tuesday, July 14 · 9:48 AM</p>
          ...
          <motion.div animate={{ opacity: 1, y: 0 }} initial={{ opacity: 0, y: 12 }} key={metric.label} transition={{ delay: index * 0.06, duration: 0.35 }}>
            <Card className="metric-sheen h-full">
              <CardContent>
                ...
                <p className="mt-2 text-[12px] font-semibold text-slate-400">{metric.change}</p>
              </CardContent>
            </Card>
          </motion.div>
        ))}
      ...
      <section className="grid gap-6 xl:grid-cols-[1.35fr_.65fr]">
        ...
        <Card className="overflow-hidden">
          ...
          <CardContent>
            ...
            <div className="mt-3 grid grid-cols-2 gap-3"><div className="rounded-xl bg-slate-50 p-3"><p className="text-[11px] font-bold text-slate-400">AVG. CLOSE TIME</p><p className="mt-1 text-lg font-extrabold text-slate-950">1h 24m</p></div><div className="rounded-xl bg-teal-50 p-3"><p className="text-[11px] font-bold text-teal-700">SIGNED TODAY</p><p className="mt-1 text-lg font-extrabold text-slate-950">8 / 9</p></div></div>
          </CardContent>
        </Card>
      </section>
      ...
      <section className="grid gap-6 xl:grid-cols-[1fr_1fr]">
        ...
        <Card>
          ...
          <CardContent className="grid gap-3 pt-4 sm:grid-cols-2">
            <div className="rounded-2xl bg-slate-950 p-4 text-white"><PhoneMissed className="size-5 text-lime-300" /><p className="mt-5 text-2xl font-extrabold">4</p><p className="mt-1 text-xs font-bold">Missed calls</p><p className="mt-2 text-[12px] leading-5 text-slate-400">3 received an automatic acknowledgment. 1 needs callback.</p></div>
            <div className="rounded-2xl bg-[#e1f6f2] p-4"><UsersRound className="size-5 text-teal-700" /><p className="mt-5 text-2xl font-extrabold text-slate-950">7</p><p className="mt-1 text-xs font-bold text-slate-900">New requests</p><p className="mt-2 text-[12px] leading-5 text-slate-500">Appointment and intake requests waiting for staff review.</p></div>
          </CardContent>
        </Card>
      </section>
    </div>
