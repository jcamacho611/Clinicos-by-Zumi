...
export function InsuranceWorkspaceReal({ workspace }: { workspace: InsuranceWorkspaceData }) {
  ...
  return <div className="space-y-6">
    <div className="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-5 lg:flex-row lg:items-end lg:justify-between">
      <div>
        <p className="text-[12px] font-extrabold uppercase tracking-[.18em] text-teal-700">Insurance</p>
    ...
    <SectionCard title="Active coverage and latest evidence" description="Member identifiers are masked in this overview. Open the patient chart for broader patient context.">
      <div className="overflow-x-auto"><table className="w-full min-w-[1050px] text-left"><thead><tr className="border-b border-slate-100 text-[11px] font-extrabold uppercase tracking-[.14em] text-slate-400"><th className="px-5 py-3">Patient</th><th className="px-3 py-3">Coverage</th><th className="px-3 py-3">Benefits recorded</th><th className="px-3 py-3">Evidence source</th><th className="px-3 py-3">Checked</th><th className="px-5 py-3">Status</th></tr></thead><tbody>{workspace.coverages.map((coverage) => {
        ...
        return <tr className="border-b border-slate-100 text-xs last:border-0" key={coverage.id}><td className="px-5 py-5"><Link className="font-extrabold text-slate-900 hover:text-sky-700" href={`/patients/${coverage.patientId}`}>{coverage.patientName}</Link><p className="mt-1 text-[12px] text-slate-400">{coverage.patientMrn}</p></td><td className="px-3 py-5"><p className="font-bold text-slate-800">{coverage.payer}</p><p className="mt-1 text-[12px] text-slate-400">{coverage.planName ?? "Plan not recorded"} · member {memberLabel(coverage.memberId)}</p><p className="mt-1 text-[11px] text-slate-400">Priority {coverage.priority}{coverage.effectiveDate ? ` · effective ${coverage.effectiveDate}` : ""}{coverage.terminationDate ? ` · terminates ${coverage.terminationDate}` : ""}</p></td><td className="px-3 py-5"><p className="font-bold text-slate-700">Copay {money(evidence?.copayCents ?? null)}</p><p className="mt-1 text-[12px] text-slate-500">Deductible {money(evidence?.deductibleCents ?? null)}{evidence?.coinsurancePercent !== null && evidence?.coinsurancePercent !== undefined ? ` · ${evidence.coinsurancePercent}% coinsurance` : ""}</p></td><td className="px-3 py-5"><p className="max-w-52 text-[12px] font-bold text-slate-600">{evidence?.source ?? "No verification evidence"}</p>{evidence?.notes ? <p className="mt-1 max-w-52 truncate text-[11px] text-slate-400" title={evidence.notes}>{evidence.notes}</p> : null}</td><td className="px-3 py-5 text-[12px] text-slate-500">{evidence?.verifiedAt ? new Date(evidence.verifiedAt).toLocaleString() : "Not checked"}</td><td className="px-5 py-5"><StatusBadge status={evidence?.eligibilityStatus ?? "Needs review"} /></td></tr>;
    ...
    <div className="grid gap-6 xl:grid-cols-[1.1fr_.9fr]">
      <SectionCard title="Prior authorization ledger" description="Stored authorization records only. Submission to a payer is not inferred from a draft record.">
        <div className="divide-y divide-slate-100">{workspace.priorAuthorizations.map((authorization) => <div className="grid gap-4 p-5 md:grid-cols-[1fr_.9fr_.65fr_auto] md:items-center" key={authorization.id}><div><Link className="text-xs font-extrabold text-slate-900 hover:text-sky-700" href={`/patients/${authorization.patientId}`}>{authorization.patientName}</Link><p className="mt-1 text-[12px] text-slate-400">{authorization.patientMrn}</p></div><div><p className="text-[11px] font-bold uppercase text-slate-400">{authorization.payer}</p><p className="mt-1 text-xs font-bold text-slate-700">{authorization.service}</p><p className="mt-1 text-[11px] text-slate-400">{authorization.referenceNumber ? `Ref ${authorization.referenceNumber}` : "No external reference recorded"}</p></div><div><p className="text-[11px] font-bold text-slate-400">SUBMITTED</p><p className="mt-1 text-[12px] text-slate-600">{authorization.submittedAt ? new Date(authorization.submittedAt).toLocaleDateString() : "Not recorded"}</p></div><StatusBadge status={authorization.status} /></div>)}{workspace.priorAuthorizations.length === 0 && <p className="p-5 text-xs text-slate-500">No prior-authorization records are stored.</p>}</div>
      </SectionCard>
