...
export function ContractorEnrollmentForm() {
  ...
  return <div className="space-y-5">
    <div className="rounded-[24px] border border-slate-200 bg-white p-4 shadow-sm sm:p-5"><div className="flex items-center justify-between gap-3">{progress.map((label,index)=>{const n=index+1; const active=step===n; const done=step>n; return <div className="flex flex-1 items-center gap-2" key={label}><span className={`grid size-7 place-items-center rounded-full text-[10px] font-black ${active ? "bg-teal-700 text-white" : done ? "bg-teal-50 text-teal-700" : "bg-slate-100 text-slate-400"}`}>{n}</span><span className={`hidden text-[10px] font-extrabold uppercase tracking-[.12em] sm:inline ${active ? "text-slate-900" : "text-slate-400"}`}>{label}</span>{index < progress.length-1 && <span className="h-px flex-1 bg-slate-200"/>}</div>;})}</div></div>

    {step === 1 && <section className="rounded-[26px] border border-slate-200 bg-white p-5 shadow-[0_20px_60px_rgba(15,23,42,.07)] sm:p-7"><p className="text-[10px] font-extrabold uppercase tracking-[.18em] text-teal-700">Start with the basics</p><h2 className="mt-2 text-2xl font-black tracking-[-.045em] text-slate-950">Who are you and what kind of work do you want?</h2><p className="mt-2 text-xs leading-6 text-slate-500">You can finish credentials and exact availability after this. Grid only needs enough to route you correctly first.</p><div className="mt-6 grid gap-4 sm:grid-cols-2"><label className={labelClass}>Full legal name<Input value={fullName} onChange={(e)=>setFullName(e.target.value)}/></label><label className={labelClass}>Email<Input type="email" value={email} onChange={(e)=>setEmail(e.target.value)}/></label><label className={labelClass}>Phone<Input value={phone} onChange={(e)=>setPhone(e.target.value)}/></label><label className={labelClass}>Organization code<Input value={organizationSlug} onChange={(e)=>setOrganizationSlug(e.target.value)}/></label><label className={labelClass}>Provider class<select className={fieldClass} value={providerType} onChange={(e)=>setProviderType(e.target.value as typeof providerType)}>{gridContractorProviderTypes.map((type)=><option key={type}>{type}</option>)}</select></label><label className={labelClass}>Credential<select className={fieldClass} value={credential} onChange={(e)=>setCredential(e.target.value as typeof credential)}><option>RN</option><option>NP</option><option>PA</option></select></label><label className={labelClass}>Experience<select className={fieldClass} value={experienceLevel} onChange={(e)=>setExperienceLevel(e.target.value as typeof experienceLevel)}>{gridExperienceLevels.map((level)=><option key={level}>{level}</option>)}</select></label><label className={labelClass}>Services you want to offer<Input value={services} onChange={(e)=>setServices(e.target.value)}/></label><label className={`${labelClass} sm:col-span-2`}>Professional summary<textarea className={`${fieldClass} min-h-24 py-3 normal-case tracking-normal`} value={bio} onChange={(e)=>setBio(e.target.value)}/></label><label className={`${labelClass} sm:col-span-2`}>Password<Input type="password" value={password} onChange={(e)=>setPassword(e.target.value)}/><span className="block normal-case tracking-normal text-slate-400">12+ characters with uppercase, lowercase, number, and symbol.</span></label></div><Button className="mt-6 w-full" variant="primary" onClick={()=>setStep(2)}>Continue to credentials <ArrowRight className="size-4"/></Button></section>}

    {step === 2 && <section className="rounded-[26px] border border-slate-200 bg-white p-5 shadow-[0_20px_60px_rgba(15,23,42,.07)] sm:p-7"><p className="text-[10px] font-extrabold uppercase tracking-[.18em] text-teal-700">Credentials & coverage</p><h2 className="mt-2 text-2xl font-black tracking-[-.045em] text-slate-950">Only the evidence required for regulated work.</h2><div className="mt-6 grid gap-4 sm:grid-cols-2"><label className={labelClass}>License number<Input value={licenseNumber} onChange={(e)=>setLicenseNumber(e.target.value)}/></label><label className={labelClass}>License state<Input maxLength={2} value={licenseState} onChange={(e)=>setLicenseState(e.target.value.toUpperCase())}/></label><label className={labelClass}>License expiration<Input type="date" value={licenseExpiration} onChange={(e)=>setLicenseExpiration(e.target.value)}/></label><label className={labelClass}>License evidence reference<Input placeholder="Synthetic file or review reference" value={licenseEvidenceReference} onChange={(e)=>setLicenseEvidenceReference(e.target.value)}/></label><label className={labelClass}>Malpractice carrier<Input value={malpracticeCarrier} onChange={(e)=>setMalpracticeCarrier(e.target.value)}/></label><label className={labelClass}>Policy number<Input value={malpracticePolicyNumber} onChange={(e)=>setMalpracticePolicyNumber(e.target.value)}/></label><label className={labelClass}>Policy expiration<Input type="date" value={malpracticeExpiration} onChange={(e)=>setMalpracticeExpiration(e.target.value)}/></label><label className={labelClass}>Coverage amount · USD<Input min={1} type="number" value={malpracticeCoverage} onChange={(e)=>setMalpracticeCoverage(Number(e.target.value))}/></label><label className={`${labelClass} sm:col-span-2`}>Malpractice evidence reference<Input placeholder="Synthetic file or review reference" value={malpracticeEvidenceReference} onChange={(e)=>setMalpracticeEvidenceReference(e.target.value)}/></label><label className={`${labelClass} sm:col-span-2`}>Certifications<Input value={certifications} onChange={(e)=>setCertifications(e.target.value)}/></label></div><p className="mt-4 rounded-xl bg-amber-50 p-3 text-[10px] leading-5 text-amber-900"><ShieldCheck className="mr-1.5 inline size-3.5"/>Uploading or referencing evidence does not verify it. Review state remains explicit.</p><div className="mt-6 flex gap-3"><Button className="flex-1" variant="secondary" onClick={()=>setStep(1)}><ArrowLeft className="size-4"/>Back</Button><Button className="flex-1" variant="primary" onClick={()=>setStep(3)}>Set availability <ArrowRight className="size-4"/></Button></div></section>}

    {step === 3 && <section className="rounded-[26px] border border-slate-200 bg-white p-5 shadow-[0_20px_60px_rgba(15,23,42,.07)] sm:p-7"><p className="text-[10px] font-extrabold uppercase tracking-[.18em] text-teal-700">Availability & preferences</p><h2 className="mt-2 text-2xl font-black tracking-[-.045em] text-slate-950">Tell Grid when and where you want opportunities.</h2><div className="mt-6 grid gap-4 sm:grid-cols-2"><label className={labelClass}>Service area<Input value={serviceArea} onChange={(e)=>setServiceArea(e.target.value)}/></label><label className={labelClass}>Travel radius · miles<Input min={0} type="number" value={travelRadiusMiles} onChange={(e)=>setTravelRadiusMiles(Number(e.target.value))}/></label></div><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">{[["Mobile", mobile, setMobile],["Clinic chair",chair,setChair],["Partner location",partnerLocation,setPartnerLocation],["At-home",atHome,setAtHome],["Available now",onCallNow,setOnCallNow]].map(([label,checked,setter])=><label className="flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 py-3 text-xs font-bold text-slate-700" key={String(label)}><input type="checkbox" checked={Boolean(checked)} onChange={(e)=>(setter as (value:boolean)=>void)(e.target.checked)}/>{String(label)}</label>)}</div><div className="mt-5 space-y-3">{availability.map((slot,index)=><div className="grid gap-3 rounded-2xl border border-slate-200 p-4 lg:grid-cols-[1fr_.7fr_.7fr_1.2fr_auto]" key={`${slot.dayOfWeek}-${index}`}><select aria-label="Availability day" className={fieldClass} value={slot.dayOfWeek} onChange={(e)=>updateSlot(index,{dayOfWeek:Number(e.target.value)})}>{weekdays.map((day,dayIndex)=><option key={day} value={dayIndex}>{day}</option>)}</select><Input aria-label="Availability start" type="time" value={slot.startTime} onChange={(e)=>updateSlot(index,{startTime:e.target.value})}/><Input aria-label="Availability end" type="time" value={slot.endTime} onChange={(e)=>updateSlot(index,{endTime:e.target.value})}/><select aria-label="Availability work setting" className={fieldClass} value={slot.locationType} onChange={(e)=>updateSlot(index,{locationType:e.target.value as Availability["locationType"]})}>{gridLocationTypes.filter((type)=>!["virtual"].includes(type)).map((type)=><option key={type} value={type}>{type.replaceAll("_"," ")}</option>)}</select><Button aria-label="Remove availability" disabled={availability.length===1} size="icon" variant="ghost" onClick={()=>setAvailability((current)=>current.filter((_,slotIndex)=>slotIndex!==index))}><Trash2 className="size-4"/></Button></div>)}</div><Button className="mt-3" size="sm" variant="secondary" onClick={()=>setAvailability((current)=>[...current,defaultAvailability])}><Plus className="size-3.5"/>Add availability</Button><label className="mt-5 flex items-start gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4 text-xs leading-6 text-slate-600"><input className="mt-1" type="checkbox" checked={accepted} onChange={(e)=>setAccepted(e.target.checked)}/><span>I am applying as an independent contractor, understand approval and work are not guaranteed, and will use synthetic demonstration information only in this environment.</span></label>{error && <p className="mt-4 rounded-xl bg-rose-50 px-4 py-3 text-xs font-bold text-rose-700" role="alert">{error}</p>}<div className="mt-6 flex gap-3"><Button className="flex-1" variant="secondary" onClick={()=>setStep(2)}><ArrowLeft className="size-4"/>Back</Button><Button className="flex-1" disabled={busy || !accepted || !(mobile || chair || partnerLocation || atHome)} variant="primary" onClick={submit}>{busy ? <LoaderCircle className="size-4 animate-spin"/> : <CalendarClock className="size-4"/>}Submit for review</Button></div></section>}
  </div>;
}
