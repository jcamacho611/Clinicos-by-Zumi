  ...
  return (
    <div className="overflow-hidden rounded-[24px] border border-slate-200 bg-white shadow-[0_22px_60px_rgba(15,23,42,.08)]">
      <div className="border-b border-slate-100 bg-slate-950 p-5 text-white sm:p-6">
        <div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-xl bg-teal-400/15 text-teal-300 ring-1 ring-inset ring-teal-300/20"><Sparkles className="size-5" /></span><div><p className="text-sm font-extrabold">Safe routing simulator</p><p className="mt-0.5 text-[12px] text-slate-400">Try an incoming patient or caller message</p></div><Badge className="ml-auto bg-white/10 text-white ring-white/15">Demo only</Badge></div>
        <div className="mt-5 rounded-2xl bg-white/7 p-3 ring-1 ring-inset ring-white/10">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            <div className="flex flex-1 flex-wrap gap-1.5">{examples.slice(0, 3).map((example, index) => <button className="rounded-full bg-white/8 px-2.5 py-1 text-[11px] font-bold text-slate-300 transition hover:bg-white/15" key={example} onClick={() => setInput(example)}>Example {index + 1}</button>)}</div>
          ...
          <p className="mt-2 text-[11px] leading-4 text-slate-400">Push to talk, review the visible transcript above, then choose Analyze. Browser demo only; do not use real patient information.</p>
        </div>
      </div>
      ...
      <div className="min-h-[286px] p-5 sm:p-6">
        ...
        {phase && <div className="grid min-h-[230px] place-items-center text-center"><div><LoaderCircle className="mx-auto size-8 animate-spin text-teal-600" /><p className="mt-4 text-sm font-extrabold text-slate-900">{phase}</p><div className="mx-auto mt-4 h-1.5 w-48 overflow-hidden rounded-full bg-slate-100"><div className="h-full w-2/3 animate-pulse rounded-full bg-teal-500" /></div><p className="mt-3 text-[12px] text-slate-400">No message has been sent.</p></div></div>}
        {decision && (
          <div>
            <div className="flex flex-wrap items-start gap-3"><div className="grid size-10 place-items-center rounded-xl bg-teal-50 text-teal-700"><Check className="size-5" /></div><div><p className="text-[12px] font-extrabold uppercase tracking-[.16em] text-slate-400">Classification</p><p className="mt-1 text-lg font-extrabold tracking-[-.03em] text-slate-950">{decision.category}</p></div><Badge className="ml-auto" tone={decision.riskLevel === "Urgent" ? "rose" : decision.riskLevel === "Needs Provider" ? "amber" : "sky"}>{decision.riskLevel}</Badge></div>
            <div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-xl bg-slate-50 p-3"><p className="text-[11px] font-bold text-slate-400">ROUTE</p><p className="mt-1 text-xs font-extrabold text-slate-900">{decision.assignedTeam}</p></div><div className="rounded-xl bg-slate-50 p-3"><p className="text-[11px] font-bold text-slate-400">HUMAN REVIEW</p><p className="mt-1 text-xs font-extrabold text-slate-900">{decision.requiresHumanReview ? "Required" : "Not required"}</p></div><div className="rounded-xl bg-slate-50 p-3"><p className="text-[11px] font-bold text-slate-400">AUTO-SEND</p><p className="mt-1 text-xs font-extrabold text-slate-900">{decision.blockedFromAutoSend ? "Blocked" : "Allowed for office info"}</p></div></div>
            <div className="mt-4 rounded-xl border border-slate-200 p-3"><p className="text-[11px] font-bold text-slate-400">NEXT ACTION</p><p className="mt-1 text-xs font-semibold text-slate-700">{decision.action}</p></div>
            ...
            {decision.safetyMessage && <p className="mt-3 text-[12px] leading-5 text-slate-500">{decision.safetyMessage}</p>}
          </div>
        )}
      </div>
    </div>
