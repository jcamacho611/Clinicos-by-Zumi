...
export function LuxeConsultationForm({ services }: { services: PublicLuxeServiceOption[] }) {
    ...
    <form className="rounded-[28px] border border-white/12 bg-white/[.045] p-5 shadow-2xl shadow-black/30 backdrop-blur-xl sm:p-8" onSubmit={submit}>
      <div className="grid gap-4 sm:grid-cols-2">
        <label className="text-xs font-bold text-slate-300">
          Name
          <input autoComplete="name" className="mt-2 h-12 w-full rounded-xl border border-white/12 bg-black/30 px-4 text-sm text-white outline-none placeholder:text-slate-600 focus:border-rose-300/50" maxLength={160} name="name" required />
        </label>
        <label className="text-xs font-bold text-slate-300">
          Phone
          <input autoComplete="tel" className="mt-2 h-12 w-full rounded-xl border border-white/12 bg-black/30 px-4 text-sm text-white outline-none placeholder:text-slate-600 focus:border-rose-300/50" inputMode="tel" maxLength={40} name="phone" placeholder="(555) 555-5555" />
        </label>
        <label className="text-xs font-bold text-slate-300 sm:col-span-2">
          Email
          <input autoComplete="email" className="mt-2 h-12 w-full rounded-xl border border-white/12 bg-black/30 px-4 text-sm text-white outline-none placeholder:text-slate-600 focus:border-rose-300/50" maxLength={254} name="email" type="email" />
        </label>
        <label className="text-xs font-bold text-slate-300 sm:col-span-2">
          What are you interested in?
          {groupedServices.length ? (
            <select className="mt-2 h-12 w-full rounded-xl border border-white/12 bg-[#160d12] px-4 text-sm text-white outline-none focus:border-rose-300/50" defaultValue={initialService} key={initialService || "service"} name="serviceInterest">
              <option value="">I’m not sure yet</option>
              {groupedServices.map(([category, items]) => (
                <optgroup key={category} label={category}>
                  {items.map((service) => <option key={service.name} value={service.name}>{service.name}</option>)}
                </optgroup>
              ))}
            </select>
          ) : (
        ...
        <label className="text-xs font-bold text-slate-300">
          ...
          <select className="mt-2 h-12 w-full rounded-xl border border-white/12 bg-[#160d12] px-4 text-sm text-white outline-none focus:border-rose-300/50" defaultValue="either" name="preferredContactMethod">
            <option value="either">Phone or text</option>
        ...
        <label className="text-xs font-bold text-slate-300">
          Preferred timing
          <input className="mt-2 h-12 w-full rounded-xl border border-white/12 bg-black/30 px-4 text-sm text-white outline-none placeholder:text-slate-600 focus:border-rose-300/50" maxLength={160} name="preferredTiming" placeholder="Example: weekday afternoon" />
        </label>
        <label className="text-xs font-bold text-slate-300 sm:col-span-2">
          What would you like to know?
          <textarea className="mt-2 min-h-28 w-full rounded-xl border border-white/12 bg-black/30 p-4 text-sm text-white outline-none placeholder:text-slate-600 focus:border-rose-300/50" maxLength={1200} name="message" placeholder="Pricing, availability, consultation questions, or the service you’re considering. Please do not send medical records or sensitive health details here." />
        </label>
      </div>
      ...
      <label className="mt-5 flex items-start gap-3 text-xs leading-5 text-slate-400">
        <input className="mt-1 size-4 accent-rose-300" name="contactConsent" required type="checkbox" />
        <span>I agree that Luxe Medi may contact me about this request using the contact information I provided. This is not consent to treatment.</span>
      </label>
      <label className="mt-3 flex items-start gap-3 text-xs leading-5 text-slate-500">
        <input className="mt-1 size-4 accent-rose-300" name="marketingConsent" type="checkbox" />
        <span>Optional: I would also like occasional Luxe Medi updates and offers. I can opt out later.</span>
      </label>
      ...
      <button className="mt-6 inline-flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-[#f1d4dc] px-5 text-sm font-extrabold text-[#1a0d12] transition hover:bg-white disabled:cursor-not-allowed disabled:opacity-60" disabled={busy} type="submit">
        {busy ? <LoaderCircle className="size-4 animate-spin" /> : <ArrowRight className="size-4" />}
        {busy ? "Sending…" : "Request consultation"}
      </button>

      <div className="mt-5 flex items-start gap-2 text-[11px] leading-5 text-slate-500">
        <ShieldCheck className="mt-0.5 size-3.5 shrink-0" />
        <p>This form is for general service inquiries. Do not upload or enter medical records, diagnoses, medication lists, insurance cards, IDs, or other sensitive clinical information.</p>
      </div>
    </form>
  );
}
