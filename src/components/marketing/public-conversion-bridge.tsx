    ...
    <section className="relative overflow-hidden border-t border-[#e6817b]/10 bg-[#080304] text-[#f8efed]">
      ...
      <div className="relative mx-auto max-w-[1500px] px-5 py-16 sm:px-8 lg:py-24">
        <div className="grid gap-8 lg:grid-cols-[.8fr_1.2fr] lg:items-end">
          <div><p className="text-[10px] font-extrabold uppercase tracking-[.23em] text-[#e6817b]">One ecosystem, different entry points</p><h2 className="mt-4 text-4xl font-light leading-tight tracking-[-.055em] sm:text-6xl">You should not have to understand Klinikos before Klinikos can understand what you need.</h2></div>
          <div className="lg:pl-12"><p className="max-w-2xl text-sm leading-7 text-[#9f8985]">Start with the outcome. Experience the route. See the real unlock. Pay only when a paid capability solves the next concrete problem. No fake scarcity, no hidden implementation vocabulary, and no decorative demo that never connects to a real working surface.</p><div className="mt-6 flex flex-wrap gap-3"><Link className="inline-flex min-h-11 items-center gap-2 rounded-full bg-[#e6817b] px-5 text-xs font-semibold text-[#19090b] hover:bg-[#efaaa1]" href="/ecosystem"><Sparkles className="size-4" />See how it connects</Link><Link className="inline-flex min-h-11 items-center gap-2 rounded-full border border-[#e6817b]/16 px-5 text-xs font-semibold text-[#d8c1bd] hover:border-[#e6817b]/30 hover:text-[#fff8f6]" href="/pricing">View current pricing <ArrowRight className="size-3.5" /></Link></div></div>
        </div>

        <div className="mt-12 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {entries.map(({ eyebrow, title, body, action, href, icon: Icon }) => <Link className="group flex min-h-[300px] flex-col rounded-[22px] border border-[#e6817b]/11 bg-[#100708]/72 p-6 transition hover:border-[#e6817b]/25 hover:bg-[#14090b]" href={href} key={eyebrow}><div className="flex items-center justify-between"><Icon className="size-5 text-[#e6817b]" /><ArrowRight className="size-4 text-[#655653] transition group-hover:translate-x-1 group-hover:text-[#efaaa1]" /></div><p className="mt-8 text-[9px] font-extrabold uppercase tracking-[.17em] text-[#806965]">{eyebrow}</p><h3 className="mt-3 text-xl font-semibold leading-snug tracking-[-.035em] text-[#fff8f6]">{title}</h3><p className="mt-4 flex-1 text-xs leading-6 text-[#9f8985]">{body}</p><span className="mt-5 text-xs font-semibold text-[#efaaa1]">{action}</span></Link>)}
        </div>
      </div>
    </section>
  );
