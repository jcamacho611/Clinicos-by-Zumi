...
export function SalesIntakeForm({ defaultOffer = "private_workflow_demo" }: { defaultOffer?: DemoOfferKey }) {
  ...
  const [submission, setSubmission] = useState<null | { reservationId: string; scenarioTitle: string; offerName: string }>(null);
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      try {
        ...
        if (!response.ok) throw new Error(payload.error || "The demo request could not be saved.");
        setSubmission({
        ...
        setError(caught instanceof Error ? caught.message : "The demo request could not be saved.");
      }
    });
  }
  ...
  if (submission) {
    return (
      <section aria-live="polite" className="relative overflow-hidden rounded-[32px] border border-emerald-300/20 bg-[linear-gradient(145deg,rgba(16,185,129,.12),rgba(255,255,255,.035))] p-7 shadow-[0_40px_120px_rgba(0,0,0,.4)] sm:p-10">
        ...
        <p className="mt-7 text-[10px] font-black uppercase tracking-[.2em] text-emerald-300">Inquiry recorded</p>
        <h2 className="mt-3 text-3xl font-black tracking-[-.05em] sm:text-4xl">Your workflow room is queued.</h2>
        <p className="mt-4 max-w-2xl text-sm leading-7 text-slate-300">A human will qualify the request before scheduling or payment. No processor settlement occurred, and no patient information was collected.</p>
        <div className="mt-8 grid gap-3 sm:grid-cols-3">
          ...
          <div className="rounded-2xl border border-white/10 bg-black/20 p-4"><p className="text-[9px] font-black uppercase tracking-[.14em] text-slate-500">Offer</p><p className="mt-2 text-xs font-bold text-white">{submission.offerName}</p></div>
          <div className="rounded-2xl border border-white/10 bg-black/20 p-4"><p className="text-[9px] font-black uppercase tracking-[.14em] text-slate-500">Payment</p><p className="mt-2 text-xs font-bold text-amber-200">Manual link pending</p></div>
        </div>
        <div className="mt-5 rounded-2xl border border-cyan-300/15 bg-cyan-300/[.06] p-5"><StatusPill status="Demo" /><p className="mt-3 text-sm font-bold">{submission.scenarioTitle}</p><p className="mt-1 text-xs leading-5 text-slate-400">Synthetic preview prepared for human review.</p></div>
        <div className="mt-8 flex flex-wrap gap-3"><Link className="rounded-full bg-white px-5 py-3 text-xs font-black text-slate-950" href="/demo">Explore the demo system</Link><Link className="rounded-full border border-white/15 px-5 py-3 text-xs font-black text-white" href="/founding-clinic">Review founding options</Link></div>
      </section>
    );
  }
    ...
    <form className="grid gap-6 xl:grid-cols-[1.08fr_.92fr]" onSubmit={submit}>
      <div className="overflow-hidden rounded-[32px] border border-white/10 bg-[#0a0e14]/90 shadow-[0_40px_120px_rgba(0,0,0,.42)]">
        <div className="border-b border-white/[.08] px-6 py-6 sm:px-8">
          <div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[.2em] text-emerald-300">Clinic signal intake</p><h2 className="mt-2 text-2xl font-black tracking-[-.04em]">Build the right demo, not a generic tour.</h2></div><StatusPill status="Live" /></div>
        </div>
        ...
        <div className="space-y-9 p-6 sm:p-8">
          ...
          <section>
            <p className="mb-4 text-[10px] font-black uppercase tracking-[.18em] text-slate-500">04 / Select the next step</p>
          ...
          <button className="group flex h-14 w-full items-center justify-center gap-3 rounded-2xl bg-white text-sm font-black text-slate-950 transition hover:bg-emerald-200 disabled:cursor-not-allowed disabled:opacity-60" disabled={pending || !form.acknowledgesSyntheticData} type="submit">{pending ? <><LoaderCircle className="size-4 animate-spin" /> Creating synthetic workflow room</> : <>Request {selectedOffer.name}<ArrowRight className="size-4 transition group-hover:translate-x-1" /></>}</button>
          <p className="text-center text-[10px] leading-5 text-slate-600">Submitting creates an inquiry and synthetic scenario. It does not charge a card, confirm a booking, or activate a production clinic workspace.</p>
        </div>
      </div>
      ...
      <aside className="xl:sticky xl:top-6 xl:self-start">
        <div className="overflow-hidden rounded-[32px] border border-white/10 bg-[linear-gradient(155deg,rgba(15,23,42,.94),rgba(6,10,15,.98))] shadow-[0_40px_120px_rgba(0,0,0,.45)]">
          ...
          <div className="border-t border-white/[.08] bg-emerald-300/[.045] p-6 sm:p-8">
            <div className="flex items-start gap-3"><Sparkles className="mt-0.5 size-4 shrink-0 text-emerald-300" /><div><p className="text-xs font-black">Recommended Clinicos workflow</p><p className="mt-2 text-[11px] leading-5 text-slate-400">{scenario.recommendedWorkflow.steps.join(" -> ")}</p></div></div>
