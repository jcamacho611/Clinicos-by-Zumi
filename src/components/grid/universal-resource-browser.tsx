  ...
  return <section className="border-t border-[#e8ded9] bg-[#f2ece8]">
    <div className="mx-auto max-w-[1500px] px-5 py-12 sm:px-8">
      <div className="flex flex-wrap items-end justify-between gap-5">
        <div>
          <p className="text-[10px] font-extrabold uppercase tracking-[.2em] text-[#a8474e]">Reviewed Grid resources</p>
        ...
        <div className="rounded-full border border-[#d7c7c1] bg-[#fffdf9] px-4 py-2 text-[10px] font-extrabold uppercase tracking-[.12em] text-[#756461]">{visible.length} published</div>
      </div>
      ...
      {visible.length === 0 ? <div className="mt-8 rounded-[18px] border border-dashed border-[#d7c7c1] bg-[#fffdf9] px-6 py-10">
        ...
        {visible.map((resource) => <article className="rounded-[18px] border border-[#dfd2cc] bg-[#fffdf9] p-5 shadow-[0_14px_40px_rgba(65,31,35,.04)]" key={resource.id}>
          <div className="flex items-start justify-between gap-4">
            <div><p className="text-[9px] font-extrabold uppercase tracking-[.16em] text-[#a8474e]">{label(resource.resourceType)}</p><h3 className="mt-2 text-xl font-semibold tracking-[-.04em] text-[#241517]">{resource.title}</h3></div>
            <span className="inline-flex shrink-0 items-center gap-1 rounded-full border border-emerald-200 bg-emerald-50 px-2.5 py-1 text-[8px] font-extrabold uppercase tracking-[.11em] text-emerald-700"><ShieldCheck className="size-3" />Reviewed</span>
          </div>
          ...
          <div className="mt-4 flex flex-wrap gap-2 text-[8px] font-extrabold uppercase tracking-[.1em] text-[#8a7772]"><span className="rounded-full border border-[#e3d8d3] px-2 py-1">{label(resource.policyClass)}</span>{resource.subtype && <span className="rounded-full border border-[#e3d8d3] px-2 py-1">{resource.subtype}</span>}</div>
          <div className="mt-5 flex items-end justify-between gap-4 border-t border-[#ece3df] pt-4"><span className="text-[9px] font-extrabold uppercase tracking-[.1em] text-[#9b8883]">Terms / rate</span><strong className="text-sm text-[#241517]">{money(resource)}</strong></div>
