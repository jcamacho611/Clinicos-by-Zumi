  ...
  return <section className="border-t border-[#e6e9ee] bg-[#f7f8fa]">
    <div className="mx-auto max-w-[1500px] px-5 py-12 sm:px-8">
      <div className="flex flex-wrap items-end justify-between gap-5">
        <div>
          <p className="text-[10px] font-extrabold uppercase tracking-[.2em] text-[#174ea6]">Reviewed Grid resources</p>
        ...
        <div className="rounded-full border border-[#d9dee5] bg-white px-4 py-2 text-[10px] font-extrabold uppercase tracking-[.12em] text-[#5b6675]">{visible.length} published</div>
      </div>
      ...
      {visible.length === 0 ? <div className="mt-8 border border-dashed border-[#cfd6df] bg-white px-6 py-10">
        ...
        {visible.map((resource) => <article className="border border-[#dfe3e8] bg-white p-5 shadow-[0_14px_40px_rgba(15,23,42,.05)]" key={resource.id}>
          <div className="flex items-start justify-between gap-4">
            <div><p className="text-[9px] font-extrabold uppercase tracking-[.16em] text-[#174ea6]">{label(resource.resourceType)}</p><h3 className="mt-2 text-xl font-black tracking-[-.04em] text-[#0b1220]">{resource.title}</h3></div>
            <span className="inline-flex shrink-0 items-center gap-1 rounded-full border border-emerald-200 bg-emerald-50 px-2.5 py-1 text-[8px] font-black uppercase tracking-[.11em] text-emerald-700"><ShieldCheck className="size-3" />Reviewed</span>
          </div>
