    ...
    <div className="mx-auto max-w-[1500px] px-5 py-12 sm:px-8">
      ...
      {visible.length === 0 ? <div className="mt-8 rounded-[18px] border border-dashed border-[#d7c7c1] bg-[#fffdf9] px-6 py-10">
        ...
        {visible.map((resource) => <article className="rounded-[18px] border border-[#dfd2cc] bg-[#fffdf9] p-5 shadow-[0_14px_40px_rgba(65,31,35,.04)]" key={resource.id}>
          ...
          <div className="mt-4 flex flex-wrap gap-2 text-[11px] font-extrabold uppercase tracking-[.1em] text-[#756461]"><span className="rounded-full border border-[#e3d8d3] px-2 py-1">{label(resource.policyClass)}</span>{resource.subtype && <span className="rounded-full border border-[#e3d8d3] px-2 py-1">{resource.subtype}</span>}</div>
          <div className="mt-5 flex items-end justify-between gap-4 border-t border-[#ece3df] pt-4"><span className="text-[11px] font-extrabold uppercase tracking-[.1em] text-[#756461]">Terms / rate</span><strong className="text-sm text-[#241517]">{money(resource)}</strong></div>
