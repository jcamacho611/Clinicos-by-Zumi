...
export function GridNeedComposer({
  ...
  if (createdId) {
    return <div className="rounded-[1.6rem] border border-emerald-300/20 bg-emerald-300/[.06] p-6">
      ...
      <p className="mt-5 text-[12px] font-black uppercase tracking-[.18em] text-emerald-200">Need saved</p>
  ...
  return <div className="rounded-[1.8rem] border border-white/10 bg-[#070b13] p-5 sm:p-7">
    ...
    <div className="mt-6 grid gap-3 md:grid-cols-2">
      ...
      <label className="text-[12px] font-bold uppercase tracking-[.12em] text-white/45 md:col-span-2">What do you need?<textarea className="mt-2 min-h-24 w-full rounded-xl border border-white/10 bg-black/25 px-3 py-3 text-xs text-white outline-none" value={description} onChange={(event) => setDescription(event.target.value)} placeholder="Describe the resource, capacity, service, or work requirement without patient information." /></label>
      <label className="text-[12px] font-bold uppercase tracking-[.12em] text-white/45">Category<Input className="mt-2" value={category} onChange={(event) => setCategory(event.target.value)} placeholder="RN coverage, treatment room, billing…" /></label>
      <label className="text-[12px] font-bold uppercase tracking-[.12em] text-white/45">Specific service or capability<Input className="mt-2" value={serviceName} onChange={(event) => setServiceName(event.target.value)} placeholder="Optional" /></label>
      <label className="text-[12px] font-bold uppercase tracking-[.12em] text-white/45">City<Input className="mt-2" value={city} onChange={(event) => setCity(event.target.value)} /></label>
      <div className="grid grid-cols-[100px_1fr] gap-2"><label className="text-[12px] font-bold uppercase tracking-[.12em] text-white/45">State<Input className="mt-2" maxLength={2} value={state} onChange={(event) => setState(event.target.value.toUpperCase())} /></label><label className="text-[12px] font-bold uppercase tracking-[.12em] text-white/45">Radius miles<Input className="mt-2" min={0} type="number" value={radiusMiles} onChange={(event) => setRadiusMiles(event.target.value)} /></label></div>
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-white/10 bg-white/[.03] p-4 md:col-span-2"><div><p className="text-xs font-extrabold text-white">Use a real search origin</p><p className="mt-1 text-[12px] leading-5 text-white/40">Optional. Grid asks only after you choose this button. Without it, results use city/state and do not claim exact distance.</p></div><Button disabled={locating} onClick={useCurrentLocation} type="button" variant="secondary">{locating ? <LoaderCircle className="size-4 animate-spin" /> : <Crosshair className="size-4" />}{latitude != null && longitude != null ? "Location captured" : "Use my location"}</Button></div>
      <label className="text-[12px] font-bold uppercase tracking-[.12em] text-white/45">Start<Input className="mt-2" type="datetime-local" value={startsAt} onChange={(event) => setStartsAt(event.target.value)} /></label>
      <label className="text-[12px] font-bold uppercase tracking-[.12em] text-white/45">End<Input className="mt-2" type="datetime-local" value={endsAt} onChange={(event) => setEndsAt(event.target.value)} /></label>
      <label className="text-[12px] font-bold uppercase tracking-[.12em] text-white/45">Quantity / capacity<Input className="mt-2" min={1} type="number" value={quantity} onChange={(event) => setQuantity(event.target.value)} /></label>
      <label className="text-[12px] font-bold uppercase tracking-[.12em] text-white/45">Max budget · USD<Input className="mt-2" inputMode="decimal" value={budget} onChange={(event) => setBudget(event.target.value)} placeholder="Optional" /></label>
      <label className="text-[12px] font-bold uppercase tracking-[.12em] text-white/45">Setting / location type<Input className="mt-2" value={locationType} onChange={(event) => setLocationType(event.target.value)} placeholder="Mobile, clinic, remote…" /></label>
      <label className="text-[12px] font-bold uppercase tracking-[.12em] text-white/45">Requirements<textarea className="mt-2 min-h-20 w-full rounded-xl border border-white/10 bg-black/25 px-3 py-3 text-xs text-white outline-none" value={requirements} onChange={(event) => setRequirements(event.target.value)} placeholder="One per line or comma separated" /></label>
      <label className="flex items-start gap-3 rounded-xl border border-white/10 bg-white/[.03] p-4 text-xs text-white/60 md:col-span-2"><input checked={clinical} className="mt-0.5" onChange={(event) => setClinical(event.target.checked)} type="checkbox" /><span><strong className="block text-white">Clinical / regulated eligibility required</strong><span className="mt-1 block text-[12px] leading-5 text-white/40">Keep this on whenever the opportunity requires professional licensing, credential, jurisdiction, malpractice, or other regulated eligibility.</span></span></label>
    </div>

    {error && <div className="mt-5 rounded-xl border border-rose-300/20 bg-rose-300/[.06] px-4 py-3 text-xs text-rose-100">{error}</div>}
    <div className="mt-6 flex flex-wrap items-center justify-between gap-3"><p className="flex max-w-2xl gap-2 text-[12px] leading-5 text-amber-100/60"><ShieldCheck className="mt-0.5 size-4 shrink-0 text-amber-200" />A saved need starts matching. It does not expose private clinical information or authorize a transaction.</p><Button disabled={busy} onClick={submit}>{busy ? <LoaderCircle className="size-4 animate-spin" /> : null}Save need & search <ArrowRight className="size-4" /></Button></div>
  </div>;
}
