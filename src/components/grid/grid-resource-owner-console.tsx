...
export function GridResourceOwnerConsole({ resources }: { resources: OwnedResource[] }) {
  ...
  return <div className="space-y-6">
    ...
    <section className="rounded-[1.8rem] border border-white/10 bg-[#070b13] p-5 sm:p-6">
      <p className="text-[12px] font-black uppercase tracking-[.2em] text-cyan-200">I have something</p>
      ...
      <p className="mt-3 text-[11px] leading-4 text-white/30">Clinical services belong in professional/provider enrollment. Prescription or otherwise regulated products may be captured for review but remain non-transactional until their dedicated transfer and custody policy exists.</p>
    </section>

    <section className="rounded-[1.8rem] border border-white/10 bg-white/[.03] p-5 sm:p-6"><div className="flex items-center justify-between gap-4"><div><p className="text-[12px] font-black uppercase tracking-[.18em] text-amber-200">My supply</p><h2 className="mt-2 text-2xl font-black text-white">Resources & capacity</h2></div><span className="rounded-full border border-white/10 px-3 py-1.5 text-[11px] font-extrabold text-white/40">{resources.length} resource{resources.length === 1 ? "" : "s"}</span></div><div className="mt-5 grid gap-3 xl:grid-cols-2">{resources.length === 0 && <p className="text-xs text-white/40">No universal resources yet.</p>}{resources.map((resource) => <article className="rounded-2xl border border-white/10 bg-black/20 p-4" key={resource.id}><div className="flex items-start justify-between gap-3"><div><div className="flex flex-wrap gap-2"><Status value={resource.status} /><Status value={resource.reviewStatus} /></div><h3 className="mt-3 text-base font-black text-white">{resource.title}</h3><p className="mt-1 text-[12px] text-white/40">{label(resource.resourceType)} · {label(resource.policyClass)} · {resource.city || resource.state || "Location flexible"}</p></div><p className="text-sm font-black text-cyan-100">{money(resource.priceCents)}</p></div><p className="mt-3 line-clamp-2 text-[12px] leading-5 text-white/45">{resource.description}</p><div className="mt-3 flex items-center justify-between text-[11px] text-white/30"><span>Capacity {resource.capacity}</span><span>{label(resource.visibility)}</span><span>{resource.availability.length} window{resource.availability.length === 1 ? "" : "s"}</span></div>{resource.status === "active" && <button className="mt-4 rounded-lg border border-white/10 px-3 py-2 text-[12px] font-extrabold text-white/60" disabled={busy === resource.id} onClick={() => pauseResource(resource.id)}><Pause className="mr-1 inline size-3" />Pause resource</button>}</article>)}</div></section>
  </div>;
}
