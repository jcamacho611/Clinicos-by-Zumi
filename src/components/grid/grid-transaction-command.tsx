...
function Pill({ value }: { value: string }) {
  ...
  return <span className={`inline-flex rounded-full border px-2.5 py-1 text-[11px] font-extrabold uppercase tracking-[.12em] ${className}`}>{label(value)}</span>;
}

function Metric({ labelText, value, detail }: { labelText: string; value: string; detail: string }) {
  return <div className="rounded-[1.4rem] border border-white/10 bg-white/[.045] p-5"><p className="text-2xl font-black tracking-[-.05em] text-white">{value}</p><p className="mt-1 text-[11px] font-extrabold text-white/75">{labelText}</p><p className="mt-2 text-[11px] leading-4 text-white/35">{detail}</p></div>;
}
...
export function GridTransactionCommand({ board }: { board: GridTransactionBoard }) {
  ...
  /**
   * Offers that already became a reservation.
   *
   * Without this, an accepted-and-already-reserved offer kept showing "Reserve
   * capacity". The API is idempotent and returns the existing reservation, so no
   * double booking was ever possible — but the button reported "Grid recorded the
   * action." in green for an action that changed nothing, which is worse than doing
   * nothing visibly. A control must not claim an effect it did not have.
   */
  const reservedOfferIds = useMemo(
    () => new Set(board.reservations.map((reservation) => reservation.offerId)),
    [board.reservations],
  );
  ...
  return <div className="space-y-6">
    ...
    <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
      ...
      <Metric labelText="Awaiting fulfillment" value={String(board.metrics.awaitingFulfillment)} detail="Reservations not yet at a terminal outcome." />
    ...
    <section className="rounded-[1.7rem] border border-white/10 bg-[#070b13] p-5 sm:p-6">
      <div className="flex flex-wrap items-center justify-between gap-4"><div><p className="text-[12px] font-black uppercase tracking-[.2em] text-cyan-200">I need something</p><h2 className="mt-2 text-2xl font-black tracking-[-.04em] text-white">Needs & matching</h2></div><button className="rounded-xl bg-cyan-300 px-4 py-2.5 text-xs font-extrabold text-slate-950" onClick={() => setComposerOpen((value) => !value)}>{composerOpen ? "Close" : "Create a need"}</button></div>
      ...
      <div className="mt-5 space-y-3">{board.demands.length === 0 && <p className="text-xs text-white/40">No saved needs yet.</p>}{board.demands.map((demand) => <div className="rounded-2xl border border-white/10 bg-white/[.035] p-4" key={demand.id}><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex items-center gap-2"><Pill value={demand.status} /><span className="text-[11px] font-black uppercase tracking-[.14em] text-white/35">{label(demand.kind)}</span></div><h3 className="mt-2 font-black text-white">{demand.title}</h3><p className="mt-1 text-[12px] text-white/45">{demand.category} · {when(demand.requestedStartAt)} · {demand.city || demand.state || "Location flexible"}</p></div><div className="flex gap-2">{["work", "provider", "service"].includes(demand.kind) && ["open", "matched"].includes(demand.status) && <button className="rounded-xl border border-cyan-300/20 px-3 py-2 text-[12px] font-extrabold text-cyan-100" onClick={() => findMatches(demand)}>{busy === `matches-${demand.id}` ? "Matching…" : "Find matches"}</button>}<Link className="rounded-xl border border-white/10 px-3 py-2 text-[12px] font-extrabold text-white/60" href={`/grid/browse?intent=${demand.kind}`}><Map className="mr-1 inline size-3" />Browse</Link></div></div>{matches[demand.id]?.length ? <div className="mt-4 grid gap-2 lg:grid-cols-2">{matches[demand.id].slice(0, 6).map((match) => <div className="rounded-xl border border-white/8 bg-black/20 p-3" key={`${match.providerId}:${match.serviceId}`}><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-extrabold text-white">{match.providerName}</p><p className="mt-1 text-[12px] text-white/45">{match.serviceName} · score {match.score}</p></div><span className="text-xs font-black text-cyan-100">{money(match.priceLowCents)}</span></div><button className="mt-3 text-[12px] font-extrabold text-cyan-200" onClick={() => setOfferDraft({ demandId: demand.id, match, gross: String((match.priceLowCents ?? 0) / 100), deposit: "0", note: `Offer for ${demand.title}.` })}>Prepare offer <ArrowRight className="inline size-3" /></button></div>)}</div> : null}{offerDraft?.demandId === demand.id && <div className="mt-4 grid gap-2 rounded-xl border border-amber-300/15 bg-amber-300/[.04] p-3 sm:grid-cols-3"><input className="rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-xs text-white" value={offerDraft.gross} onChange={(event) => setOfferDraft({ ...offerDraft, gross: event.target.value })} placeholder="Gross $" /><input className="rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-xs text-white" value={offerDraft.deposit} onChange={(event) => setOfferDraft({ ...offerDraft, deposit: event.target.value })} placeholder="Deposit $" /><button className="rounded-lg bg-amber-200 px-3 py-2 text-xs font-extrabold text-slate-950" onClick={() => sendOffer(demand)}>Send offer</button></div>}</div>)}</div>
    </section>

    <section className="grid gap-5 xl:grid-cols-2">
      <div className="rounded-[1.7rem] border border-white/10 bg-white/[.035] p-5"><div className="flex items-center gap-3"><Handshake className="size-5 text-cyan-200" /><h2 className="text-xl font-black text-white">Offers</h2></div><div className="mt-4 space-y-3">{board.offers.length === 0 && <p className="text-xs text-white/40">No offers yet.</p>}{board.offers.map((offer) => {const recipient = offer.recipientOrganizationId === board.organizationId;const sender = offer.senderOrganizationId === board.organizationId;const owner = offer.ownerOrganizationId === board.organizationId;return <div className="rounded-2xl border border-white/10 bg-black/20 p-4" key={offer.id}><div className="flex items-start justify-between gap-3"><div><Pill value={offer.status} /><p className="mt-2 text-sm font-extrabold text-white">{offer.demandTitle}</p><p className="mt-1 text-[12px] text-white/45">{offer.senderName ?? "Grid participant"} → {offer.recipientName ?? "Grid participant"} · v{offer.version}</p></div><p className="text-lg font-black text-white">{money(offer.grossAmountCents)}</p></div><p className="mt-2 text-[12px] text-white/40">Deposit {money(offer.depositAmountCents)} · Location {money(offer.locationPayableCents)} · {when(offer.offeredStartAt)}</p>{offer.status === "sent" && recipient && <div className="mt-3 flex flex-wrap gap-2"><button className="rounded-lg bg-emerald-300 px-3 py-2 text-[12px] font-extrabold text-slate-950" onClick={() => decideOffer(offer, "accepted")}><Check className="mr-1 inline size-3" />Accept</button><button className="rounded-lg border border-rose-300/20 px-3 py-2 text-[12px] font-extrabold text-rose-100" onClick={() => decideOffer(offer, "declined")}><X className="mr-1 inline size-3" />Decline</button><input className="w-28 rounded-lg border border-white/10 bg-black/30 px-2 py-2 text-[12px] text-white" placeholder="Counter $" value={counterAmounts[offer.id] ?? ""} onChange={(event) => setCounterAmounts({ ...counterAmounts, [offer.id]: event.target.value })} /><button className="rounded-lg border border-amber-300/20 px-3 py-2 text-[12px] font-extrabold text-amber-100" onClick={() => counterOffer(offer)}>Counter</button></div>}{offer.status === "sent" && sender && <button className="mt-3 rounded-lg border border-white/10 px-3 py-2 text-[12px] font-extrabold text-white/60" onClick={() => decideOffer(offer, "withdrawn")}>Withdraw</button>}{offer.status === "accepted" && owner && (reservedOfferIds.has(offer.id) ? <p className="mt-3 text-[12px] font-extrabold text-emerald-100/70">Capacity reserved · see Reservations &amp; fulfillment below</p> : <button className="mt-3 rounded-lg bg-cyan-300 px-3 py-2 text-[12px] font-extrabold text-slate-950" disabled={busy === `reserve-${offer.id}`} onClick={() => reserveOffer(offer.id)}>Reserve capacity</button>)}</div>})}</div></div>

      <div className="rounded-[1.7rem] border border-white/10 bg-white/[.035] p-5"><div className="flex items-center gap-3"><CircleDollarSign className="size-5 text-emerald-200" /><h2 className="text-xl font-black text-white">Earnings & obligations</h2></div><div className="mt-4 space-y-3">{board.obligations.length === 0 && <p className="text-xs text-white/40">No financial obligations allocated yet.</p>}{board.obligations.map((line) => {
        /* Direction, not just amount. This panel shows every obligation attached to the
           transaction, and for a buying organization most of them are money going out.
           Rendering what you owe in the same green as what you earn turns a money surface
           into a misleading one, so the direction is stated in words and carried by
           colour rather than left for the reader to infer from the beneficiary name. */
        const toYou = line.beneficiaryType === "organization" && line.beneficiaryReference === board.organizationId;
        return <div className="flex items-center justify-between gap-4 rounded-xl border border-white/10 bg-black/20 p-3" key={line.id}><div><Pill value={line.status} /><p className="mt-2 text-xs font-extrabold text-white">{label(line.obligationType)}</p><p className="mt-1 text-[11px] text-white/35">{toYou ? "To you" : "You owe"} · {line.beneficiaryName ?? (line.beneficiaryType === "platform" ? "Klinikos" : "Organization")}</p></div><p className={`text-base font-black ${toYou ? "text-emerald-100" : "text-white/70"}`}>{toYou ? "" : "−"}{money(line.amountCents)}</p></div>;
      })}</div></div>
    </section>

    <section className="rounded-[1.7rem] border border-white/10 bg-[#070b13] p-5 sm:p-6"><div className="flex items-center gap-3"><BadgeDollarSign className="size-5 text-amber-200" /><h2 className="text-xl font-black text-white">Reservations & fulfillment</h2></div><div className="mt-4 grid gap-3 xl:grid-cols-2">{board.reservations.length === 0 && <p className="text-xs text-white/40">No reservations yet.</p>}{board.reservations.map((reservation) => {const owner = reservation.ownerOrganizationId === board.organizationId;const lines = obligationsByReservation.get(reservation.id) ?? [];return <div className="rounded-2xl border border-white/10 bg-white/[.035] p-4" key={reservation.id}><div className="flex items-start justify-between gap-3"><div><Pill value={reservation.status} /><span className="ml-2"><Pill value={reservation.fulfillmentStatus} /></span><p className="mt-2 text-sm font-extrabold text-white">{reservation.demandTitle}</p></div><p className="font-black text-white">{money(reservation.grossAmountCents)}</p></div><p className="mt-2 text-[12px] text-white/40">{when(reservation.reservedStartAt)} · payment {label(reservation.paymentStatus)}</p>{owner && reservation.paymentStatus === "manual_link_required" && <div className="mt-3 rounded-xl border border-amber-300/15 bg-amber-300/[.04] p-3"><a className="text-[12px] font-extrabold text-amber-100" href={KLINIKOS_GODADDY_PAYLINK} rel="noreferrer" target="_blank">Open GoDaddy checkout <ArrowRight className="inline size-3" /></a><div className="mt-2 flex gap-2"><input className="min-w-0 flex-1 rounded-lg border border-white/10 bg-black/30 px-2 py-2 text-[12px] text-white" placeholder="Real payment/reference" value={paymentRefs[reservation.id] ?? ""} onChange={(event) => setPaymentRefs({ ...paymentRefs, [reservation.id]: event.target.value })} /><button className="rounded-lg bg-amber-200 px-3 py-2 text-[12px] font-extrabold text-slate-950" onClick={() => recordPayment(reservation.id)}>Record evidence</button></div></div>}<div className="mt-3 flex flex-wrap gap-2">{reservation.status === "held" && reservation.fulfillmentStatus === "not_started" && <button className="rounded-lg bg-cyan-300 px-3 py-2 text-[12px] font-extrabold text-slate-950" onClick={() => fulfill(reservation.id, "checked_in")}>Check in</button>}{reservation.fulfillmentStatus === "checked_in" && <button className="rounded-lg bg-cyan-300 px-3 py-2 text-[12px] font-extrabold text-slate-950" onClick={() => fulfill(reservation.id, "in_progress")}>Start work</button>}{reservation.fulfillmentStatus === "in_progress" && <><button className="rounded-lg bg-emerald-300 px-3 py-2 text-[12px] font-extrabold text-slate-950" onClick={() => fulfill(reservation.id, "fulfilled")}>Fulfilled</button><button className="rounded-lg border border-amber-300/20 px-3 py-2 text-[12px] font-extrabold text-amber-100" onClick={() => fulfill(reservation.id, "partial")}>Partial</button><button className="rounded-lg border border-rose-300/20 px-3 py-2 text-[12px] font-extrabold text-rose-100" onClick={() => fulfill(reservation.id, "failed")}>Failed</button></>}{["checked_in", "in_progress", "fulfilled"].includes(reservation.fulfillmentStatus) && <button className="rounded-lg border border-rose-300/20 px-3 py-2 text-[12px] font-extrabold text-rose-100" onClick={() => fulfill(reservation.id, "disputed")}>Dispute</button>}{owner && reservation.fulfillmentStatus === "fulfilled" && lines.length === 0 && <button className="rounded-lg border border-emerald-300/20 px-3 py-2 text-[12px] font-extrabold text-emerald-100" onClick={() => allocate(reservation.id)}>Allocate obligations</button>}</div></div>})}</div></section>

    <div className="rounded-[1.4rem] border border-white/10 bg-white/[.035] p-4 text-[12px] leading-5 text-white/45"><Sparkles className="mr-2 inline size-4 text-cyan-200" />Current transaction command is synthetic/demo gated. Generic product, equipment, space, education, and referral demand remain discoverable through Grid, but regulated acceptance/reservation only advances when that resource class has a real policy verifier.</div>
  </div>;
}
