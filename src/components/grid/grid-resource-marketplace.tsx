...
export function GridResourceMarketplace({ resources }: { resources: PublicResource[] }) {
  ...
  return <div className="space-y-5">
    <div className="grid gap-3 rounded-[1.5rem] border border-[#dfe3e8] bg-white p-4 sm:grid-cols-[1fr_220px]">
      <label className="flex items-center gap-2 rounded-xl border border-[#dfe3e8] bg-[#f8fafc] px-3"><Search className="size-4 text-[#4b5768]" /><input className="min-h-12 w-full bg-transparent text-sm outline-none" placeholder="Search rooms, equipment, services, training, organizations…" value={query} onChange={(event) => setQuery(event.target.value)} /></label>
    ...
    <div className="flex items-center justify-between gap-4"><p className="text-[12px] font-extrabold uppercase tracking-[.16em] text-[#4b5768]">{filtered.length} approved resource{filtered.length === 1 ? "" : "s"}</p><p className="text-[12px] text-[#526174]">Only public, reviewed inventory is shown here.</p></div>

    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{filtered.map((resource) => {const Icon = icons[resource.resourceType] ?? Boxes;return <article className="group flex min-h-[310px] flex-col rounded-[1.5rem] border border-[#dfe3e8] bg-white p-5 shadow-[0_18px_50px_rgba(15,23,42,.04)] transition hover:-translate-y-0.5 hover:border-[#b8c7dc]" key={resource.id}><div className="flex items-start justify-between gap-3"><span className="grid size-10 place-items-center rounded-xl bg-[#eef5ff] text-[#174ea6]"><Icon className="size-5" /></span><span className="rounded-full border border-[#dfe3e8] px-2.5 py-1 text-[11px] font-extrabold uppercase tracking-[.12em] text-[#4b5768]">{label(resource.resourceType)}</span></div><h2 className="mt-5 text-xl font-black tracking-[-.04em] text-[#0b1220]">{resource.title}</h2><p className="mt-2 line-clamp-3 text-[12px] leading-6 text-[#4b5768]">{resource.description}</p><div className="mt-5 grid grid-cols-2 gap-3 text-[12px]"><div><p className="font-extrabold uppercase tracking-[.12em] text-[#526174]">Where</p><p className="mt-1 font-bold text-[#334155]">{resource.city || resource.state ? [resource.city, resource.state].filter(Boolean).join(", ") : "Location flexible"}</p></div><div><p className="font-extrabold uppercase tracking-[.12em] text-[#526174]">Capacity</p><p className="mt-1 font-bold text-[#334155]">{resource.capacity}</p></div><div><p className="font-extrabold uppercase tracking-[.12em] text-[#526174]">Price</p><p className="mt-1 font-bold text-[#334155]">{money(resource)}</p></div><div><p className="font-extrabold uppercase tracking-[.12em] text-[#526174]">Next window</p><p className="mt-1 font-bold text-[#334155]">{firstWindow(resource)}</p></div></div><Link className="mt-auto flex items-center justify-between gap-3 border-t border-[#eef1f4] pt-4 text-xs font-extrabold text-[#174ea6]" href={`/grid/resources/request/${resource.id}`}>Request this resource <ArrowRight className="size-4 transition group-hover:translate-x-1" /></Link></article>})}</div>

    {filtered.length === 0 && <div className="rounded-[1.5rem] border border-dashed border-[#cbd5e1] bg-white px-6 py-16 text-center"><Boxes className="mx-auto size-8 text-[#526174]" /><h2 className="mt-4 text-lg font-black text-[#0b1220]">No approved resources match yet.</h2><p className="mx-auto mt-2 max-w-xl text-sm leading-6 text-[#4b5768]">Try another resource class or broader search. Grid does not fabricate supply to fill an empty marketplace.</p></div>}
  </div>;
}
