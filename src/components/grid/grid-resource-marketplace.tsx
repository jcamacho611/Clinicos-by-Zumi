...
export function GridResourceMarketplace({ resources }: { resources: PublicResource[] }) {
  ...
  return <div className="space-y-5">
    ...
    <div className="flex items-center justify-between gap-4"><p className="text-[10px] font-extrabold uppercase tracking-[.16em] text-[#64748b]">{filtered.length} approved resource{filtered.length === 1 ? "" : "s"}</p><p className="text-[10px] text-[#94a3b8]">Only public, reviewed inventory is shown here.</p></div>

    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{filtered.map((resource) => {const Icon = icons[resource.resourceType] ?? Boxes;return <article className="group flex min-h-[310px] flex-col rounded-[1.5rem] border border-[#dfe3e8] bg-white p-5 shadow-[0_18px_50px_rgba(15,23,42,.04)] transition hover:-translate-y-0.5 hover:border-[#b8c7dc]" key={resource.id}><div className="flex items-start justify-between gap-3"><span className="grid size-10 place-items-center rounded-xl bg-[#eef5ff] text-[#174ea6]"><Icon className="size-5" /></span><span className="rounded-full border border-[#dfe3e8] px-2.5 py-1 text-[9px] font-extrabold uppercase tracking-[.12em] text-[#64748b]">{label(resource.resourceType)}</span></div><h2 className="mt-5 text-xl font-black tracking-[-.04em] text-[#0b1220]">{resource.title}</h2><p className="mt-2 line-clamp-3 text-[12px] leading-6 text-[#64748b]">{resource.description}</p><div className="mt-5 grid grid-cols-2 gap-3 text-[10px]"><div><p className="font-extrabold uppercase tracking-[.12em] text-[#94a3b8]">Where</p><p className="mt-1 font-bold text-[#334155]">{resource.city || resource.state ? [resource.city, resource.state].filter(Boolean).join(", ") : "Location flexible"}</p></div><div><p className="font-extrabold uppercase tracking-[.12em] text-[#94a3b8]">Capacity</p><p className="mt-1 font-bold text-[#334155]">{resource.capacity}</p></div><div><p className="font-extrabold uppercase tracking-[.12em] text-[#94a3b8]">Price</p><p className="mt-1 font-bold text-[#334155]">{money(resource)}</p></div><div><p className="font-extrabold uppercase tracking-[.12em] text-[#94a3b8]">Next window</p><p className="mt-1 font-bold text-[#334155]">{firstWindow(resource)}</p></div></div><Link className="mt-auto flex items-center justify-between gap-3 border-t border-[#eef1f4] pt-4 text-xs font-extrabold text-[#174ea6]" href={`/grid/resources/request/${resource.id}`}>Request this resource <ArrowRight className="size-4 transition group-hover:translate-x-1" /></Link></article>})}</div>

    {filtered.length === 0 && <div className="rounded-[1.5rem] border border-dashed border-[#cbd5e1] bg-white px-6 py-16 text-center"><Boxes className="mx-auto size-8 text-[#94a3b8]" /><h2 className="mt-4 text-lg font-black text-[#0b1220]">No approved resources match yet.</h2><p className="mx-auto mt-2 max-w-xl text-sm leading-6 text-[#64748b]">Try another resource class or broader search. Grid does not fabricate supply to fill an empty marketplace.</p></div>}
  </div>;
}
