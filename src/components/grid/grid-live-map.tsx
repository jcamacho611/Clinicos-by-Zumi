      ...
      <div className="mx-auto max-w-[1500px] px-5 py-14 sm:px-8 lg:py-20">
        ...
        <div className="grid overflow-hidden border border-[#d5dae0] bg-white lg:grid-cols-[1.35fr_.65fr]">
          ...
          <aside className={`${mobileView === "list" ? "block" : "hidden"} border-t border-[#d5dae0] bg-white lg:block lg:border-l lg:border-t-0`}>
            <div className="border-b border-[#e6e9ee] p-6"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-[12px] font-extrabold uppercase tracking-[.14em] text-[#5b6675]">Published inventory</p>{userLocation && <p className="mt-2 text-[12px] font-bold text-[#174ea6]">Mapped results sorted by real distance</p>}</div>{userLocation && <label className="text-[11px] font-extrabold uppercase tracking-[.12em] text-[#5b6675]">Distance<select aria-label="Map search radius" className="mt-1 block h-9 border border-[#d5dae0] bg-white px-2 text-[11px] font-bold normal-case tracking-normal text-[#0b1220]" value={radiusMiles ?? "all"} onChange={(event) => setRadiusMiles(event.target.value === "all" ? null : Number(event.target.value))}><option value="5">Within 5 mi</option><option value="10">Within 10 mi</option><option value="25">Within 25 mi</option><option value="50">Within 50 mi</option><option value="100">Within 100 mi</option><option value="all">Any distance</option></select></label>}</div>{radiusActive && (hiddenMappedCount > 0 || unpinnedCount > 0) && <p className="mt-3 text-[12px] leading-5 text-[#6f6240]">{hiddenMappedCount > 0 ? `${hiddenMappedCount} mapped result${hiddenMappedCount === 1 ? " is" : "s are"} outside this radius. ` : ""}{unpinnedCount > 0 ? `${unpinnedCount} city/state or service-area result${unpinnedCount === 1 ? " is" : "s are"} excluded because exact distance cannot be calculated.` : ""}</p>}</div>
