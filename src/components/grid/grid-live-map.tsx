    ...
    <section className="grid-marble-surface border-b border-[#dfe3e8] bg-[#f5f7f8]">
      <div className="mx-auto max-w-[1500px] px-5 py-14 sm:px-8 lg:py-20">
        <div className="mb-10 grid gap-8 lg:grid-cols-[1fr_auto] lg:items-end">
          ...
          <div className="flex flex-wrap gap-2 text-[11px] font-bold lg:justify-end"><span className="inline-flex items-center gap-1.5 border border-[#d5dae0] bg-white px-3 py-2 text-[#0b1220]"><MapPin className="size-3.5 text-[#174ea6]" /> {rankedMapped.length} mapped{radiusActive ? ` in ${radiusMiles} mi` : ""}</span><span className="inline-flex items-center gap-1.5 border border-[#d5dae0] bg-white px-3 py-2 text-[#0b1220]"><Building2 className="size-3.5 text-[#174ea6]" /> {locations.length} spaces</span><span className="inline-flex items-center gap-1.5 border border-[#d5dae0] bg-white px-3 py-2 text-[#0b1220]"><Users className="size-3.5 text-[#0f766e]" /> {providers.length} professionals</span></div>
        </div>
        ...
        <div className="grid overflow-hidden border border-[#d5dae0] bg-white lg:grid-cols-[1.35fr_.65fr]">
          ...
          <aside className={`${mobileView === "list" ? "block" : "hidden"} border-t border-[#d5dae0] bg-white lg:block lg:border-l lg:border-t-0`}>
            <div className="border-b border-[#e6e9ee] p-6"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-[10px] font-extrabold uppercase tracking-[.14em] text-[#5b6675]">Published inventory</p>{userLocation && <p className="mt-2 text-[10px] font-bold text-[#174ea6]">Mapped results sorted by real distance</p>}</div>{userLocation && <label className="text-[9px] font-extrabold uppercase tracking-[.12em] text-[#5b6675]">Distance<select aria-label="Map search radius" className="mt-1 block h-9 border border-[#d5dae0] bg-white px-2 text-[11px] font-bold normal-case tracking-normal text-[#0b1220]" value={radiusMiles ?? "all"} onChange={(event) => setRadiusMiles(event.target.value === "all" ? null : Number(event.target.value))}><option value="5">Within 5 mi</option><option value="10">Within 10 mi</option><option value="25">Within 25 mi</option><option value="50">Within 50 mi</option><option value="100">Within 100 mi</option><option value="all">Any distance</option></select></label>}</div>{radiusActive && (hiddenMappedCount > 0 || unpinnedCount > 0) && <p className="mt-3 text-[10px] leading-5 text-[#6f6240]">{hiddenMappedCount > 0 ? `${hiddenMappedCount} mapped result${hiddenMappedCount === 1 ? " is" : "s are"} outside this radius. ` : ""}{unpinnedCount > 0 ? `${unpinnedCount} city/state or service-area result${unpinnedCount === 1 ? " is" : "s are"} excluded because exact distance cannot be calculated.` : ""}</p>}</div>
            <div className="max-h-[460px] overflow-auto">
              {rankedMapped.map((resource) => <article className={`border-b p-6 ${selectedPointId === resource.id ? "border-[#174ea6] bg-[#174ea6]/[.04]" : "border-[#e6e9ee]"}`} id={`grid-map-result-${resource.id}`} key={`mapped-resource-${resource.id}`}><button className="w-full text-left" onClick={() => selectPoint(resource.id)} type="button"><div className="flex items-start justify-between gap-3"><div><h3 className="text-sm font-extrabold text-[#0b1220]">{resource.title}</h3><p className="mt-1 text-[11px] text-[#5b6675]">{[resource.city, resource.state].filter(Boolean).join(", ") || humanize(resource.category)}</p></div><MapPin className="size-4 shrink-0 text-[#174ea6]" /></div><div className="mt-4 flex flex-wrap gap-2"><span className="border border-emerald-200 bg-emerald-50 px-2 py-1 text-[10px] font-bold text-emerald-800">Reviewed public resource</span>{resource.distanceMiles != null && <span className="border border-[#174ea6]/20 bg-[#174ea6]/[.05] px-2 py-1 text-[10px] font-bold text-[#174ea6]">{resource.distanceMiles.toFixed(1)} mi</span>}</div><p className="mt-4 text-[11px] font-extrabold text-[#174ea6]">Center on map →</p></button><Link className="mt-3 inline-flex min-h-11 items-center text-[11px] font-extrabold text-[#0b1220] underline decoration-[#174ea6] decoration-2 underline-offset-4" href={`/login?returnTo=${encodeURIComponent(`/grid/resources/request/${resource.id}?from=map`)}`}>Start governed request</Link></article>)}

              {!radiusActive && locations.map((location) => <article className="border-b border-[#e6e9ee] p-6" key={`location-${location.id}`}><div className="flex items-start justify-between gap-3"><div><h3 className="text-sm font-extrabold text-[#0b1220]">{location.name}</h3><p className="mt-1 text-[11px] text-[#5b6675]">{[location.city, location.state].filter(Boolean).join(", ") || location.locationType}</p></div><Building2 className="size-4 text-[#174ea6]" /></div><div className="mt-4 flex flex-wrap gap-1.5">{location.roomTypes.slice(0, 3).map((room) => <span className="border border-[#e6e9ee] px-2 py-1 text-[10px] font-semibold text-[#5b6675]" key={room}>{room}</span>)}{location.chairRentalAvailable && <span className="border border-[#174ea6]/20 bg-[#174ea6]/[.05] px-2 py-1 text-[10px] font-bold text-[#174ea6]">Chair rental</span>}</div><p className="mt-4 text-[12px] font-bold text-[#0b1220]">{money(location.hourlyRateCents) ? `${money(location.hourlyRateCents)}/hr` : money(location.dailyRateCents) ? `${money(location.dailyRateCents)}/day` : "Rate on request"}</p><Link className="mt-4 inline-flex text-[11px] font-extrabold text-[#174ea6]" href={`/grid/browse?intent=space&q=${encodeURIComponent(location.city ?? location.name)}`}>Explore location →</Link><p className="mt-3 text-[10px] leading-5 text-[#6f6240]">No pin shown until this legacy location supplies reviewed coordinates.</p></article>)}

              {!radiusActive && providers.map((provider) => <article className="border-b border-[#e6e9ee] p-6" key={`provider-${provider.id}`}><div className="flex items-start justify-between gap-3"><div><h3 className="text-sm font-extrabold text-[#0b1220]">{provider.serviceName}</h3><p className="mt-1 text-[11px] text-[#5b6675]">{provider.providerName} · {humanize(provider.providerType)}</p></div><BriefcaseBusiness className="size-4 text-[#0f766e]" /></div><div className="mt-4 flex flex-wrap gap-1.5">{provider.onCallNow && <span className="border border-emerald-200 bg-emerald-50 px-2 py-1 text-[10px] font-bold text-emerald-800">Available now</span>}{provider.states.slice(0, 3).map((state) => <span className="border border-[#e6e9ee] px-2 py-1 text-[10px] font-semibold text-[#5b6675]" key={state}>{state}</span>)}</div><p className="mt-3 text-[10px] leading-5 text-[#6f6240]">Professional service area is shown without an exact public location pin.</p></article>)}

              {!radiusActive && unmappedResources.slice(0, 20).map((resource) => <article className="border-b border-[#e6e9ee] p-6" key={`resource-${resource.id}`}><div className="flex items-start justify-between gap-3"><div><h3 className="text-sm font-extrabold text-[#0b1220]">{resource.title}</h3><p className="mt-1 text-[11px] text-[#5b6675]">{[resource.city, resource.state].filter(Boolean).join(", ") || humanize(resource.resourceType)}</p></div><MapPin className="size-4 text-[#9a7a1f]" /></div><p className="mt-4 text-[10px] leading-5 text-[#6f6240]">Published, but not pinned because a reviewed map position has not been supplied.</p></article>)}

              {radiusActive && rankedMapped.length === 0 && <div className="p-10 text-center"><Radar className="mx-auto size-7 text-[#174ea6]" /><p className="mt-5 text-sm font-extrabold text-[#0b1220]">No mapped resources are inside this radius.</p><p className="mt-3 text-[12px] leading-6 text-[#5b6675]">Widen the distance filter or choose Any distance. Grid does not pull unpinned inventory into an exact-radius result.</p></div>}
