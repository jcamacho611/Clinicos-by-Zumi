...
function Status({ value }: { value: string }) {
  ...
  return <span className={`rounded-full border px-2 py-1 text-[9px] font-extrabold uppercase tracking-[.12em] ${closed ? "border-emerald-300/20 bg-emerald-300/10 text-emerald-100" : "border-amber-300/20 bg-amber-300/10 text-amber-100"}`}>{human(value)}</span>;
}
...
export function GridTrustWorkspace({ workspace }: { workspace: Workspace }) {
  ...
  return <div className="space-y-6">
    <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
      ...
      ].map(([value, label]) => <div className="rounded-2xl border border-white/10 bg-white/[.035] p-4" key={label}><p className="text-2xl font-black text-white">{value}</p><p className="mt-1 text-[9px] font-extrabold uppercase tracking-[.12em] text-white/35">{label}</p></div>)}
    </section>

    <section className="rounded-[1.6rem] border border-white/10 bg-[#070b13] p-5 sm:p-6">
      <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-[10px] font-black uppercase tracking-[.18em] text-cyan-200">Objective trust</p><h2 className="mt-2 text-2xl font-black text-white">Transaction history, not a popularity score.</h2><p className="mt-3 max-w-3xl text-xs leading-6 text-white/45">{workspace.summary.note}</p></div><div className="rounded-xl border border-white/10 bg-white/[.03] px-4 py-3 text-right"><p className="text-lg font-black text-white">{workspace.summary.repeatCounterparties}</p><p className="text-[9px] uppercase tracking-[.12em] text-white/35">Repeat relationships</p></div></div>
    </section>
    ...
    <section className="grid gap-5 xl:grid-cols-[.85fr_1.15fr]">
      <div className="rounded-[1.6rem] border border-white/10 bg-white/[.03] p-5 sm:p-6">
        <p className="text-[10px] font-black uppercase tracking-[.18em] text-amber-200">Report an issue</p>
        ...
        <label className="mt-5 block text-[9px] font-bold uppercase tracking-[.12em] text-white/40">Reservation<select className="mt-2 h-11 w-full rounded-xl border border-white/10 bg-black/25 px-3 text-xs text-white" value={reservationId} onChange={(event) => setReservationId(event.target.value)}><option value="">Choose reservation</option>{workspace.reservations.map((reservation) => <option key={reservation.id} value={reservation.id}>{reservation.demandTitle} · {new Date(reservation.reservedStartAt).toLocaleString()}</option>)}</select></label>

        <label className="mt-4 block text-[9px] font-bold uppercase tracking-[.12em] text-white/40">Category<select className="mt-2 h-11 w-full rounded-xl border border-white/10 bg-black/25 px-3 text-xs text-white" value={category} onChange={(event) => setCategory(event.target.value)}>{(mode === "dispute" ? disputeCategories : safetyCategories).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>

        {mode === "safety" && <label className="mt-4 block text-[9px] font-bold uppercase tracking-[.12em] text-white/40">Severity<select className="mt-2 h-11 w-full rounded-xl border border-white/10 bg-black/25 px-3 text-xs text-white" value={severity} onChange={(event) => setSeverity(event.target.value)}><option value="low">Low</option><option value="medium">Medium</option><option value="high">High</option><option value="urgent">Urgent</option></select></label>}

        <label className="mt-4 block text-[9px] font-bold uppercase tracking-[.12em] text-white/40">What happened?<textarea className="mt-2 min-h-28 w-full rounded-xl border border-white/10 bg-black/25 p-3 text-xs text-white outline-none" value={summary} onChange={(event) => setSummary(event.target.value)} placeholder="State observable facts. Do not use this box for patient records or broad PHI." /></label>
        {mode === "dispute" && <label className="mt-4 block text-[9px] font-bold uppercase tracking-[.12em] text-white/40">Requested outcome<textarea className="mt-2 min-h-20 w-full rounded-xl border border-white/10 bg-black/25 p-3 text-xs text-white outline-none" value={requestedOutcome} onChange={(event) => setRequestedOutcome(event.target.value)} placeholder="Example: review cancellation charge; release reservation; recommend refund review." /></label>}

        {message && <p className={`mt-4 rounded-xl border px-4 py-3 text-[10px] leading-5 ${message.ok ? "border-emerald-300/20 bg-emerald-300/[.06] text-emerald-100" : "border-rose-300/20 bg-rose-300/[.06] text-rose-100"}`}>{message.ok ? <CheckCircle2 className="mr-1.5 inline size-3.5" /> : <AlertTriangle className="mr-1.5 inline size-3.5" />}{message.text}</p>}
      ...
      <div className="space-y-5">
        ...
        <section className="rounded-[1.6rem] border border-white/10 bg-white/[.03] p-5 sm:p-6">
          ...
          <div className="mt-4 space-y-3">{workspace.safetyIncidents.length ? workspace.safetyIncidents.map((item) => <article className="rounded-xl border border-white/10 bg-black/20 p-4" key={item.id}><div className="flex flex-wrap items-center justify-between gap-2"><div className="flex flex-wrap items-center gap-2"><p className="text-xs font-extrabold text-white">{human(item.category)}</p><span className="text-[9px] font-black uppercase tracking-[.12em] text-amber-200/70">{item.severity}</span></div><Status value={item.status}/></div><p className="mt-2 text-[11px] leading-5 text-white/50">{item.summary}</p>{item.resolutionNote && <p className="mt-2 text-[10px] leading-5 text-emerald-100/55"><strong>Governance note:</strong> {item.resolutionNote}</p>}</article>) : <p className="rounded-xl border border-dashed border-white/15 p-5 text-xs text-white/35">No safety incidents recorded.</p>}</div>
        </section>
      </div>
    </section>
  </div>;
