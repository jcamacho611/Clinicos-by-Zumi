...
function Status({ value }: { value: string }) {
  ...
  return <span className={`rounded-full border px-2.5 py-1 text-[11px] font-extrabold uppercase tracking-[.12em] ${good ? "border-emerald-300/20 bg-emerald-300/10 text-emerald-100" : bad ? "border-rose-300/20 bg-rose-300/10 text-rose-100" : "border-amber-300/20 bg-amber-300/10 text-amber-100"}`}>{title(value)}</span>;
}

function Metric({ titleText, value, detail }: { titleText: string; value: string; detail: string }) {
  return <div className="rounded-[1.4rem] border border-white/10 bg-white/[.045] p-5"><p className="text-2xl font-black tracking-[-.05em] text-white">{value}</p><p className="mt-1 text-[11px] font-extrabold text-white/75">{titleText}</p><p className="mt-2 text-[11px] leading-4 text-white/35">{detail}</p></div>;
}
  ...
  return <div className="space-y-6">
    ...
    <section className="grid gap-5 xl:grid-cols-[.8fr_1.2fr]">
      <div className="rounded-[1.7rem] border border-white/10 bg-[#070b13] p-5 sm:p-6">
        <div className="flex items-center gap-3"><Percent className="size-5 text-cyan-200" /><div><p className="text-[11px] font-black uppercase tracking-[.18em] text-cyan-200">Platform policy</p><h2 className="mt-1 text-xl font-black text-white">Set Grid economics</h2></div></div>
        <p className="mt-3 text-[12px] leading-5 text-white/40">The platform fee is never accepted from public offer input. This console replaces the active policy for one exact scope while preserving prior policy history.</p>
      ...
      <div className="rounded-[1.7rem] border border-white/10 bg-white/[.035] p-5 sm:p-6"><div className="flex items-center gap-3"><Landmark className="size-5 text-amber-200" /><h2 className="text-xl font-black text-white">Policy history</h2></div><div className="mt-4 space-y-2">{board.policies.length === 0 && <p className="text-xs text-white/40">No fee policy has been configured yet. Fulfilled transactions will refuse financial allocation until one applies.</p>}{board.policies.map((item) => <div className="flex items-center justify-between gap-4 rounded-xl border border-white/10 bg-black/20 p-3" key={item.id}><div><Status value={item.status} /><p className="mt-2 text-xs font-extrabold text-white">{title(item.scopeKind)} {item.scopeValue ? `· ${title(item.scopeValue)}` : ""}</p></div><div className="text-right"><p className="text-sm font-black text-white">{(item.platformFeeBps / 100).toFixed(2)}%</p><p className="text-[11px] text-white/35">+ {money(item.platformFeeFlatCents)} flat</p></div></div>)}</div></div>
    </section>

    <section className="rounded-[1.7rem] border border-white/10 bg-[#070b13] p-5 sm:p-6"><div className="flex items-center gap-3"><BadgeDollarSign className="size-5 text-emerald-200" /><div><p className="text-[11px] font-black uppercase tracking-[.18em] text-emerald-200">Settlement control</p><h2 className="mt-1 text-xl font-black text-white">Financial obligations</h2></div></div><div className="mt-5 grid gap-3 xl:grid-cols-2">{board.obligations.length === 0 && <p className="text-xs text-white/40">No fulfilled Grid transactions have been allocated yet.</p>}{board.obligations.map((line) => <article className="rounded-2xl border border-white/10 bg-white/[.035] p-4" key={line.id}><div className="flex items-start justify-between gap-3"><div><Status value={line.status} /><p className="mt-2 text-sm font-extrabold text-white">{title(line.obligationType)}</p><p className="mt-1 text-[12px] text-white/40">{line.demandTitle} · {line.ownerName}</p></div><p className="text-xl font-black text-white">{money(line.amountCents)}</p></div><p className="mt-3 text-[12px] text-white/40">Beneficiary: {line.beneficiaryName ?? (line.beneficiaryType === "platform" ? "Klinikos" : line.beneficiaryReference ?? "Unresolved")}</p>{line.externalReference && <p className="mt-1 break-all text-[11px] text-emerald-200/70">Reference: {line.externalReference}</p>}{(transitions[line.status] ?? []).length > 0 && <div className="mt-3"><input className="w-full rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-[12px] text-white" placeholder="External/batch reference (required to settle)" value={references[line.id] ?? ""} onChange={(event) => setReferences({ ...references, [line.id]: event.target.value })} /><div className="mt-2 flex flex-wrap gap-2">{(transitions[line.status] ?? []).map((action) => <button className="rounded-lg border border-white/10 bg-white/[.04] px-3 py-2 text-[12px] font-extrabold text-white/65 hover:text-white" disabled={busy === `${line.id}:${action.status}`} key={action.status} onClick={() => transition(line.id, action.status)}>{busy === `${line.id}:${action.status}` ? "Recording…" : action.label}</button>)}</div></div>}</article>)}</div></section>

    <div className="rounded-[1.4rem] border border-amber-300/10 bg-amber-300/[.04] p-4 text-[12px] leading-5 text-amber-100/65"><ShieldCheck className="mr-2 inline size-4" /><strong className="text-amber-100">Settlement truth:</strong> this console records manual reconciliation today. It does not claim processor verification, escrow, automated payout, tax reporting, or Stripe Connect until those rails are actually connected and tested.</div>
  </div>;
}
