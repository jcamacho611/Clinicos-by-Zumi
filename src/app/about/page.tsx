    ...
    <main className="min-h-screen bg-[#070b13] text-white">
        ...
        <div className="mx-auto flex h-20 max-w-[1500px] items-center gap-4 px-5 sm:px-8">
          <Link className="flex items-center gap-3" href="/">
            ...
            <span>
              ...
              <span className="block text-[11px] font-extrabold uppercase tracking-[.18em] text-cyan-200/70">Mission & company story</span>
            </span>
          </Link>
          <nav className="ml-auto hidden items-center gap-6 text-[12px] font-extrabold uppercase tracking-[.14em] text-white/45 md:flex">
      ...
      <section className="relative overflow-hidden border-b border-white/10 px-5 py-20 sm:px-8 lg:py-28">
        ...
        <div className="relative mx-auto max-w-[1500px]">
          <Link className="inline-flex items-center gap-2 text-[12px] font-extrabold uppercase tracking-[.16em] text-white/40 hover:text-white" href="/"><ArrowLeft className="size-3.5" />Back to Klinikos</Link>
          <div className="mt-14 grid gap-12 xl:grid-cols-[1.2fr_.8fr] xl:items-end">
            <div>
              <p className="text-[12px] font-black uppercase tracking-[.24em] text-cyan-200">{story.mission.eyebrow}</p>
            ...
            <div className="border border-white/10 bg-white/[.035] p-6 sm:p-8">
              <p className="text-[11px] font-extrabold uppercase tracking-[.18em] text-amber-200">Why this exists</p>
      ...
      <section className="border-b border-white/10 px-5 py-16 sm:px-8 lg:py-20">
        <div className="mx-auto max-w-[1500px]">
          <div className="max-w-4xl"><p className="text-[12px] font-black uppercase tracking-[.2em] text-cyan-200">One ecosystem</p><h2 className="mt-4 text-4xl font-black tracking-[-.055em] sm:text-5xl">Different surfaces. One continuity model underneath.</h2></div>
          <div className="mt-10 grid gap-px overflow-hidden border border-white/10 bg-white/10 md:grid-cols-2 xl:grid-cols-4">
            {story.ecosystem.map((item, index) => <article className="bg-[#090e18] p-6" key={item.name}><span className="text-[11px] font-black uppercase tracking-[.18em] text-white/25">0{index + 1}</span><h3 className="mt-5 text-2xl font-black tracking-[-.04em]">{item.name}</h3><p className="mt-1 text-[12px] font-extrabold uppercase tracking-[.14em] text-cyan-200/70">{item.role}</p><p className="mt-5 text-xs leading-6 text-white/45">{item.description}</p></article>)}
          </div>
        </div>
      </section>
      ...
      <section className="border-b border-white/10 bg-[#090e18] px-5 py-16 sm:px-8 lg:py-24">
        <div className="mx-auto grid max-w-[1500px] gap-12 xl:grid-cols-[.8fr_1.2fr]">
          <div>
            <p className="text-[12px] font-black uppercase tracking-[.2em] text-amber-200">{story.founder.label}</p>
      ...
      <section className="border-b border-white/10 px-5 py-16 sm:px-8 lg:py-24">
        <div className="mx-auto max-w-[1500px]">
          <div className="grid gap-10 xl:grid-cols-[.75fr_1.25fr]">
            <div><p className="text-[12px] font-black uppercase tracking-[.2em] text-cyan-200">How we build</p><h2 className="mt-4 text-4xl font-black tracking-[-.055em] sm:text-5xl">Principles before promises.</h2><p className="mt-5 max-w-xl text-sm leading-7 text-white/45">The system is deliberately broad in scope, but each resource class, regulated workflow, and consequential action keeps the boundary it actually requires.</p></div>
            <div className="grid gap-px overflow-hidden border border-white/10 bg-white/10 sm:grid-cols-2">{story.principles.map((principle, index) => <article className="bg-[#070b13] p-6" key={principle.title}><span className="text-[11px] font-black uppercase tracking-[.16em] text-white/20">0{index + 1}</span><h3 className="mt-4 text-lg font-black">{principle.title}</h3><p className="mt-3 text-xs leading-6 text-white/45">{principle.description}</p></article>)}</div>
          </div>
        </div>
      </section>
      ...
      <section className="px-5 py-16 sm:px-8 lg:py-24">
        <div className="mx-auto max-w-[1500px] rounded-[2rem] border border-amber-300/15 bg-amber-300/[.035] p-6 sm:p-9">
          <div className="grid gap-8 xl:grid-cols-[.6fr_1.4fr]">
            <div><ShieldCheck className="size-6 text-amber-200" /><p className="mt-5 text-[12px] font-black uppercase tracking-[.2em] text-amber-200">Public truth boundary</p><h2 className="mt-3 text-3xl font-black tracking-[-.05em]">What we refuse to blur.</h2></div>
