  ...
  return (
    <div className="mx-auto max-w-[1180px] space-y-7 text-[#f8efed]">
        ...
        <div className="relative grid gap-8 lg:grid-cols-[1.2fr_.8fr] lg:items-start">
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <p className="text-[12px] font-extrabold uppercase tracking-[.21em] text-[#e6817b]">Klinikos Route</p>
              <span className="rounded-full border border-[#e6817b]/16 bg-[#e6817b]/[.07] px-2.5 py-1 text-[11px] font-bold text-[#efaaa1]">{availabilityLabel[definition.availability]}</span>
            </div>
            ...
            <div className="mt-7 grid gap-3 sm:grid-cols-[1fr_auto_1fr] sm:items-center">
              <div className="rounded-[18px] border border-[#e6817b]/10 bg-[#090405]/72 p-4"><p className="text-[11px] font-extrabold uppercase tracking-[.15em] text-[#725d59]">Where you are</p><p className="mt-2 text-sm font-semibold text-[#e9d8d5]">{definition.from}</p></div>
              <ArrowRight className="hidden size-4 text-[#e6817b] sm:block" />
              <div className="rounded-[18px] border border-[#e6817b]/10 bg-[#090405]/72 p-4"><p className="text-[11px] font-extrabold uppercase tracking-[.15em] text-[#725d59]">Where you want to be</p><p className="mt-2 text-sm font-semibold text-[#e9d8d5]">{definition.to}</p></div>
            </div>
          </div>

          <aside className="rounded-[22px] border border-[#d6b787]/16 bg-[#d6b787]/[.045] p-5">
            <div className="flex items-center gap-2 text-[#efd8ad]"><ShieldCheck className="size-4" /><p className="text-[12px] font-extrabold uppercase tracking-[.16em]">Governance</p></div>
            <p className="mt-4 text-xs leading-6 text-[#bca6a1]">{definition.governance}</p>
            <div className="mt-6 border-t border-[#d6b787]/12 pt-5"><p className="text-[11px] font-extrabold uppercase tracking-[.15em] text-[#806965]">Progress</p><div className="mt-2 flex items-end gap-3"><p className="text-4xl font-light tracking-[-.05em] text-[#fff8f6]">{Math.round(runtime.progress * 100)}%</p><p className="pb-1 text-[12px] font-semibold uppercase tracking-[.12em] text-[#8f7773]">{runtime.status}</p></div><div className="mt-3 h-1 overflow-hidden rounded-full bg-[#2c171a]"><div className="h-full rounded-full bg-[#e6817b]" style={{ width: `${Math.max(2, Math.round(runtime.progress * 100))}%` }} /></div></div>
          </aside>
        </div>

        {current ? (
          <div className="relative mt-8 rounded-[22px] border border-[#e6817b]/12 bg-[#12090b]/78 p-5 sm:p-6">
            <p className="text-[11px] font-extrabold uppercase tracking-[.16em] text-[#e6817b]">Next eligible step</p>
      ...
      <section className="rounded-[28px] border border-[#e6817b]/12 bg-[#0d0608] p-6 sm:p-8">
        <div className="mb-7"><p className="text-[12px] font-extrabold uppercase tracking-[.18em] text-[#806965]">Journey</p><h2 className="mt-2 text-2xl font-light tracking-[-.045em] text-[#fff8f6]">What is complete, what is current, and what comes next.</h2></div>
        <PathRail nodes={nodes} />
      </section>

      {definition.commercialBoundary ? <section className="rounded-[22px] border border-[#e6817b]/10 bg-[#100708]/70 p-5"><p className="text-[11px] font-extrabold uppercase tracking-[.16em] text-[#e6817b]">When paid capability becomes relevant</p><p className="mt-3 max-w-4xl text-xs leading-6 text-[#9f8985]">{definition.commercialBoundary}</p></section> : null}
    </div>
  );
}
