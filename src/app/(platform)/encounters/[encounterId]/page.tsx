...
import { getEncounterStaffHandoffProjection } from "@/lib/repositories/encounter-handoff-repository";
...
export default async function EncounterPage({ params }: { params: Promise<{ encounterId: string }> }) {
  ...
  const [patient, handoff] = await Promise.all([
    findPatientForOrganization(encounter.patientId, session.organizationId),
    getEncounterStaffHandoffProjection(encounter.id, encounter.patientId, session.organizationId),
  ]);
  if (!patient) notFound();
  return <EncounterEditor canSign={can(session.role, "encounters", "sign")} encounter={encounter} handoff={handoff} patient={patient} />;
}
