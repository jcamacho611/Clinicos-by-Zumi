...
export default async function EncounterPage({ params }: { params: Promise<{ encounterId: string }> }) {
  ...
  const [patient, vital, clinicalEvidence] = await Promise.all([
  ...
  if (!patient) notFound();
  return <EncounterEditor canSign={can(session.role, "encounters", "sign")} clinicalEvidence={clinicalEvidence} encounter={encounter} patient={patient} vital={vital} />;
}
