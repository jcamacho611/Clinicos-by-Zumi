...
export default async function AdminEduPage() {
  ...
  return (
    <section className="p-5 sm:p-8">
      <p className="text-[11px] font-extrabold uppercase tracking-[.16em] text-[#e6c55b]">Klinikos EDU</p>
      ...
      <div className="mt-4 overflow-x-auto border border-white/10 bg-white/[.03]">
          ...
          <tbody>
            {institutions.map((institution) => (
              <tr className="border-t border-white/10" key={institution.id}>
                <th className="px-4 py-3 font-bold text-white" scope="row">{institution.name}</th>
                <td className="px-4 py-3 text-slate-400">{institution.slug}</td>
                <td className="px-4 py-3 tabular-nums text-slate-300">{institution._count.courses}</td>
                <td className="px-4 py-3 tabular-nums text-slate-300">{institution._count.enrollments}</td>
                <td className="px-4 py-3 text-slate-400">{institution.status}</td>
              </tr>
            ))}
            {!institutions.length && (
      ...
      <p className="mt-8 max-w-3xl border-t border-white/10 pt-5 text-[11px] leading-5 text-slate-500">
        AI scenario drafting and feedback: <strong className="font-bold text-[#f0dda0]">Pending Connection</strong>. {gateway.detail}
      </p>
    </section>
  );
}
