...
const pathways = [
  {
    title: "Clinic",
    detail: "Run daily operations, patient workflows, follow-up, referrals, results, tasks, and revenue continuity.",
    href: "/login",
    action: "Enter Klinikos",
  ...
  {
    title: "Education",
    detail: "Learn, practice, build verified skills, and move from education toward healthcare opportunity.",
    href: "/edu",
  ...
  {
    title: "Private demo",
    detail: "See how Klinikos maps operational gaps using a synthetic, no-PHI demonstration.",
    href: "/private-demo",
    action: "See the demo",
    icon: Sparkles,
  },
  {
    title: "Founding clinics",
    detail: "Explore the evaluation and early implementation pathway for clinics building with Klinikos.",
    href: "/founding-clinic",
    action: "View the program",
    icon: BookOpen,
  },
] as const;
  ...
  return (
    <>
      ...
      <section
        ...
        className="relative z-20 border-t border-white/10 bg-[#070b13] px-5 py-20 text-white sm:px-8 lg:px-12"
      >
        <div className="mx-auto max-w-7xl">
          <div className="max-w-3xl">
            <p className="text-xs font-bold uppercase tracking-[0.2em] text-cyan-200/70">One Klinikos</p>
            <h2 id="pathways-title" className="mt-4 text-3xl font-semibold tracking-tight sm:text-5xl">
          ...
          <div className="mt-10 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            {pathways.map(({ title, detail, href, action, icon: Icon }) => (
              <Link
                key={title}
                href={href}
                className="group flex min-h-52 flex-col justify-between rounded-3xl border border-white/10 bg-white/[0.035] p-6 transition hover:-translate-y-0.5 hover:border-cyan-300/30 hover:bg-white/[0.06] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300"
              >
                <div>
                  <div className="flex size-10 items-center justify-center rounded-2xl border border-white/10 bg-white/[0.04] text-cyan-100">
                    <Icon className="size-5" aria-hidden="true" />
                  </div>
                  <h3 className="mt-6 text-xl font-semibold">{title}</h3>
                  <p className="mt-3 text-sm leading-6 text-white/55">{detail}</p>
                </div>
                <span className="mt-7 inline-flex items-center gap-2 text-sm font-semibold text-white/80 transition group-hover:text-white">
                  {action} <ArrowRight className="size-4 transition group-hover:translate-x-1" aria-hidden="true" />
                </span>
              </Link>
            ))}
          </div>
