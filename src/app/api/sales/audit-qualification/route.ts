    ...
    const result = await db.$transaction(async (tx) => {
      ...
      const task = await tx.task.create({
        data: {
          ...
          details: `lead:${lead.id} Confirm processor payment evidence in the Klinikos commercial ledger before beginning the audit. Never infer payment from checkout launch or browser return.`,
      ...
      await tx.leadEvent.create({
        data: {
          ...
          note: "Qualified prospect saved before external processor checkout.",
