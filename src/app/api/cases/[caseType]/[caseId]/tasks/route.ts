...
import { createCaseTask } from "@/lib/repositories/case-repository";
...
export async function POST(request: Request, { params }: { params: Promise<{ caseType: string; caseId: string }> }) {
  ...
  try { return NextResponse.json({ data: await createCaseTask(session, caseType, caseId, await request.json()) }, { status: 201 }); }
