...
import { createGridDispute, listGridIssuesForReservation } from "@/lib/grid/trust-repository";
...
export async function GET(request: Request, { params }: { params: Promise<{ reservationId: string }> }) {
  ...
  try {
    ...
    return NextResponse.json({ data: issues.disputes });
  } catch (error) {
...
export async function POST(request: Request, { params }: { params: Promise<{ reservationId: string }> }) {
  ...
  try {
    const { reservationId } = await params;
    return NextResponse.json({ data: await createGridDispute(session, reservationId, await request.json()) }, { status: 201 });
  } catch (error) {
    return networkAccessErrorResponse(error);
  }
}
