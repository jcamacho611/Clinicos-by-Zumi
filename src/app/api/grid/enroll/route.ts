...
export async function POST(request: Request) {
  ...
  try {
    return NextResponse.json({ data: await createGridContractorEnrollment(await request.json().catch(() => null)), message: "Application submitted for human credential and malpractice review." }, { status: 201 });
  } catch (error) {
