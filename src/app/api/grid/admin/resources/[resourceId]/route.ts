...
import { reviewGridResource } from "@/lib/grid/resource-repository";
...
export async function PATCH(request: Request, { params }: { params: Promise<{ resourceId: string }> }) {
  ...
  try {
    ...
    return NextResponse.json({ data: await reviewGridResource(session, resourceId, await request.json()) });
  } catch (error) {
