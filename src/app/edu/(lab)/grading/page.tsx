  ...
  return (
    <>
      ...
      <div className="px-5 py-6 sm:px-8">
        <p className="max-w-3xl border-l-2 border-slate-300 pl-4 text-[12px] leading-6 text-slate-600">{HUMAN_REVIEW_AUTHORITY}</p>
        <div className="mt-6">
          {submissions.length ? (
                ...
                <tbody>
                  {submissions.map((submission) => (
                    <tr className="border-t border-slate-200" key={submission.id}>
                      <th className="px-4 py-3 font-bold text-slate-950" scope="row">{submission.enrollment.studentDisplayName}</th>
                      <td className="px-4 py-3 text-slate-700">{submission.assignment.title}</td>
                      <td className="px-4 py-3 text-slate-600">{submission.simulationRole?.replace(/_/g, " ") ?? "—"}</td>
                      <td className="px-4 py-3 text-slate-600">{submission.submittedAt?.toISOString().slice(0, 10) ?? "—"}</td>
