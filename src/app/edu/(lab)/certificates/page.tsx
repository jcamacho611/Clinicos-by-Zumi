...
export default async function EduCertificatesPage() {
  ...
  const [certificates, enrollments] = await Promise.all([
    db.educationCertificate.findMany({
      ...
      select: { id: true, certificateType: true, title: true, disclaimer: true, serialNumber: true, issuedAt: true, revokedAt: true, revokedReason: true, enrollment: { select: { studentDisplayName: true, studentEmail: true } } },
      orderBy: { issuedAt: "desc" }, take: 250,
    }),
    canEdu(identity.role, "certificate", "create")
      ? db.educationEnrollment.findMany({ where: enrollmentScope, select: { id: true, studentDisplayName: true, status: true, competencies: { where: { status: "demonstrated" }, select: { competencyArea: true } } }, orderBy: { studentDisplayName: "asc" }, take: 500 })
      : Promise.resolve([]),
  ]);

  return <>
    <EduCommandHeader eyebrow="Educational evidence" title="Certificates" description="Issue and review completion or competency evidence. These records never replace licensure, credentialing, clinical privileges, or Grid eligibility checks." />
    <div className="px-5 py-6 sm:px-8">
      <EduCertificateManager
        ...
        certificates={certificates.map((certificate) => ({ ...certificate, issuedAt: certificate.issuedAt.toISOString(), revokedAt: certificate.revokedAt?.toISOString() ?? null }))}
        enrollments={enrollments.map((enrollment) => ({ id: enrollment.id, studentDisplayName: enrollment.studentDisplayName, status: enrollment.status, demonstratedCompetencyAreas: enrollment.competencies.map((competency) => competency.competencyArea) }))}
      />
