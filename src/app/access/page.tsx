...
export default function AccessPage() {
  ...
  function continueIntoKlinikos(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!accepted || !email.trim()) return;
    try {
      window.localStorage.setItem("klinikos-access-email", email.trim().toLowerCase());
      window.localStorage.setItem("klinikos-access-terms-version", "2026-08-10");
      window.localStorage.setItem("klinikos-access-accepted-at", new Date().toISOString());
    } catch {
      // Access remains available when client storage is disabled; account-level terms are enforced again later.
    }
    setSubmitted(true);
  }
        ...
        <div className="mx-auto flex min-h-screen max-w-3xl items-center px-5 py-14 sm:px-8">
          <section className="w-full border-y border-slate-200 bg-white py-14 sm:px-10">
            <div className="mx-auto max-w-xl px-5 sm:px-0">
              ...
              <p className="mt-6 text-[11px] font-extrabold uppercase tracking-[.18em] text-[#9a7a1f]">Access acknowledged</p>
              <h1 className="mt-3 text-4xl font-extrabold tracking-[-.055em]">Welcome to Klinikos.</h1>
              <p className="mt-5 text-sm leading-7 text-slate-600">Your evaluation access is subject to the Klinikos Access, Confidentiality & Intellectual Property Terms. Choose how you want to continue.</p>
    ...
    <main className="min-h-screen bg-[#07151c] text-white">
      <div className="mx-auto grid min-h-screen max-w-7xl gap-10 px-5 py-12 sm:px-8 lg:grid-cols-[1fr_.8fr] lg:items-center lg:py-16">
        <section>
          ...
          <p className="mt-7 max-w-2xl text-base leading-8 text-slate-300">Klinikos contains proprietary software architecture, workflows, interface concepts, commercial methods, implementation materials, product documentation and confidential operating knowledge. Product access is provided for evaluation only.</p>
        ...
        <section className="bg-white p-6 text-slate-950 shadow-[0_28px_90px_rgba(0,0,0,.35)] sm:p-8">
          ...
          <form className="mt-6" onSubmit={continueIntoKlinikos}>
            ...
            <Button className="mt-5 w-full" disabled={!accepted || !email.trim()} size="lg" type="submit" variant="primary">Enter Klinikos <ArrowRight className="size-4" /></Button>
          </form>
          <div className="mt-6 border-t border-slate-200 pt-5 text-[10px] leading-5 text-slate-500">Evaluation acceptance is separate from clinic, provider, GRID and production agreements. Those workflows require their own role-specific terms and operational approvals.</div>
