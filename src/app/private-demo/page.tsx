    ...
    <SalesSiteShell>
      <section className="mx-auto max-w-[1500px] px-5 pb-24 pt-24 sm:px-8 lg:px-12 lg:pb-36 lg:pt-32">
        <CinematicReveal className="grid gap-16 lg:grid-cols-[1.08fr_.92fr] lg:items-end">
          <div>
            ...
            <p className="mt-10 text-[12px] font-black uppercase tracking-[.24em] text-[#efaaa1]">{analysisOffer.name}</p>
          ...
          <div className="border-t border-white/12 pt-8 lg:pl-8">
            <div className="flex items-end justify-between gap-5"><div><p className="text-[12px] font-black uppercase tracking-[.2em] text-slate-500">{analysisOffer.name}</p><p className="mt-3 text-6xl font-black tracking-[-.07em]">{analysisOffer.priceLabel}</p></div><Clock3 className="size-9 text-[#efaaa1]" strokeWidth={1.3} /></div>
            ...
            <p className="mt-5 max-w-md text-[12px] leading-5 text-slate-600">Your clinic and the server-owned {analysisOffer.priceLabel} analysis amount are saved before the secure payment page opens. Opening or returning from checkout never marks the purchase paid; Klinikos waits for signed processor evidence or authorized reconciliation.</p>
          </div>
        </CinematicReveal>
        ...
        <CinematicReveal className="mt-28 grid gap-12 lg:grid-cols-2" delay={0.12}>
          <section>
            <p className="text-[12px] font-black uppercase tracking-[.2em] text-[#efaaa1]">Included</p>
          ...
          <section>
            <p className="text-[12px] font-black uppercase tracking-[.2em] text-rose-300">Not included</p>
        ...
        <CinematicReveal className="mt-28 border-y border-[#e6817b]/15 py-12" delay={0.14}>
          <div className="grid gap-10 lg:grid-cols-[.7fr_1.3fr] lg:items-start">
            <div><p className="text-[12px] font-black uppercase tracking-[.2em] text-[#efaaa1]">Why the {analysisOffer.priceLabel} fee exists</p><h2 className="mt-4 text-3xl font-black tracking-[-.05em]">Preparation is the product.</h2></div>
      ...
      <section className="border-y border-white/[.08] bg-black/20" id="reserve">
        <div className="mx-auto max-w-[1500px] px-5 py-24 sm:px-8 lg:px-12 lg:py-32">
          <div className="mb-14 max-w-3xl"><p className="text-[12px] font-black uppercase tracking-[.22em] text-[#efaaa1]">Start the analysis</p><h2 className="mt-5 text-4xl font-black tracking-[-.055em] sm:text-5xl">Give Klinikos the business problem. Keep patient data out.</h2><p className="mt-5 text-sm leading-7 text-slate-400">This intake is for the {analysisOffer.name}. Klinikos saves the clinic and buyer first, then creates a server-owned checkout intent for exactly {analysisOffer.priceLabel} before exposing a configured payment page.</p></div>
      ...
      <section className="mx-auto flex max-w-[1500px] flex-col gap-8 px-5 py-20 sm:px-8 lg:flex-row lg:items-end lg:justify-between lg:px-12 lg:py-24">
        <div><p className="text-[12px] font-black uppercase tracking-[.2em] text-violet-300">What comes after the analysis?</p><h2 className="mt-3 max-w-4xl text-2xl font-black tracking-[-.04em]">The Implementation Blueprint, Founding Clinic Implementation, and recurring plans are separate reviewed decisions after the analysis.</h2></div>
