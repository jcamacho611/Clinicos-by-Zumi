...
export default async function GridLocationJoinPage({ searchParams }: { searchParams: Promise<{ type?: string }> }) {
  ...
  return <main className="min-h-screen bg-[#f6f8fa] text-[#0b1220]">
    <header className="border-b border-[#e2e6ea] bg-white"><div className="mx-auto flex h-20 max-w-[1400px] items-center px-5 sm:px-8"><Link className="flex items-center gap-3" href="/grid"><BrandMark /><span><span className="block text-sm font-extrabold">Klinikos Grid</span><span className="block text-[9px] font-bold uppercase tracking-[.18em] text-[#174ea6]">{organizationMode ? "Organization capacity" : "Space & capacity"}</span></span></Link><Link className="ml-auto inline-flex items-center gap-2 text-xs font-bold text-[#5b6675]" href="/grid"><ArrowLeft className="size-4"/>Back to Grid</Link></div></header>
    ...
    <section className="mx-auto max-w-[1400px] px-5 py-12 sm:px-8"><div className="grid gap-8 lg:grid-cols-[.7fr_1.3fr]"><aside><p className="text-[10px] font-extrabold uppercase tracking-[.18em] text-[#174ea6]">Real setup</p><h2 className="mt-3 text-3xl font-black tracking-[-.05em]">Create the account and resource once.</h2><p className="mt-4 text-sm leading-7 text-[#5b6675]">This path now writes to the Grid resource registry. A successful screen means the account and resource actually exist; it does not mean the resource has been approved.</p><div className="mt-7 border-t border-[#dfe3e8]">{steps.map(([n,title,body])=><div className="grid gap-3 border-b border-[#dfe3e8] py-4 sm:grid-cols-[40px_1fr_auto] sm:items-center" key={n}><span className="grid size-8 place-items-center rounded-full bg-[#174ea6] text-xs font-black text-white">{n}</span><div><h3 className="text-sm font-extrabold">{title}</h3><p className="mt-1 text-[11px] leading-5 text-[#5b6675]">{body}</p></div><Check className="size-4 text-[#0f766e]"/></div>)}</div></aside><CapacityIntakeForm mode={mode} /></div></section>
  </main>;
}
