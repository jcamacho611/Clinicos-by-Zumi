...
export async function listNetworkGrowthWorkspace(session: ClinicSession) {
  ...
  const [invitations, organizations, facilities, providers, referrals, connections, audits] = await Promise.all([
...
export async function transitionNetworkInvitation(session: ClinicSession, invitationId: string, rawInput: unknown) {
  ...
  const invitation = await db.networkInvitation.findFirst({ where: { id: invitationId, OR: [{ invitingOrganizationId: session.organizationId }, { targetOrganizationId: session.organizationId }] } });
  ...
  if (input.action === "accept" && !targetSide && invitation.targetOrganizationId) throw new NetworkAccessError("Only the invited organization can accept this invitation.", 403);
  ...
  const status = input.action === "verify" ? "verified" : input.action === "accept" ? "accepted" : input.action === "reject" ? "rejected" : input.action === "cancel" ? "cancelled" : input.action === "suspend" ? "suspended" : "accepted";
  const now = new Date();
