...
function resolveDocumentKey() {
  ...
  if (process.env.NODE_ENV === "production") throw new NetworkAccessError("Encrypted document storage is not configured.", 503);
  if (!process.env.AUTH_SECRET) throw new NetworkAccessError("AUTH_SECRET is required for the encrypted development document fallback.", 503);
...
export function decryptDocumentContent(input: {
  ...
  if (input.checksumSha256 && checksum !== input.checksumSha256) throw new NetworkAccessError("Document integrity verification failed.", 409);
  return content;
}
