...
export async function bindAcceptanceToAccountIdentity(
  ...
  if (acceptanceRows.length !== 1) {
    throw new Error("Protected-entry acceptance is missing, already organization-bound, or ambiguous.");
  }
  ...
  if (!inserted[0]) {
    ...
    if (!concurrent[0] || !sameBinding(concurrent[0], input)) {
      throw new Error("Concurrent protected-entry binding conflicts with this account identity.");
    }
