...
import { getLegalConfigurationStatus, isEntryGateEnforcementEnabled } from "@/lib/legal/legal-config";
import {
  ...
  return db.$transaction(async (tx) => {
    const duplicate = await tx.$queryRaw<EntryAcceptanceRow[]>(Prisma.sql`
      SELECT "id", "documentKey", "documentVersion", "documentSha256", "acceptedAt", "sessionId",
             "userId", "organizationId", "email", "status", "signedAt", "electronicSignatureConsentedAt"
    ...
    if (duplicate[0]) {
      ...
      if (
        ...
        row.status !== "active" ||
        !row.signedAt ||
        !row.electronicSignatureConsentedAt
      ) {
    ...
    if (!inserted[0]) {
      const concurrent = await tx.$queryRaw<EntryAcceptanceRow[]>(Prisma.sql`
        SELECT "id", "documentKey", "documentVersion", "documentSha256", "acceptedAt", "sessionId",
               "userId", "organizationId", "email", "status", "signedAt", "electronicSignatureConsentedAt"
        FROM "access_gate_acceptances"
        WHERE "idempotencyKey" = ${input.idempotencyKey}
        LIMIT 1
      `);
      if (!concurrent[0]) throw new Error("Protected entry acceptance could not be recorded safely.");
      if (
        concurrent[0].sessionId !== input.entrySessionId ||
        concurrent[0].documentSha256 !== sha256 ||
        concurrent[0].documentKey !== input.agreement.documentKey ||
        concurrent[0].documentVersion !== input.agreement.documentVersion
      ) {
        throw new Error("Concurrent entry acceptance does not match the current evidence.");
      }
      return concurrent[0];
    }
    ...
    await tx.$executeRaw(Prisma.sql`
      INSERT INTO "legal_agreement_events" (
        ...
        ${eventId}, ${inserted[0].id}, NULL, NULL, 'legal.entry.accepted_anonymous',
...
export async function findAnonymousEntryAcceptance(input: {
  acceptanceId: string;
  entrySessionId: string;
  agreement: AgreementPresentation;
}) {
...
export async function bindEntryAcceptanceToIdentity(input: {
  ...
  if (input.identity.source !== "database") {
    throw new Error("Protected entry evidence can only bind to a persistent account.");
  }

  await db.$transaction(async (tx) => {
    const anonymous = await tx.$queryRaw<EntryAcceptanceRow[]>(Prisma.sql`
      SELECT "id", "documentKey", "documentVersion", "documentSha256", "acceptedAt", "sessionId",
             "userId", "organizationId", "email", "status", "signedAt", "electronicSignatureConsentedAt"
      FROM "access_gate_acceptances"
      WHERE "id" = ${input.acceptanceId}
    ...
    if (anonymous.length !== 1) {
      throw new Error("Protected entry acceptance is missing, already bound, or ambiguous.");
    }
    ...
    const existing = await tx.$queryRaw<Array<{ id: string }>>(Prisma.sql`
      ...
      WHERE "userId" = ${input.identity.userId}
        AND "organizationId" = ${input.identity.organizationId}
        AND "documentKey" = ${input.documentKey}
        AND "documentVersion" = ${input.documentVersion}
        AND "documentSha256" = ${input.documentSha256}
        AND "status" = 'active'
    ...
    let boundAcceptanceId = input.acceptanceId;
    if (existing[0]) {
      boundAcceptanceId = existing[0].id;
      await tx.$executeRaw(Prisma.sql`
        UPDATE "access_gate_acceptances"
        SET "status" = 'superseded', "supersededAt" = CURRENT_TIMESTAMP, "supersededById" = ${existing[0].id}
        WHERE "id" = ${input.acceptanceId} AND "userId" IS NULL AND "organizationId" IS NULL
      `);
    } else {
      const updated = await tx.$executeRaw(Prisma.sql`
        ...
        SET "userId" = ${input.identity.userId},
            ...
            "source" = 'protected-entry-bound-login'
        WHERE "id" = ${input.acceptanceId}
          AND "sessionId" = ${input.entrySessionId}
          AND "userId" IS NULL
          AND "organizationId" IS NULL
          AND "status" = 'active'
      `);
      if (updated !== 1) throw new Error("Protected entry acceptance could not be bound atomically.");
    }
    ...
    await tx.$executeRaw(Prisma.sql`
      INSERT INTO "legal_agreement_events" (
        ...
        ${eventId}, ${boundAcceptanceId}, ${input.identity.userId}, ${input.identity.organizationId},
        'legal.entry.bound_to_identity', ${input.documentKey}, ${input.documentVersion},
        CAST(${JSON.stringify({ documentSha256: input.documentSha256, entrySessionId: input.entrySessionId, authSessionId: input.authSessionId })} AS JSONB)
      )
    `);
  });
}

export async function hasBoundEntryAcceptance(session: ClinicSession, agreement: AgreementPresentation) {
...
export async function requireEntryAccessForLogin(returnTo?: string | null) {
  if (!isEntryGateEnforcementEnabled()) return null;
  const proof = await readAcceptedEntryProof();
  if (proof) return proof;

  const safeDestination = safeReturnTo(returnTo);
  const loginTarget = safeDestination
    ? `/login?returnTo=${encodeURIComponent(safeDestination)}`
    : "/login";
  redirect(`/access?returnTo=${encodeURIComponent(loginTarget)}`);
}
