...
export async function createEntryPresentedToken(
  entrySessionId: string,
  agreement: EntryAgreementIdentity,
  ...
  }, ENTRY_PRESENTATION_TTL_SECONDS);
}

export async function createEntryReviewedToken(
  ...
  return signClaims({
    ...claims,
    kind: "reviewed",
    reachedEndAt: reachedEndAt.toISOString(),
  }, ENTRY_PRESENTATION_TTL_SECONDS);
}

export async function createEntryAcceptedToken(
  claims: EntryTokenClaims,
  acceptanceId: string,
  acceptedAt = new Date(),
) {
  if (claims.kind !== "reviewed" || !claims.reachedEndAt) {
    throw new Error("Only reviewed entry evidence can establish acceptance.");
  }
  return signClaims({
    ...claims,
    kind: "accepted",
...
export async function verifyEntryToken(
  token: string,
  agreement: EntryAgreementIdentity,
  expectedKind: EntryTokenKind,
  expectedEntrySessionId?: string,
) {
  ...
  if (expectedKind === "reviewed" && !claims.reachedEndAt) {
    throw new Error("Protected entry review evidence is incomplete.");
  }
  if (expectedKind === "accepted" && (!claims.reachedEndAt || !claims.acceptedAt || !claims.acceptanceId)) {
    throw new Error("Protected entry acceptance evidence is incomplete.");
  }
