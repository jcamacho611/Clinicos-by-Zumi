...
export const connectorCatalog: readonly ConnectorDefinition[] = [
  ...
  connector({
    id: "twilio",
    name: "Twilio Messaging / Voice",
    gateway: "communication",
    integration: "webhook_driven",
    ownership: "clinic_owned",
    economics: "B_customer_owned",
    env: ["TWILIO_ACCOUNT_SID", "TWILIO_API_KEY_SID", "TWILIO_API_KEY_SECRET", "TWILIO_MESSAGING_SERVICE_SID"],
    handlesPhi: true,
    baaRequired: true,
    customerConnectable: true,
    gates: NO_GATES,
    status: "configurable",
    externalGate: "Restricted API-key credentials plus Messaging Service/sender readiness. Before clinical messaging, require the approved HIPAA-capable Twilio relationship, BAA/security review, consent and minimum-necessary message policy.",
    notes: "Outbound REST authentication uses the restricted API-key SID/secret. The master Auth Token is not the normal outbound credential and is reserved for capabilities such as future inbound webhook-signature validation that specifically require it.",
  }),
  connector({
    id: "twilio-verify",
    name: "Twilio Verify",
    ...
    ownership: "klinikos_owned",
    economics: "A_platform",
    env: ["TWILIO_ACCOUNT_SID", "TWILIO_API_KEY_SID", "TWILIO_API_KEY_SECRET", "TWILIO_VERIFY_SERVICE_SID"],
    handlesPhi: false,
    baaRequired: false,
    customerConnectable: false,
    gates: NO_GATES,
    status: "configurable",
    externalGate: "Provision an approved Verify service and restricted API-key credentials for account-verification traffic only.",
    notes: "Identity/contact verification, not clinical content. Verification status must come from Twilio's server response, never a browser claim.",
  }),
  ...
  connector({ id: "nppes", name: "CMS NPPES", gateway: "credentialing", integration: "server_only", ownership: "klinikos_owned", economics: "A_platform", env: [], handlesPhi: false, baaRequired: false, customerConnectable: false, gates: NO_GATES, status: "live", externalGate: "Public source availability and schema remain external dependencies; continue runtime monitoring.", notes: "Bounded, time-limited NPI/taxonomy evidence lookup is connected to the credentialing workspace. It informs human review and never activates a provider." }),
  ...
  connector({ id: "oig-leie", name: "HHS OIG LEIE", gateway: "credentialing", integration: "server_only", ownership: "klinikos_owned", economics: "A_platform", env: [], handlesPhi: false, baaRequired: false, customerConnectable: false, gates: NO_GATES, status: "live", externalGate: "Public dataset availability, freshness, and schema remain external dependencies; continue runtime monitoring and documented OIG verification.", notes: "Bounded, cached exact-NPI pre-screening is connected to the credentialing workspace. It is not exclusion clearance or provider activation authority." }),
