 ...
 *
 * SameSite cookies remain useful, but these high-risk routes also require the browser's
 * Origin to match the canonical Klinikos origin. Sec-Fetch-Site is an additional signal,
 * never the sole authority. Server-to-server callers must use a separate authenticated
 * integration surface rather than silently bypassing this browser boundary.
 */
