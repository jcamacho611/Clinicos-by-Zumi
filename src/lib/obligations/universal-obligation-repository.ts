...
function obligationOrder(
  left: UniversalObligationProjection,
  right: UniversalObligationProjection,
) {
...
export async function listUniversalObligations(
  ...
  const [taskRows, referralRows] = await Promise.all([
    ...
    // Source-owned referrals only. Inbound referral visibility has separate
  ...
  const sourceWindowComplete =
    taskRows.length <= SOURCE_LIMIT && referralRows.length <= SOURCE_LIMIT;

  const tasks = taskRows.slice(0, SOURCE_LIMIT).map((task) =>
    projectTaskObligation(
      {
        ...task,
  ...
  const referrals = referralRows.slice(0, SOURCE_LIMIT).map((referral) =>
    projectReferralObligation(
      {
        ...referral,
...
export type UniversalObligationWorkspace = Awaited<
  ReturnType<typeof listUniversalObligations>
>;
