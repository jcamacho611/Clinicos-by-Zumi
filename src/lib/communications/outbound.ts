...
const resendEmailAdapter: OutboundAdapter = {
  ...
  send: async (message, env) => {
    if (!message.to.includes("@")) return { ok: false, reason: "invalid_recipient", detail: "The recipient is not an email address." };
...
export function outboundChannelStatus(channel: OutboundChannel, env: OutboundEnv = process.env) {
  ...
  if (!adapter.configured(env)) return { deliverable: false as const, reason: "no_connector" as const, detail: `No ${channel} provider is configured for this deployment.` };
