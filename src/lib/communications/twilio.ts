...
type TwilioApiKeyCredentials = { accountSid: string; apiKeySid: string; apiKeySecret: string };

function clean(value: string | undefined | null) { return value?.trim() ?? ""; }
function e164(value: string) { return /^\+[1-9]\d{7,14}$/.test(value); }
function messagingServiceSid(value: string) { return /^MG[0-9a-fA-F]{32}$/.test(value); }
function verifySid(value: string) { return /^VE[0-9a-fA-F]{32}$/.test(value); }
...
function authorization(credentials: TwilioApiKeyCredentials) { return `Basic ${Buffer.from(`${credentials.apiKeySid}:${credentials.apiKeySecret}`).toString("base64")}`; }

async function twilioFormRequest(input: { url: string; body: URLSearchParams; credentials: TwilioApiKeyCredentials }) {
  return fetch(input.url, { method: "POST", headers: { authorization: authorization(input.credentials), "content-type": "application/x-www-form-urlencoded;charset=UTF-8" }, body: input.body.toString(), signal: AbortSignal.timeout(10_000) }).catch(() => null);
}

async function twilioJsonGet(input: { url: string; credentials: TwilioApiKeyCredentials }) {
  return fetch(input.url, { method: "GET", headers: { authorization: authorization(input.credentials), accept: "application/json" }, signal: AbortSignal.timeout(10_000) }).catch(() => null);
}
...
export async function sendTwilioSms(input: { to: string; body: string; from?: string | null; messagingServiceSid?: string | null; env?: TwilioEnv }): Promise<TwilioSendResult> {
  ...
  if (!e164(input.to)) return { ok: false, reason: "invalid_recipient", detail: "SMS recipient must be a valid E.164 phone number." };
  const bodyText = input.body.trim();
  if (!bodyText || bodyText.length > 480) return { ok: false, reason: "provider_error", detail: "SMS body must contain 1 to 480 characters." };
  ...
  if (explicitFrom && !e164(explicitFrom)) return { ok: false, reason: "no_sender", detail: "The configured tenant SMS sender is not valid E.164." };
  if (serviceSid && !messagingServiceSid(serviceSid)) return { ok: false, reason: "no_sender", detail: "The configured Twilio Messaging Service SID is invalid." };
  if (!explicitFrom && !serviceSid) return { ok: false, reason: "no_sender", detail: "No valid SMS sender or Twilio Messaging Service is configured." };
  ...
  const response = await twilioFormRequest({ url: `https://api.twilio.com/2010-04-01/Accounts/${encodeURIComponent(credentials.accountSid)}/Messages.json`, body, credentials });
  ...
  if (!payload?.sid || !/^[A-Z]{2}[0-9a-fA-F]{32}$/.test(payload.sid)) return { ok: false, reason: "provider_error", detail: "Twilio accepted the request without a valid evidentiary message SID." };
...
export type TwilioSmsRoutingVerificationResult =
  ...
  | { ok: false; reason: "not_configured" | "invalid_sender" | "invalid_messaging_service_sid" | "sender_not_owned" | "sender_not_in_service" | "sender_not_sms_capable" | "provider_error"; detail: string };

export async function verifyTwilioSmsRouting(input: { senderPhone: string; messagingServiceSid: string; env?: TwilioEnv }): Promise<TwilioSmsRoutingVerificationResult> {
  const env = input.env ?? process.env;
  const credentials = twilioApiKeyCredentials(env);
  if (!credentials) return { ok: false, reason: "not_configured", detail: "Twilio restricted API-key credentials are not configured." };
  ...
  ownedUrl.searchParams.set("PhoneNumber", senderPhone); ownedUrl.searchParams.set("PageSize", "2");
  ...
  const ownedPayload = (await ownedResponse.json().catch(() => null)) as { incoming_phone_numbers?: Array<{ sid?: string; phone_number?: string }> } | null;
  ...
  const poolUrl = new URL(`https://messaging.twilio.com/v1/Services/${encodeURIComponent(serviceSid)}/PhoneNumbers`); poolUrl.searchParams.set("PageSize", "1000");
  ...
  const poolPayload = (await poolResponse.json().catch(() => null)) as { phone_numbers?: Array<{ sid?: string; phone_number?: string; service_sid?: string; capabilities?: string[] }> } | null;
  ...
  if (Array.isArray(poolMatch.capabilities) && !poolMatch.capabilities.includes("SMS")) return { ok: false, reason: "sender_not_sms_capable", detail: "The configured sender is not SMS-capable in this Messaging Service." };
...
function verifyService(env: TwilioEnv) { const serviceSid = clean(env.TWILIO_VERIFY_SERVICE_SID); return /^VA[0-9a-fA-F]{32}$/.test(serviceSid) ? serviceSid : null; }

export async function startTwilioPhoneVerification(input: { to: string; env?: TwilioEnv }): Promise<TwilioVerifyResult> {
  const env = input.env ?? process.env; const credentials = twilioApiKeyCredentials(env); const serviceSid = verifyService(env);
  ...
  const response = await twilioFormRequest({ url: `https://verify.twilio.com/v2/Services/${encodeURIComponent(serviceSid)}/Verifications`, body: new URLSearchParams({ To: input.to, Channel: "sms" }), credentials });
...
export async function checkTwilioPhoneVerification(input: { to: string; code: string; env?: TwilioEnv }): Promise<TwilioVerifyResult> {
  ...
  const response = await twilioFormRequest({ url: `https://verify.twilio.com/v2/Services/${encodeURIComponent(serviceSid)}/VerificationCheck`, body: new URLSearchParams({ To: input.to, Code: input.code.trim() }), credentials });
  if (!response) return { ok: false, reason: "provider_error", detail: "Twilio Verify could not be reached." };
  if (!response.ok) return { ok: false, reason: "provider_error", detail: `Twilio Verify returned HTTP ${response.status}.` };
  const payload = (await response.json().catch(() => null)) as { sid?: string; status?: string } | null;
