...
function phoneNumberSid(value: string) {
  return /^PN[0-9a-fA-F]{32}$/.test(value);
}

function verifyServiceSid(value: string) {
  return /^VA[0-9a-fA-F]{32}$/.test(value);
}
...
function messageSid(value: string) {
  return /^(SM|MM)[0-9a-fA-F]{32}$/.test(value);
}
 ...
 *
 * Patient authorization is not decided here. The governed patient service must supply
 * its tenant sender and Messaging Service only after consent, identity, routing,
 * production, quiet-hours, and commercial-spend gates have passed.
 */
export async function sendTwilioSms(input: {
  ...
  if (!payload?.sid || !messageSid(payload.sid)) return { ok: false, reason: "provider_error", detail: "Twilio accepted the request without a valid evidentiary message SID." };
...
/** Prove that a tenant sender is owned and belongs to the configured Messaging Service. */
export async function verifyTwilioSmsRouting(input: {
  ...
  const ownedMatches = (ownedPayload?.incoming_phone_numbers ?? []).filter((item) => item.phone_number === senderPhone && phoneNumberSid(item.sid ?? ""));
  ...
  const poolMatch = (poolPayload?.phone_numbers ?? []).find((item) => item.sid === owned.sid && item.phone_number === senderPhone && item.service_sid === serviceSid);
  ...
  return { ok: true, phoneNumberSid: owned.sid!, messagingServiceSid: serviceSid, smsCapable: true };
}
...
function verifyService(env: TwilioEnv) {
  ...
  return verifyServiceSid(serviceSid) ? serviceSid : null;
}
...
export async function startTwilioPhoneVerification(input: { to: string; env?: TwilioEnv }): Promise<TwilioVerifyResult> {
  ...
  if (!response) return { ok: false, reason: "provider_error", detail: "Twilio Verify could not be reached within the request timeout." };
