...
export async function createAccountSession(
  identity: MemberIdentity | ClinicAccountIdentity,
...
async function validateAccountSession(token: string): Promise<AccountSession | null> {
  ...
  try {
    ...
    if (
      ...
      persisted.account.personId !== claims.personId ||
      persisted.account.primaryEmail !== claims.email
    ) {
      return null;
    }
    ...
    const legacyUser = await db.user.findUnique({
      where: { id: claims.legacyUserId },
      include: { organization: true },
    });
    if (
      !legacyUser ||
      legacyUser.status !== "active" ||
      legacyUser.organization.status !== "active" ||
      legacyUser.organizationId !== claims.organizationId ||
      legacyUser.organization.name !== claims.organizationName ||
      legacyUser.organization.slug !== claims.organizationSlug ||
      legacyUser.email.trim().toLowerCase() !== claims.email ||
      normalizeClinicRole(legacyUser.roleKey) !== claims.role
    ) {
      return null;
    }
...
export async function requireAccountClinicSession() {
  ...
  // Re-validation above proves the AccountSession, Account, Person,
  // legacyUserAccountLink, legacy User and organization are still current.
...
export async function revokeAccountSession(session: AccountSession | null) {
  ...
  await db.accountSession.updateMany({
    where: { id: session.sessionId, accountId: session.accountId, revokedAt: null },
