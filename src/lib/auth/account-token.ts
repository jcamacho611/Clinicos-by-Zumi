...
function key() { return new TextEncoder().encode(getAuthSecret()); }

export function accountSessionCookieOptions() {
  return { httpOnly: true, secure: process.env.NODE_ENV === "production", sameSite: "lax" as const, path: "/", maxAge: ACCOUNT_SESSION_TTL_SECONDS };
}

export async function signAccountSessionToken(session: AccountSession) {
  return new SignJWT({ ...session })
    .setProtectedHeader({ alg: "HS256", typ: "JWT" })
    .setIssuer(issuer).setAudience(audience).setSubject(session.accountId).setJti(session.sessionId)
    .setIssuedAt().setExpirationTime(session.expiresAt).sign(key());
}
