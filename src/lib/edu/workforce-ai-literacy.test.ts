...
describe("workforce AI literacy foundation", () => {
  ...
  it("makes practice and human instruction explicit", () => {
    expect(workforceAiDeliveryPrinciples.some((principle) => principle.toLowerCase().includes("live instructors"))).toBe(true);
    expect(workforceAiDeliveryPrinciples.some((principle) => principle.toLowerCase().includes("learners practice doing work"))).toBe(true);
  });

  it("distinguishes the built healthcare foundation from configurable pathways", () => {
    expect(getWorkforceAiOccupationalPathway("healthcare")?.status).toBe("built_healthcare_foundation");

    for (const pathway of workforceAiOccupationalPathways) {
      if (pathway.key === "healthcare") continue;
      expect(pathway.status).toBe("configured_training_pathway");
