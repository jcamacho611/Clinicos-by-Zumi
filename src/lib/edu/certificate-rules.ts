...
export const eduCertificateMutationSchema = z.discriminatedUnion("action", [
  issueEduCertificateSchema,
  revokeEduCertificateSchema,
]);
...
export function validateEduCertificateEvidence(input: {
  ...
  if (input.certificateType === "competency_evidence") {
    ...
    if (!requested.length) {
      problems.push("Competency evidence must name at least one competency area.");
    }
    const demonstrated = new Set(input.demonstratedCompetencyAreas);
    const missing = requested.filter((area) => !demonstrated.has(area));
    if (missing.length) {
      problems.push(`Competency has not been human-determined as demonstrated for: ${missing.join(", ")}.`);
    }
  }
