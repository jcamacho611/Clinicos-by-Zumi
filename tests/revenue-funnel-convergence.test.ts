...
describe("clinic revenue funnel convergence", () => {
  ...
  it("does not imply the $500 analysis activates production software", () => {
    ...
    expect(salesGuide).toContain("Paid analysis ≠ software activation");
  });
  ...
  it("documents Stripe as conditional preferred rail and GoDaddy as fallback", () => {
    expect(salesGuide).toContain("Stripe is preferred only when both live Checkout credentials and the signed live webhook secret are configured");
    expect(salesGuide).toContain("GoDaddy paylink/manual-reconciliation path");
    expect(salesGuide).toContain("A browser redirect/return is not payment proof");
  });
});
