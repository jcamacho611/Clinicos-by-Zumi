...
describe("Zumi governed memory and knowledge authority", () => {
  ...
  it("ranks query-relevant reviewed knowledge ahead of unrelated context without changing authority", () => {
    const ranked = rankZumiGovernedContext([personal, organization, globalReference], "Why are our callbacks overdue?", 3);
  ...
  it("lets organization-approved knowledge override a same-title global reference for that organization", () => {
    const globalPolicy = { ...organization, id: "global-policy", scope: "global" as const, authority: "human_approved_global_reference" as const, content: "Escalate callbacks after 48 hours.", version: 1 };
    const organizationPolicy = { ...organization, id: "org-policy", content: "Escalate callbacks after 24 hours.", version: 2 };
  ...
  it("keeps the specialist reference subordinate to the Master Canon", () => {
    const reference = readFileSync("docs/ZUMI_MEMORY_AND_KNOWLEDGE_OS_CANON.md", "utf8");
    expect(reference).toContain("SPECIALIST_REFERENCE");
    expect(reference).toContain("SUBORDINATE TO `docs/KLINIKOS_MASTER_CANON.md`");
    expect(reference).not.toContain("AUTHORITATIVE SPECIALIST CANON");
    ...
    ]) expect(reference).toContain(invariant);
  });
});
