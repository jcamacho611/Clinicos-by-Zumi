...
describe("Klinikos global agreement", () => {
  ...
  it("requires extra professional and Grid acknowledgments without weakening base assent", () => {
    ...
    expect(owner.map(({ key }) => key)).toEqual(["terms", "confidentiality", "electronic_signature", "ai_authority", "ai_service_processing"]);
    expect(provider.map(({ key }) => key)).toContain("professional_truth");
    expect(contractor.map(({ key }) => key)).toEqual(expect.arrayContaining(["terms", "confidentiality", "electronic_signature", "ai_authority", "ai_service_processing", "professional_truth", "grid_truth"]));
  });
