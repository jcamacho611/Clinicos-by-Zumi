...
describe("P16 security evidence", () => {
  ...
  it("rejects future verification timestamps", () => {
    const path = mutate((ledger) => {
      ledger.controls[0].lastVerifiedAt = "2999-01-01T00:00:00Z";
    });
  ...
  it("rejects expired evidence that still claims production verified", () => {
    const path = mutate((ledger) => {
      ledger.controls[0] = {
        ...
        operationalEvidenceRefs: [".github/workflows/quality.yml"],
  ...
  it("rejects PHI external rails without legal and external evidence", () => {
    const path = mutate((ledger) => {
      ledger.controls[0] = {
        ...
        environments: ["production"],
        dataClasses: ["PHI"],
        capabilities: ["external_ai_phi"],
        state: "PRODUCTION_VERIFIED",
        technicalEvidenceRefs: ["src/features/zumi"],
        operationalEvidenceRefs: ["docs/ZUMI.md"],
        externalEvidenceRefs: [],
        legalEvidenceRefs: [],
      };
    });
