...
describe("universal trust wiring", () => {
  it("projects an active Grid dispute without pretending a refund or restriction executed", () => {
    const [signal] = projectGridTrustSignals({
      disputes: [{ id: "dispute-1", reservationId: "reservation-1", category: "payment_disagreement", summary: "The parties disagree about the cancellation charge.", status: "under_review", updatedAt: "2026-08-22T20:00:00.000Z" }],
      safetyIncidents: [],
    });
    expect(signal).toMatchObject({ id: "grid:dispute:dispute-1", domain: "grid", kind: "commercial_dispute", status: "under_review", severity: "review", open: true, blocksSettlement: true, authority: { refundExecuted: false, payoutActionExecuted: false, participantRestrictionExecuted: false, resourceSuspensionExecuted: false, medicalDeterminationMade: false }, source: { recordType: "GridDisputeRecord", recordId: "dispute-1" } });
  });

  it("projects an active Grid safety incident with severity while preserving governance boundaries", () => {
    const [signal] = projectGridTrustSignals({ disputes: [], safetyIncidents: [{ id: "incident-1", reservationId: "reservation-1", category: "credential_concern", severity: "urgent", summary: "Credential evidence appears inconsistent with the offered service.", status: "triage_required", updatedAt: "2026-08-22T21:00:00.000Z" }] });
    expect(signal).toMatchObject({ id: "grid:safety:incident-1", domain: "grid", kind: "safety_incident", severity: "urgent", open: true, blocksSettlement: true });
  ...
  it("does not keep settlement blocked after the governed source record is closed", () => {
    const [signal] = projectGridTrustSignals({ disputes: [{ id: "dispute-closed", reservationId: "reservation-2", category: "no_show", summary: "The reservation was reviewed and the dispute record was closed.", status: "closed", updatedAt: "2026-08-22T19:00:00.000Z" }], safetyIncidents: [] });
  ...
  it("orders normalized signals by most recent source evidence without changing source truth", () => {
    const signals = projectGridTrustSignals({ disputes: [{ id: "older", reservationId: "reservation-1", category: "other", summary: "Older commercial issue with enough factual detail.", status: "open", updatedAt: "2026-08-22T18:00:00.000Z" }], safetyIncidents: [{ id: "newer", reservationId: "reservation-1", category: "conduct_concern", severity: "high", summary: "Newer safety concern with enough factual detail for review.", status: "open", updatedAt: "2026-08-22T22:00:00.000Z" }] });
