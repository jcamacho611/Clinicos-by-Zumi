...
const configuredEnv = {
  ...
  // Dated TEST FIXTURES only. Production must configure the selected model's current
  // rates from the provider rather than relying on values embedded in application code.
...
describe("Cloudflare Zumi adapter", () => {
  ...
  it("refuses provider selection when cost-rate configuration is omitted", () => {
    const {
      ZUMI_CLOUDFLARE_INPUT_MICRO_USD_PER_M_TOKENS: _input,
      ZUMI_CLOUDFLARE_OUTPUT_MICRO_USD_PER_M_TOKENS: _output,
      ...withoutRates
    } = configuredEnv;
