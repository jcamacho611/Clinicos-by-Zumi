...
- Whop-first paid portal entry with a server-resolved access-pass catalog at `/entry`, an access-terms clickwrap and verified-work-email prerequisite, server-issued checkout intents, a signature-verified Whop webhook as the authoritative grant/revoke path, an append-only webhook ledger, self-expiring entitlements, and a `/entry/return` confirmation that reports only what Whop actually confirmed
- Marketplace access purchases at `/pricing` covering one-time clinic workflow review, Founding Clinic seat, contractor application review, room/chair listing review, seller listing review, and advisory call fees, with server-controlled pricing, an explicit "what this does not include" disclosure on every product, a manual payment-reference path at `/payments/verify`, human verification in `/admin/payments`, and a paid-onboarding record that keeps portal access closed until review approves it
- GRID marketplace access layer that gates listing publication, request creation, request decisions, location listings, and payout records on an active access pass, while keeping license and malpractice verification as a separate human-reviewed gate that payment never satisfies
...
| `WHOP_API_KEY` | Paid entry | Server-side Whop API key used to confirm a membership before any access is granted; without it paid entry reports `Pending Connection` |
| `WHOP_WEBHOOK_SECRET` | Paid entry | Shared secret for `x-whop-signature` verification; without it `/api/whop/webhook` rejects every delivery |
| `WHOP_PLAN_EVALUATOR_PASS` | Paid entry | Whop plan id sold as the Evaluation Access Pass |
| `WHOP_PLAN_CLINIC_OPERATOR` | Paid entry | Whop plan id sold as Clinic Operator Access |
| `WHOP_PLAN_GRID_PROVIDER` | Paid entry | Whop plan id sold as GRID Provider Access |
| `WHOP_PLAN_GRID_LOCATION_PARTNER` | Paid entry | Whop plan id sold as GRID Location Partner Access |
| `WHOP_API_BASE` | No | Overrides the Whop API host; defaults to `https://api.whop.com` |
| `WHOP_CHECKOUT_BASE` | No | Overrides the hosted checkout host; defaults to `https://whop.com` |
| `WHOP_WEBHOOK_ALLOW_UNTIMESTAMPED` | No | Set `true` only if the configured Whop webhook signs without a timestamp; leaving it unset keeps replay protection on |
| `ACCESS_EMAIL_FROM` | No | From address for access-verification email |
| `STRIPE_SECRET_KEY` | No | Future payment integration |
| `STRIPE_WEBHOOK_SECRET` | No | Future payment webhook verification |
| `RESEND_API_KEY` | Production access gate | Transactional email delivery for access verification; required in production |
...
- `POST /api/commerce/payments/create` rate-limits public purchase starts and resolves price, currency, and role target from the server product catalog; the request body may name a product but never a price. A product with no configured checkout link is reported as `Pending Connection` rather than ordered. Creating a payment records intent only — it grants nothing.
- `POST /api/commerce/payments/reference` lets a buyer submit a provider reference on the manual return path. It moves the payment to `pending_verification` for a human to confirm and never marks it paid. Its failure response is deliberately uniform so the public endpoint cannot be used to probe who has purchased.
- `GET|POST /api/commerce/payments/verify` requires the sales-manage permission. Administrators record verify, hold, fail, refund, and reconcile decisions with a mandatory note; portal access is derived from the resulting status and the product's review requirement rather than set by the caller, a payment cannot be recorded as settled without a provider reference, and every decision writes an audit receipt.
- These routes are namespaced under `/api/commerce` to stay clearly separate from `/api/payments`, which is patient billing scoped to a patient inside a tenant.
- `POST /api/whop/checkout` rate-limits public checkout starts, requires a catalog tier with a configured Whop plan id and an already-verified work email, and returns only a hosted Whop checkout URL bound to a server-issued single-use state value. It never creates an entitlement.
- `POST /api/whop/webhook` verifies `x-whop-signature` against the raw request body before parsing it, rejects stale or unsigned deliveries, records every delivery (including rejections) in an append-only ledger keyed for idempotent redelivery, and re-confirms the membership against the Whop API before writing a grant. It fails closed when no webhook secret is configured.
- `GET /api/whop/entitlement` returns only the signed-in account's own access state, resolved from the session rather than any client-supplied identifier.
- `GET /entry` renders the access-pass catalog with each tier's capabilities, required agreements, and remaining human review; a tier with no configured plan id is shown as `Pending Connection` rather than as orderable. `GET /entry/return` reconciles a checkout by server-side lookup and reports only the state Whop actually confirmed.
- `GET /access/verify` consumes a single-use verification token server-side and marks the recorded acceptance as email-verified, which is the prerequisite for paid entry.
- GRID marketplace mutations (`POST /api/grid/services`, `/api/grid/requests`, `/api/grid/requests/:requestId/transition`, `/api/grid/locations`, `/api/grid/payouts/:payoutId/transition`) additionally require an active access pass covering the specific marketplace action. Denials that a purchase or renewal would resolve return `402` with an entry URL; denials waiting on human credential review return `403`, because no payment resolves them. Every denial writes an `authorization.denied` audit event. `GET /api/grid` stays open to authorized roles and reports the same access summary the guard enforces, so the workspace cannot offer an action the API would reject.
...
Migration `20260810120000_access_email_verification` adds the `access_email_verifications` table and the `verifiedEmailAt` column that the access gate's verification service already depended on, along with status and verification-evidence checks. The application code for these was merged without a matching migration.

Migration `20260810140000_marketplace_access_payments` adds the marketplace `access_payments` and `paid_onboardings` records, kept separate from the patient-billing `payments` table. PostgreSQL checks confine products, role targets, statuses, and access states to their known vocabularies; require a positive server-controlled amount; require a provider reference before a payment may read as settled; require a recorded human verifier and timestamp together; prevent portal access from being granted off a payment that never settled; and require a named reviewer on any onboarding approval. No card data is stored — only a provider reference to the external transaction.

Migration `20260810123000_whop_paid_access` adds the Whop entitlement grant, the append-only webhook ledger, and the server-issued checkout intent. PostgreSQL checks confine entitlements to catalog tiers and known states, require provenance (`grantedAt` and `lastVerifiedAt`) on any active grant, prevent a revoked grant from also reading as active, and prevent a webhook delivery from being marked `applied` unless its signature verified.
...
- A live Whop company, access passes, and configured webhook endpoint. The Klinikos side of paid entry is implemented — catalog, checkout handoff, signature-verified webhook, entitlement ledger, and marketplace gating — but until `WHOP_API_KEY`, `WHOP_WEBHOOK_SECRET`, and at least one plan id are configured, `/entry` reports `Pending Connection` and no entitlement can be granted. Refunds, disputes, chargebacks, tax handling, and subscription proration remain Whop's responsibility and are not mirrored into Klinikos. Periodic revalidation of stored entitlements against Whop is not yet scheduled, so a grant currently relies on webhook delivery plus its own `validUntil` expiry
