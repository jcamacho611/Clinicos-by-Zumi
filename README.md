...
- PostgreSQL-backed Grid demonstration with credential-aware provider profiles, hidden admin identity fields, verified service listings, location/chair inventory, provider availability, cross-organization request states, consent/deposit gates, dual-tenant audit receipts, and authenticated `/grid/*` plus `/admin/grid` workspaces
...
- `GET /api/grid` returns the credential-aware Grid command context with tenant-owned verification details and public-safe cross-organization listings. The Grid provider, service, availability, location, and request APIs require explicit network permissions; provider and request transition routes enforce human review, credential, consent, deposit, and lifecycle gates while writing append-only events and organization-specific audit receipts.
