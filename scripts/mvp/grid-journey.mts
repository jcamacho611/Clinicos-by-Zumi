...
async function main() {
  await reset();
  const org = await db.organization.create({ data: { name: "MVP Grid Clinic", slug: SLUG, clinicType: "medspa", status: "active", demoMode: true }, select: { id: true } });
  const supplier = await db.organization.create({ data: { name: "MVP Grid Supplier", slug: SUPPLIER_SLUG, clinicType: "medspa", status: "active", demoMode: true }, select: { id: true } });
  const user = await db.user.create({ data: { organizationId: org.id, email: "grid-owner@mvp.test", name: "Grid Owner", roleKey: "clinic_owner" }, select: { id: true } });
  const supplierUser = await db.user.create({ data: { organizationId: supplier.id, email: "supplier-owner@mvp.test", name: "Supplier Owner", roleKey: "clinic_owner" }, select: { id: true } });
  ...
  let demandError = "";
  try {
    const demand = await createSavedGridDemand(session, { kind: "provider", title: "Cover a Friday clinic", description: "Need an eligible clinical professional for one full day of appointments.", category: "Clinical coverage", requestedStartAt: iso(7 * DAY), requestedEndAt: iso(7 * DAY + 8 * 3600 * 1000), state: "NY", quantity: 1, status: "open", visibility: "matched_only", requirements: ["Required professional credential", "Clinic eligibility review"] });
    demandId = (demand as { id: string }).id;
  } catch (error) { demandError = error instanceof Error ? error.message : "unknown"; }
  ...
  check("provider class: the saved need is readable back only in its organization", savedDemands.some((d) => (d as { id: string }).id === demandId), `${savedDemands.length} demand(s) visible to this organization`);

  let offerId = "";
  let offerError = "";
  try {
    const offer = await createGridOffer(session, { demandId, resourceKind: "provider", resourceReference: resourceId, recipientOrganizationId: supplier.id, offeredStartAt: iso(7 * DAY), offeredEndAt: iso(7 * DAY + 8 * 3600 * 1000), grossAmountCents: 120_000, depositAmountCents: 20_000, locationPayableCents: 0, note: "Offering reviewed professional capacity at the agreed day rate.", expiresAt: iso(2 * DAY) });
    offerId = (offer as { id: string }).id;
  } catch (error) { offerError = error instanceof Error ? error.message : "unknown"; }
  ...
  let prematureReason = "";
  try { await createReservationFromAcceptedOffer(session, offerId); } catch (error) { prematureRefused = true; prematureReason = error instanceof Error ? error.message : "unknown"; }
  ...
  let accepted = false;
  let acceptError = "";
  try { await transitionGridOffer(supplierSession, offerId, { targetStatus: "accepted", note: "Accepting the offered professional coverage." }); accepted = true; } catch (error) { acceptError = error instanceof Error ? error.message : "unknown"; }
  ...
  try { const reservation = await createReservationFromAcceptedOffer(session, offerId); reservationId = (reservation as { id: string }).id; } catch (error) { reservationError = error instanceof Error ? error.message : "unknown"; }
  ...
  let doubleReason = "";
  try { await createReservationFromAcceptedOffer(session, offerId); } catch (error) { doubleRefused = true; doubleReason = error instanceof Error ? error.message : "unknown"; }
  ...
  check("provider class: the same offer cannot be reserved twice", doubleRefused || forThisOffer.length <= 1, doubleRefused ? `refused: ${doubleReason}` : `${forThisOffer.length} reservation(s) for this offer`);
  ...
  try {
    const demand = await createSavedGridDemand(session, { kind: "space", title: "Treatment room for five hours", description: "Need reviewed treatment-room capacity for an approved operational use.", category: "Treatment room", requestedStartAt: spaceStart, requestedEndAt: spaceEnd, state: "NY", quantity: 1, status: "open", visibility: "matched_only", requirements: ["Reviewed healthcare space"] });
    spaceDemandId = (demand as { id: string }).id;
  } catch (error) { spaceDemandError = error instanceof Error ? error.message : "unknown"; }
  ...
  let spaceOfferError = "";
  try {
    const offer = await createGridOffer(session, { demandId: spaceDemandId, resourceKind: "space", resourceReference: spaceResourceId, recipientOrganizationId: supplier.id, offeredStartAt: spaceStart, offeredEndAt: spaceEnd, grossAmountCents: 50_000, depositAmountCents: 10_000, locationPayableCents: 50_000, note: "Offering the reviewed treatment-room availability window.", expiresAt: iso(2 * DAY) });
    spaceOfferId = (offer as { id: string }).id;
  } catch (error) { spaceOfferError = error instanceof Error ? error.message : "unknown"; }
  ...
  try { await transitionGridOffer(supplierSession, spaceOfferId, { targetStatus: "accepted", note: "Accepting the treatment-room offer." }); spaceAccepted = true; } catch (error) { spaceAcceptError = error instanceof Error ? error.message : "unknown"; }
  ...
  try { const reservation = await createReservationFromAcceptedOffer(session, spaceOfferId); spaceReservationId = (reservation as { id: string }).id; } catch (error) { spaceReservationError = error instanceof Error ? error.message : "unknown"; }
  check("space class: accepted space offer becomes a reservation only inside real availability", Boolean(spaceReservationId), spaceReservationError || `reservation ${spaceReservationId.slice(0, 12)}…`);
